...
class CrossAttention(nn.Module):
    ...
    def __init__(
        ...
        if self.use_attn_mask:
            ...
            action_horizon = attn_mask_kwargs['action_horizon'] # 实际上是预测长度，而非执行长度
    
            # Create a unified mask tensor: (4, action_horizon, max_cond_tokens)
            masks = torch.ones((2, action_horizon, max_cond_tokens), dtype=torch.bool)
            
            # 只会mask robot state，也就是last action
            masks[0, :, obs_part_length[0]:obs_part_length[1]] = False
            ...
            # # mask 3: only mask third part
            # masks[2, :, obs_part_length[0]+obs_part_length[1]:obs_part_length[0]+obs_part_length[1]+obs_part_length[2]] = False
            
            # mask 4: no masking
            
            self.register_buffer('masks', masks)
            self.register_buffer('probs', torch.tensor([0.1, 0.9]))
