        ...
        with JsonLogger(log_path) as json_logger:
            for local_epoch_idx in range(cfg.training.num_epochs):
                ...
                def log_action_mse(step_log, category, pred_action, gt_action):
                    ...
                    overall_mse = torch.nn.functional.mse_loss(pred_action, gt_action)
                    step_log[f'{category}_action_mse_error'] = overall_mse.item()

                    dim_mse = torch.mean((pred_action - gt_action)**2, dim=(0, 1))
                    
                    for i in range(D):
                        step_log[f'{category}_action_mse_dim_{i}'] = dim_mse[i].item()
