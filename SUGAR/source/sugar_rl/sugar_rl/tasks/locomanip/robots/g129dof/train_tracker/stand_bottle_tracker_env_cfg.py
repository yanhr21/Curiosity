...
from sugar_rl.assets.objects.objects import BOTTLE_CFG
import torch

from .base_tracker_env_cfg import BaseRobotSceneCfg, BaseCommandsCfg, BaseActionsCfg, BaseObservationsCfg, BaseEventCfg, BaseRewardsCfg, BaseTerminationsCfg

@configclass
class RobotSceneCfg(BaseRobotSceneCfg):
    obj: RigidObjectCfg = BOTTLE_CFG.replace(prim_path="{ENV_REGEX_NS}/Obj")
    ...
    undesired_contacts = RewTerm(
        ...
        params={
            "sensor_cfg": SceneEntityCfg(
                "contact_forces",
                body_names=[
                    r"^(?!left_ankle_roll_link$)(?!right_ankle_roll_link$)(?!left_rubber_hand$)(?!right_rubber_hand$)(?!left_knee_link$)(?!right_knee_link$).+$"
                ],
            ),
            "threshold": 1.0,
        },
...
class RobotRolloutPlayEnvCfg(RobotEnvCfg):
    def __post_init__(self):
        ...
        self.terminations.trajectory_complete = None
        self.terminations.rollout_window_timeout = DoneTerm(
            func=mdp.rollout_window_timeout,
