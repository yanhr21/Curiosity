...
15:53 在 server31 核实已完成 24 次真实更新，warmup 后 LR 为 `1e-5`。
24 次更新均有限且 GPU/CPU AdamW 主参数回读一致；八个样本记录始终相同，已使用
192 个不同噪声种子。前两次检查的 64 个 cross-attention 模块均有非零梯度。
16/24 步触发既定梯度裁剪；第 24 次范数 `1.20156`，未触发裁剪。平均每步约
`450.55 s`，主要耗时仍为 CPU 梯度搬运／累加。第 24 次单次 video/action/IFP loss 为
`0.0186574 / 1.1237062 / 0.3403513`；由于每步噪声／时间不同，不把它们与平均初始
探针直接相除来宣称通过。终点评估和新画面尚未完成。
...
基线，同时报告新段相对父终点的变化。当前进程不会热加载这些入口修改，仍在完成首个
32 步实验。15:58 最新 64 项 CPU 回归通过，包含权重／状态文件恢复及恢复后下一步的
