...
if [[ "$(hostname)" == mgmtserver* ]]; then
  echo "Refusing to run G1 low-creep positive-brake tuning on login/management node: $(hostname)" >&2
  exit 2
fi
...
SUITE_STAMP="${SUITE_STAMP:-$(date +%Y%m%d_core_world_g1_low_creep_positive_brake_tune)}"
SUITE_DIR="${SUITE_DIR:-${ROOT_DIR}/experiments/outputs/core_world_g1_low_creep_positive_brake_tune/${SUITE_STAMP}}"
DEVICE="${DEVICE:-cpu}"
STRICT="${STRICT:-0}"
STEPS="${STEPS:-700}"
...
status_file="${SUITE_DIR}/low_creep_positive_brake_tune_status.tsv"
...
run_case() {
  ...
  mkdir -p "${out}"
  echo "[POS-BRAKE] ${case_id} brake_pitch=${brake_pitch} brake_bias=${brake_bias}"
  ...
  if [[ "${STRICT}" == "1" && ( "${build_status}" != "0" || "${check_status}" != "0" ) ]]; then
    echo "[POS-BRAKE] strict mode stopping after ${case_id}: build=${build_status} check=${check_status}" >&2
    exit 1
  fi
}

run_case posbrake008_bias0 0.08 0.0 0.10 0.25 0.14 0.10
run_case posbrake010_bias0 0.10 0.0 0.12 0.28 0.16 0.12
run_case posbrake012_bias0 0.12 0.0 0.14 0.32 0.18 0.14
run_case posbrake010_bias02 0.10 0.2 0.12 0.28 0.16 0.12
run_case posbrake012_bias02 0.12 0.2 0.14 0.32 0.18 0.14

echo "[POS-BRAKE] status file: ${status_file}"
