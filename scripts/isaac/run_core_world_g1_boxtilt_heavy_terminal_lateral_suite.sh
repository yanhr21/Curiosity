...
if [[ "$(hostname)" == mgmtserver* ]]; then
  echo "Refusing to run G1 boxtilt heavy terminal-lateral suite on login/management node: $(hostname)" >&2
  exit 2
fi

ROOT_DIR="${ROOT_DIR:-/public/home/yanhongru/Curiosity}"
SUITE_STAMP_PREFIX="${SUITE_STAMP_PREFIX:-20260707_g1_boxtilt_heavy_terminal_lateral}"
OUTPUT_ROOT="${OUTPUT_ROOT:-${ROOT_DIR}/experiments/outputs/core_world_g1_boxtilt_heavy_terminal_lateral/${SUITE_STAMP_PREFIX}}"

cd "${ROOT_DIR}"
mkdir -p "${OUTPUT_ROOT}"

status_file="${OUTPUT_ROOT}/boxtilt_heavy_terminal_lateral_status.tsv"
summary_out="${OUTPUT_ROOT}/boxtilt_heavy_terminal_lateral_summary.json"
...
run_case() {
  ...
  echo "[G1-BOXTILT-TERMLAT] case=${case_name} suite_stamp=${suite_stamp}"
...
run_case termlat_excess030_l006 \
  AGILE_COMMAND_HOLD_LATERAL_ERROR_START=0.30 \
  AGILE_COMMAND_HOLD_LATERAL_GAIN=0.04 \
  AGILE_COMMAND_HOLD_LATERAL_LIMIT=0.006

run_case termlat_excess030_l012 \
  AGILE_COMMAND_HOLD_LATERAL_ERROR_START=0.30 \
  AGILE_COMMAND_HOLD_LATERAL_GAIN=0.04 \
  AGILE_COMMAND_HOLD_LATERAL_LIMIT=0.012

run_case termlat_excess045_l012 \
  AGILE_COMMAND_HOLD_LATERAL_ERROR_START=0.45 \
  AGILE_COMMAND_HOLD_LATERAL_GAIN=0.04 \
  AGILE_COMMAND_HOLD_LATERAL_LIMIT=0.012

run_case termlat_tiltgate_l012 \
  AGILE_COMMAND_HOLD_LATERAL_ERROR_START=0.30 \
  AGILE_COMMAND_HOLD_LATERAL_GAIN=0.04 \
  AGILE_COMMAND_HOLD_LATERAL_LIMIT=0.012 \
  AGILE_COMMAND_HOLD_LATERAL_MAX_TILT=0.38 \
  AGILE_COMMAND_HOLD_LATERAL_MAX_BOX_TILT=0.55
