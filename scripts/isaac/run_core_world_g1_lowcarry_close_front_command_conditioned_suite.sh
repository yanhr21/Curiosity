...
if [[ "$(hostname)" == mgmtserver* ]]; then
  echo "Refusing to run G1 close-front command-conditioned suite on login/management node: $(hostname)" >&2
  exit 2
fi

ROOT_DIR="${ROOT_DIR:-/public/home/yanhongru/Curiosity}"
SUITE_STAMP_PREFIX="${SUITE_STAMP_PREFIX:-20260707_g1_lowcarry_close_front_command_conditioned}"
OUTPUT_ROOT="${OUTPUT_ROOT:-${ROOT_DIR}/experiments/outputs/core_world_g1_lowcarry_close_front_command_conditioned/${SUITE_STAMP_PREFIX}}"

mkdir -p "${OUTPUT_ROOT}"
status_file="${OUTPUT_ROOT}/close_front_command_conditioned_status.tsv"
summary_out="${OUTPUT_ROOT}/close_front_command_conditioned_summary.json"
...
run_case() {
  ...
  echo "[G1-CLOSE-CMD] case=${case_name} command_y=${command_y} suite_stamp=${suite_stamp}"
...
run_case command_y_neg002 -0.02 || overall_status=1
run_case command_y_neg004 -0.04 || overall_status=1
run_case command_y_neg006 -0.06 || overall_status=1
