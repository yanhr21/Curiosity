...
if [[ "$(hostname)" == mgmtserver* ]]; then
  echo "Refusing to run G1 close-front repair suite on login/management node: $(hostname)" >&2
  exit 2
fi

ROOT_DIR="${ROOT_DIR:-/public/home/yanhongru/Curiosity}"
SUITE_STAMP_PREFIX="${SUITE_STAMP_PREFIX:-20260707_g1_lowcarry_close_front_repair}"
OUTPUT_ROOT="${OUTPUT_ROOT:-${ROOT_DIR}/experiments/outputs/core_world_g1_lowcarry_close_front_repair/${SUITE_STAMP_PREFIX}}"

mkdir -p "${OUTPUT_ROOT}"
status_file="${OUTPUT_ROOT}/close_front_repair_status.tsv"
summary_out="${OUTPUT_ROOT}/close_front_repair_summary.json"
...
run_case() {
  ...
  echo "[G1-CLOSE-REPAIR] case=${case_name} suite_stamp=${suite_stamp}"
...
run_case lateral_sign_neg || overall_status=1
run_case lateral_sign_neg_stronger \
  AGILE_COMMAND_HOLD_LATERAL_GAIN=0.012 \
  AGILE_COMMAND_HOLD_LATERAL_LIMIT=0.004 || overall_status=1
run_case lateral_sign_neg_stronger_tiltpad \
  AGILE_COMMAND_HOLD_LATERAL_GAIN=0.012 \
  AGILE_COMMAND_HOLD_LATERAL_LIMIT=0.004 \
  CRADLE_CHEST_PAD_ENABLE_ON_BOX_TILT=1 \
  CRADLE_CHEST_PAD_BOX_TILT_THRESHOLD=0.35 \
  CRADLE_CHEST_PAD_BOX_TILT_MIN_STEP=600 || overall_status=1
