...
if [[ "$(hostname)" == mgmtserver* ]]; then
  echo "Refusing to run G1 close-front rescue/lateral refine suite on login/management node: $(hostname)" >&2
  exit 2
fi

ROOT_DIR="${ROOT_DIR:-/public/home/yanhongru/Curiosity}"
SUITE_STAMP_PREFIX="${SUITE_STAMP_PREFIX:-20260707_g1_lowcarry_close_front_rescue_lateral_refine}"
OUTPUT_ROOT="${OUTPUT_ROOT:-${ROOT_DIR}/experiments/outputs/core_world_g1_lowcarry_close_front_rescue_lateral_refine/${SUITE_STAMP_PREFIX}}"

cd "${ROOT_DIR}"
mkdir -p "${OUTPUT_ROOT}"

status_file="${OUTPUT_ROOT}/close_front_rescue_lateral_refine_status.tsv"
summary_out="${OUTPUT_ROOT}/close_front_rescue_lateral_refine_summary.json"
...
run_case() {
  ...
  echo "[G1-CLOSE-FRONT-RESCUE-LATERAL] case=${case_name} suite_stamp=${suite_stamp}"
  ...
  env \
    ...
    MAX_FINAL_HOLD_COMMAND_Y=0.0035 \
    ...
    AGILE_COMMAND_BOX_LATERAL_DEADBAND=0.08 \
    AGILE_COMMAND_BOX_LATERAL_GAIN=0.018 \
    AGILE_COMMAND_BOX_LATERAL_LIMIT=0.0025 \
...
run_case rescue040_lat_unscaled \
  AGILE_COMMAND_BOX_LATERAL_SCALE_ON_HOLD=0

run_case rescue040_lat_unscaled_signneg \
  AGILE_COMMAND_BOX_LATERAL_SCALE_ON_HOLD=0 \
  AGILE_COMMAND_BOX_LATERAL_SIGN=-1.0

run_case rescue040_milder_crouch \
  AGILE_COMMAND_BOX_LATERAL_SCALE_ON_HOLD=0 \
  AGILE_COMMAND_HOLD_RESCUE_BLEND_RATE=0.08 \
  AGILE_COMMAND_HOLD_RESCUE_HIP_PITCH=-0.18 \
  AGILE_COMMAND_HOLD_RESCUE_KNEE=0.50 \
  AGILE_COMMAND_HOLD_RESCUE_ANKLE_PITCH=-0.30 \
  AGILE_COMMAND_HOLD_RESCUE_WAIST_PITCH=-0.06

run_case rescue045_mid_crouch \
  AGILE_COMMAND_BOX_LATERAL_SCALE_ON_HOLD=0 \
  AGILE_COMMAND_HOLD_RESCUE_ABS_ROLL_THRESHOLD=0.45 \
  AGILE_COMMAND_HOLD_RESCUE_BLEND_RATE=0.10 \
  AGILE_COMMAND_HOLD_RESCUE_HIP_PITCH=-0.23 \
  AGILE_COMMAND_HOLD_RESCUE_KNEE=0.60 \
  AGILE_COMMAND_HOLD_RESCUE_ANKLE_PITCH=-0.34 \
  AGILE_COMMAND_HOLD_RESCUE_WAIST_PITCH=-0.08
