...
if [[ "$(hostname)" == mgmtserver* ]]; then
  echo "Refusing to run G1 low-carry 0.60 kg runtime chest-pad timing suite on login/management node: $(hostname)" >&2
  exit 2
fi

ROOT_DIR="${ROOT_DIR:-/public/home/yanhongru/Curiosity}"
SUITE_STAMP_PREFIX="${SUITE_STAMP_PREFIX:-20260707_g1_lowcarry_060_runtime_chestpad_timing}"
OUTPUT_ROOT="${OUTPUT_ROOT:-${ROOT_DIR}/experiments/outputs/core_world_g1_lowcarry_060_runtime_chestpad_timing/${SUITE_STAMP_PREFIX}}"

mkdir -p "${OUTPUT_ROOT}"
status_file="${OUTPUT_ROOT}/runtime_chestpad_timing_status.tsv"
summary_out="${OUTPUT_ROOT}/runtime_chestpad_timing_summary.json"
printf "case\tstatus\tsuite_stamp\n" > "${status_file}"
summary_case_roots=()

run_timing_case() {
  local case_name="$1"
  local target_min_step="$2"
  local suite_stamp="${SUITE_STAMP_PREFIX}_${case_name}"
  echo "[G1-060-RUNTIME-CHEST-TIMING] case=${case_name} target_min_step=${target_min_step} suite_stamp=${suite_stamp}"
  ...
  env \
    ...
    CRADLE_CHEST_PAD_TARGET_WINDOW_MIN_STEP="${target_min_step}" \
...
run_timing_case target_window_min680 680 || overall_status=1
run_timing_case target_window_min700 700 || overall_status=1
run_timing_case target_window_min720 720 || overall_status=1
run_timing_case target_window_min740 740 || overall_status=1
