...
if [[ "$(hostname)" == mgmtserver* ]]; then
  echo "Refusing to run G1 close-front tilt-escape suite on login/management node: $(hostname)" >&2
  exit 2
fi
...
status_file="${OUTPUT_ROOT}/close_front_tilt_escape_status.tsv"
summary_out="${OUTPUT_ROOT}/close_front_tilt_escape_summary.json"
...
run_case() {
  ...
  echo "[G1-CLOSE-FRONT-TILT-ESCAPE] case=${case_name} suite_stamp=${suite_stamp}"
