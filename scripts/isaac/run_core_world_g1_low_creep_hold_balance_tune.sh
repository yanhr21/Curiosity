...
if [[ "$(hostname)" == mgmtserver* ]]; then
  echo "Refusing to run G1 low-creep hold+balance tuning on login/management node: $(hostname)" >&2
  exit 2
fi
...
SUITE_STAMP="${SUITE_STAMP:-$(date +%Y%m%d_core_world_g1_low_creep_hold_balance_tune)}"
SUITE_DIR="${SUITE_DIR:-${ROOT_DIR}/experiments/outputs/core_world_g1_low_creep_hold_balance_tune/${SUITE_STAMP}}"
DEVICE="${DEVICE:-cpu}"
STRICT="${STRICT:-0}"
STEPS="${STEPS:-700}"
...
status_file="${SUITE_DIR}/low_creep_hold_balance_tune_status.tsv"
...
base_python=(
  ...
  --balance-pitch-activation-threshold 0.04
  --balance-pitch-rate-activation-threshold 0.18
)
...
run_case() {
  ...
  local pitch_sign="$4"
  local limit="$5"
  local min_final_travel="$6"
  local max_tilt="$7"
  local max_rel="$8"
  local max_final_rel="$9"
  local out="${SUITE_DIR}/${case_id}"
  local build_log="${out}/build.log"
  local check_log="${out}/check.json"
  mkdir -p "${out}"
  echo "[HOLD-BAL] ${case_id} hold=${hold_travel} gain=${pitch_gain} sign=${pitch_sign} limit=${limit}"
  ...
  "${base_python[@]}" \
    ...
    --balance-pitch-sign "${pitch_sign}" \
    --balance-adjustment-limit "${limit}" \
  ...
  if [[ "${STRICT}" == "1" && ( "${build_status}" != "0" || "${check_status}" != "0" ) ]]; then
    echo "[HOLD-BAL] strict mode stopping after ${case_id}: build=${build_status} check=${check_status}" >&2
    exit 1
  fi
}

run_case hold008_gain035_signneg 0.08 0.35 -1.0 0.16 0.08 0.30 0.16 0.12
run_case hold010_gain035_signneg 0.10 0.35 -1.0 0.16 0.10 0.32 0.18 0.14
run_case hold010_gain060_signneg 0.10 0.60 -1.0 0.22 0.10 0.35 0.20 0.16
run_case hold010_gain035_signpos 0.10 0.35 1.0 0.16 0.10 0.35 0.20 0.16
run_case hold012_gain035_signneg 0.12 0.35 -1.0 0.16 0.12 0.36 0.22 0.18

echo "[HOLD-BAL] status file: ${status_file}"
