...
if [[ "$(hostname)" == mgmtserver* ]]; then
  echo "Refusing to run G1 held-out geometry planted-stance suite on login/management node: $(hostname)" >&2
  exit 2
fi

ROOT_DIR="${ROOT_DIR:-/public/home/yanhongru/Curiosity}"
SUITE_STAMP_PREFIX="${SUITE_STAMP_PREFIX:-20260707_g1_closefront_heldout_geometry_planted_stance}"
OUTPUT_ROOT="${OUTPUT_ROOT:-${ROOT_DIR}/experiments/outputs/core_world_g1_closefront_heldout_geometry_planted_stance/${SUITE_STAMP_PREFIX}}"

mkdir -p "${OUTPUT_ROOT}"
status_file="${OUTPUT_ROOT}/closefront_heldout_geometry_planted_stance_status.tsv"
summary_out="${OUTPUT_ROOT}/closefront_heldout_geometry_planted_stance_summary.json"
...
run_case() {
  ...
  echo "[G1-HELDOUT-GEOMETRY-PLANTED-STANCE] case=${case_name} suite_stamp=${suite_stamp}"
  ...
  env \
    ...
    AGILE_COMMAND_HOLD_POLICY_THEN_STAND_DELAY_STEPS=24 \
    AGILE_COMMAND_HOLD_STAND_BLEND_RATE=0.12 \
    AGILE_COMMAND_HOLD_SCALE=0.018 \
    ...
    AGILE_COMMAND_HOLD_STAND_HIP_PITCH=-0.13 \
    AGILE_COMMAND_HOLD_STAND_KNEE=0.42 \
    AGILE_COMMAND_HOLD_STAND_ANKLE_PITCH=-0.26 \
    AGILE_COMMAND_HOLD_STAND_HIP_ROLL=0.0 \
    AGILE_COMMAND_HOLD_STAND_ANKLE_ROLL=0.0 \
    AGILE_COMMAND_HOLD_STAND_WAIST_PITCH=-0.035 \
    ...
    AGILE_COMMAND_TERMINAL_SUPPORT_MAX_TILT=0.80 \
    AGILE_COMMAND_TERMINAL_SUPPORT_MAX_BOX_TILT=0.90 \
...
run_case wide_y012_planted \
  FREE_BOX_SIZE_Y=0.12 \
  AGILE_COMMAND_STOP_BOX_TARGET_TRAVEL=1.25 \
  AGILE_COMMAND_TERMINAL_SUPPORT_START_BOX_TARGET_TRAVEL=1.10 \
  CRADLE_FINAL_SIDE_GUARD_HALF_SPACING=0.13 \
  CRADLE_FINAL_SIDE_GUARD_MASS_SCALE=0.15 || overall_status=1

run_case tall_z009_planted \
  FREE_BOX_SIZE_Z=0.09 \
  AGILE_COMMAND_STOP_BOX_TARGET_TRAVEL=1.55 \
  AGILE_COMMAND_TERMINAL_SUPPORT_START_BOX_TARGET_TRAVEL=1.35 || overall_status=1
