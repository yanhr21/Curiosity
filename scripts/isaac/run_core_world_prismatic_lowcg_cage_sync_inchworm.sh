...
echo "[CONFIG] STAMP=${STAMP} low-CG cage sync_inchworm STEPS=${STEPS:-1000} TARGET_X=${TARGET_X:-0.030} STEP_LENGTH=${STEP_LENGTH:-0.015} STEP_HEIGHT=${STEP_HEIGHT:-0.020} SETTLE_STEPS=${SETTLE_STEPS:-260} GAIT_PERIOD_STEPS=${GAIT_PERIOD_STEPS:-320}"
"${ISAAC_VENV}/bin/python" \
  ...
  --step-length "${STEP_LENGTH:-0.015}" \
  --step-height "${STEP_HEIGHT:-0.020}" \
  --gait-period-steps "${GAIT_PERIOD_STEPS:-320}" \
  --swing-fraction "${SWING_FRACTION:-0.16}" \
  --settle-steps "${SETTLE_STEPS:-260}" \
  --ramp-steps "${RAMP_STEPS:-220}" \
