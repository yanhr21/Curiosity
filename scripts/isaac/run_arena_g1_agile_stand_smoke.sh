...
LOG_DIR="${LOG_DIR:-${ROOT_DIR}/logs/arena_g1_agile_stand_smoke}"
OUTPUT_DIR="${OUTPUT_DIR:-${ROOT_DIR}/experiments/outputs/arena_g1_agile_stand_smoke/${STAMP}}"
...
LOG_PATH="${LOG_DIR}/arena_g1_agile_stand_smoke_${STAMP}.log"
echo "[CONFIG] STAMP=${STAMP} Arena G1 AGILE stand/walk STEPS=${STEPS:-300} BASE_HEIGHT=${BASE_HEIGHT_COMMAND:-0.75} COMMAND=(${COMMAND_X:-0.0},${COMMAND_Y:-0.0},${COMMAND_YAW:-0.0}) COMMAND_START_STEP=${COMMAND_START_STEP:-80}"
EXTRA_ARGS=()
if [[ "${SKIP_ENV_RESET:-0}" == "1" ]]; then
  EXTRA_ARGS+=(--skip-env-reset)
fi
"${ISAAC_VENV}/bin/python" \
  "${ROOT_DIR}/scripts/isaac/build_arena_g1_agile_stand_smoke.py" \
  --headless \
