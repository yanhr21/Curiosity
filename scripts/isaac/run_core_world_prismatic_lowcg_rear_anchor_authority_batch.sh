...
if [[ "$(hostname)" == mgmtserver* ]]; then
  echo "Refusing to run low-CG rear-anchor authority batch on login/management node: $(hostname)" >&2
  exit 2
fi
...
run_diag() {
  ...
  local max_force="$5"
  local rear_mu="$6"
  local stamp="20260705_lowcg_rear_anchor_authority_${diag}_target${target_x}_k${stiffness}_f${max_force}"
  local output_dir="experiments/outputs/core_world_prismatic_carrier_stand/${stamp}"
  echo "[BATCH] ${diag} target=${target_x} x_k=${stiffness} x_d=${damping} x_force=${max_force} rear_mu=${rear_mu}"
  "${ISAAC_VENV}/bin/python" scripts/isaac/build_core_world_prismatic_carrier_stand.py \
    ...
    --x-slide-limit 0.08 \
    --x-slide-stiffness "${stiffness}" \
    --x-slide-damping "${damping}" \
    --x-slide-max-force "${max_force}" \
...
run_diag diag9 0.030 90000.0 8000.0 120000.0 8.0
run_diag diag10 0.030 300000.0 12000.0 600000.0 8.0
run_diag diag11 -0.030 300000.0 12000.0 600000.0 8.0
