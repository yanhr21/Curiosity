...
if [[ "$(hostname)" == mgmtserver* ]]; then
  echo "Refusing to run G1 held-out geometry adaptive-support suite on login/management node: $(hostname)" >&2
  exit 2
fi

ROOT_DIR="${ROOT_DIR:-/public/home/yanhongru/Curiosity}"
SUITE_STAMP_PREFIX="${SUITE_STAMP_PREFIX:-20260707_g1_closefront_heldout_geometry_adaptive_support}"
OUTPUT_ROOT="${OUTPUT_ROOT:-${ROOT_DIR}/experiments/outputs/core_world_g1_closefront_heldout_geometry_adaptive_support/${SUITE_STAMP_PREFIX}}"

mkdir -p "${OUTPUT_ROOT}"
status_file="${OUTPUT_ROOT}/closefront_heldout_geometry_adaptive_support_status.tsv"
summary_out="${OUTPUT_ROOT}/closefront_heldout_geometry_adaptive_support_summary.json"
...
run_case() {
  ...
  echo "[G1-HELDOUT-GEOMETRY-ADAPTIVE] case=${case_name} suite_stamp=${suite_stamp}"
...
run_case wide_y012_terminal_guard \
  FREE_BOX_SIZE_Y=0.12 \
  CRADLE_FINAL_SIDE_GUARD_ENABLE_ON_FINAL_HOLD=0 \
  CRADLE_FINAL_SIDE_GUARD_ENABLE_ON_TERMINAL_HOLD=1 || overall_status=1

run_case tall_z009_final_stand \
  FREE_BOX_SIZE_Z=0.09 \
  AGILE_COMMAND_HOLD_FINAL_STAND=1 \
  AGILE_COMMAND_HOLD_FINAL_STAND_DELAY_STEPS=120 \
  AGILE_COMMAND_HOLD_STAND_BLEND_RATE=0.02 || overall_status=1
