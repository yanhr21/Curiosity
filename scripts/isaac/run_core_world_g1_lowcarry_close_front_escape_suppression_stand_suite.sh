...
if [[ "$(hostname)" == mgmtserver* ]]; then
  echo "Refusing to run G1 close-front escape-suppression stand suite on login/management node: $(hostname)" >&2
  exit 2
fi

ROOT_DIR="${ROOT_DIR:-/public/home/yanhongru/Curiosity}"
SUITE_STAMP_PREFIX="${SUITE_STAMP_PREFIX:-20260707_g1_lowcarry_close_front_escape_suppression_stand}"
OUTPUT_ROOT="${OUTPUT_ROOT:-${ROOT_DIR}/experiments/outputs/core_world_g1_lowcarry_close_front_escape_suppression_stand/${SUITE_STAMP_PREFIX}}"

cd "${ROOT_DIR}"
mkdir -p "${OUTPUT_ROOT}"

status_file="${OUTPUT_ROOT}/close_front_escape_suppression_stand_status.tsv"
summary_out="${OUTPUT_ROOT}/close_front_escape_suppression_stand_summary.json"
...
run_case() {
  ...
  echo "[G1-CLOSE-FRONT-ESCAPE-SUPPRESS-STAND] case=${case_name} stand_delay=${stand_delay} blend_rate=${blend_rate} suite_stamp=${suite_stamp}"
  ...
  env \
    ...
    AGILE_COMMAND_HOLD_FINAL_STAND_DELAY_STEPS="${stand_delay}" \
    AGILE_COMMAND_HOLD_STAND_BLEND_RATE="${blend_rate}" \
...
run_case suppress60_stand240_blend002 240 0.02
run_case suppress60_stand300_blend004 300 0.04
