...
if [[ "$(hostname)" == mgmtserver* ]]; then
  echo "Refusing to run drive-authority validation on login/management node: $(hostname)" >&2
  exit 2
fi
...
run_diag() {
  ...
  local gain_scale="$2"
  local force_scale="$3"
  local terminal_pitch="$4"
  local stamp="20260705_core_world_g1_drive_authority_${diag}_gain${gain_scale}_force${force_scale}_tp${terminal_pitch}"
  local output_dir="experiments/outputs/core_world_g1_box_scene/${stamp}"
  local terminal_args=()
  if [[ "${terminal_pitch}" != "off" ]]; then
    terminal_args=(
      --terminal-hold-pitch-threshold "${terminal_pitch}"
  ...
  echo "[BATCH] ${diag} gain_scale=${gain_scale} force_scale=${force_scale} terminal_pitch=${terminal_pitch}"
  "${ISAAC_VENV}/bin/python" scripts/isaac/build_core_world_g1_box_scene.py \
    ...
    --stand-gain-scale "${gain_scale}" \
    --stand-force-scale "${force_scale}" \
...
run_diag diag89 1.0 2.0 off
run_diag diag90 1.0 4.0 off
run_diag diag91 2.0 2.0 0.20
run_diag diag92 2.0 4.0 0.20
