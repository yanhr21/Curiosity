...
if [[ "$(hostname)" == mgmtserver* ]]; then
  echo "Refusing to run G1 close-front retention-posture suite on login/management node: $(hostname)" >&2
  exit 2
fi

ROOT_DIR="${ROOT_DIR:-/public/home/yanhongru/Curiosity}"
SUITE_STAMP_PREFIX="${SUITE_STAMP_PREFIX:-20260707_g1_lowcarry_close_front_retention_posture}"
OUTPUT_ROOT="${OUTPUT_ROOT:-${ROOT_DIR}/experiments/outputs/core_world_g1_lowcarry_close_front_retention_posture/${SUITE_STAMP_PREFIX}}"
CASE_SET="${RETENTION_POSTURE_CASE_SET:-quick}"

cd "${ROOT_DIR}"
mkdir -p "${OUTPUT_ROOT}"

status_file="${OUTPUT_ROOT}/close_front_retention_posture_status.tsv"
summary_out="${OUTPUT_ROOT}/close_front_retention_posture_summary.json"
...
run_case() {
  ...
  echo "[G1-CLOSE-RETENTION-POSTURE] case=${case_name} suite_stamp=${suite_stamp}"
  ...
  env \
    ...
    BOX_RETENTION_POSTURE_CONTROLLER=1 \
...
case "${CASE_SET}" in
  quick)
    run_case retention_mild \
      BOX_RETENTION_REL_START=0.07 \
      BOX_RETENTION_REL_STOP=0.18 \
      BOX_RETENTION_TILT_START=0.22 \
      BOX_RETENTION_TILT_STOP=0.48 \
      BOX_RETENTION_BLEND_RATE=0.08 \
      BOX_RETENTION_HIP_PITCH_OFFSET=-0.03 \
      BOX_RETENTION_KNEE_OFFSET=0.08 \
      BOX_RETENTION_ANKLE_PITCH_OFFSET=-0.04 \
      BOX_RETENTION_WAIST_PITCH_OFFSET=-0.02 \
      BOX_RETENTION_SHOULDER_PITCH_OFFSET=-0.08 \
      BOX_RETENTION_ELBOW_OFFSET=0.12 \
      BOX_RETENTION_WRIST_PITCH_OFFSET=-0.03
    ;;
  full)
    run_case retention_mild \
      BOX_RETENTION_REL_START=0.07 \
    ...
    run_case retention_strong \
      BOX_RETENTION_REL_START=0.05 \
      BOX_RETENTION_REL_STOP=0.16 \
      BOX_RETENTION_TILT_START=0.18 \
      BOX_RETENTION_TILT_STOP=0.42 \
      BOX_RETENTION_BLEND_RATE=0.12 \
      BOX_RETENTION_HIP_PITCH_OFFSET=-0.06 \
      BOX_RETENTION_KNEE_OFFSET=0.16 \
      BOX_RETENTION_ANKLE_PITCH_OFFSET=-0.08 \
      BOX_RETENTION_WAIST_PITCH_OFFSET=-0.05 \
      BOX_RETENTION_SHOULDER_PITCH_OFFSET=-0.12 \
      BOX_RETENTION_ELBOW_OFFSET=0.20 \
      BOX_RETENTION_WRIST_PITCH_OFFSET=-0.06
    ;;
  *)
    echo "Unknown RETENTION_POSTURE_CASE_SET=${CASE_SET}; expected quick or full" >&2
    exit 2
    ;;
esac
