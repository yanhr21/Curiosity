...
if [[ "$(hostname)" == mgmtserver* ]]; then
  echo "Refusing to run G1 close-front command refine suite on login/management node: $(hostname)" >&2
  exit 2
fi

ROOT_DIR="${ROOT_DIR:-/public/home/yanhongru/Curiosity}"
SUITE_STAMP_PREFIX="${SUITE_STAMP_PREFIX:-20260707_g1_lowcarry_close_front_command_refine}"
OUTPUT_ROOT="${OUTPUT_ROOT:-${ROOT_DIR}/experiments/outputs/core_world_g1_lowcarry_close_front_command_refine/${SUITE_STAMP_PREFIX}}"

mkdir -p "${OUTPUT_ROOT}"
status_file="${OUTPUT_ROOT}/close_front_command_refine_status.tsv"
summary_out="${OUTPUT_ROOT}/close_front_command_refine_summary.json"
...
run_case() {
  ...
  echo "[G1-CLOSE-CMD-REFINE] case=${case_name} command_x=${command_x} command_y=${command_y} suite_stamp=${suite_stamp}"
...
run_case x011_yneg004 0.11 -0.04 || overall_status=1
run_case x012_yneg004 0.12 -0.04 || overall_status=1
run_case x012_yneg003 0.12 -0.03 || overall_status=1
