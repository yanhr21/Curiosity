...
if [[ "$(hostname)" == mgmtserver* ]]; then
  echo "Refusing to run G1 low-creep decel tuning on login/management node: $(hostname)" >&2
  exit 2
fi
...
SUITE_STAMP="${SUITE_STAMP:-$(date +%Y%m%d_core_world_g1_low_creep_decel_tune)}"
SUITE_DIR="${SUITE_DIR:-${ROOT_DIR}/experiments/outputs/core_world_g1_low_creep_decel_tune/${SUITE_STAMP}}"
DEVICE="${DEVICE:-cpu}"
STRICT="${STRICT:-0}"
STEPS="${STEPS:-700}"
...
status_file="${SUITE_DIR}/low_creep_decel_tune_status.tsv"
...
run_case() {
  ...
  local brake_push="$9"
  local brake_bias="${10}"
  local min_final_travel="${11}"
  local max_tilt="${12}"
  local max_rel="${13}"
  local max_final_rel="${14}"
  local out="${SUITE_DIR}/${case_id}"
  local build_log="${out}/build.log"
  local check_log="${out}/check.json"
  mkdir -p "${out}"
  echo "[LOW-DECEL] ${case_id} decel=${decel_start}->${decel_end} min=(${min_amp},${min_push},${min_bias}) brake_pitch=${brake_pitch}"

  local build_status=0
  set +e
  "${base_python[@]}" \
    --creep-decel-box-travel-start "${decel_start}" \
    --creep-decel-box-travel-end "${decel_end}" \
    --creep-decel-robot-travel-start "${decel_start}" \
    --creep-decel-robot-travel-end "${decel_end}" \
    --creep-min-amplitude-scale "${min_amp}" \
    --creep-min-push-scale "${min_push}" \
    --creep-min-bias-scale "${min_bias}" \
    --creep-pitch-brake-threshold "${brake_pitch}" \
    --creep-pitch-brake-rate-threshold 999.0 \
    --creep-pitch-brake-amplitude-scale "${brake_amp}" \
    --creep-pitch-brake-push-scale "${brake_push}" \
    --creep-pitch-brake-bias-scale "${brake_bias}" \
  ...
  if [[ "${STRICT}" == "1" && ( "${build_status}" != "0" || "${check_status}" != "0" ) ]]; then
    echo "[LOW-DECEL] strict mode stopping after ${case_id}: build=${build_status} check=${check_status}" >&2
    exit 1
  fi
}

run_case decel010_018_zero 0.10 0.18 0.0 0.0 0.0 999.0 0.0 0.0 0.0 0.12 0.25 0.14 0.10
run_case decel012_020_zero 0.12 0.20 0.0 0.0 0.0 999.0 0.0 0.0 0.0 0.14 0.25 0.14 0.10
run_case decel014_024_zero 0.14 0.24 0.0 0.0 0.0 999.0 0.0 0.0 0.0 0.16 0.25 0.14 0.10
run_case decel010_018_creep05 0.10 0.18 0.05 0.05 0.0 999.0 0.0 0.0 0.0 0.12 0.25 0.14 0.10
run_case decel012_022_creep05 0.12 0.22 0.05 0.05 0.0 999.0 0.0 0.0 0.0 0.14 0.25 0.14 0.10
run_case decel012_022_brake010 0.12 0.22 0.05 0.05 0.0 0.10 0.0 0.0 0.0 0.14 0.25 0.14 0.10
run_case decel014_024_brake012 0.14 0.24 0.05 0.05 0.0 0.12 0.0 0.0 0.0 0.16 0.25 0.14 0.10

echo "[LOW-DECEL] status file: ${status_file}"
