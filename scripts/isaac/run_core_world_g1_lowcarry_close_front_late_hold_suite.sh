...
if [[ "$(hostname)" == mgmtserver* ]]; then
  echo "Refusing to run G1 close-front late-hold suite on login/management node: $(hostname)" >&2
  exit 2
fi

ROOT_DIR="${ROOT_DIR:-/public/home/yanhongru/Curiosity}"
SUITE_STAMP_PREFIX="${SUITE_STAMP_PREFIX:-20260707_g1_lowcarry_close_front_late_hold}"
OUTPUT_ROOT="${OUTPUT_ROOT:-${ROOT_DIR}/experiments/outputs/core_world_g1_lowcarry_close_front_late_hold/${SUITE_STAMP_PREFIX}}"

cd "${ROOT_DIR}"
mkdir -p "${OUTPUT_ROOT}"

status_file="${OUTPUT_ROOT}/close_front_late_hold_status.tsv"
summary_out="${OUTPUT_ROOT}/close_front_late_hold_summary.json"
...
run_case() {
  ...
  echo "[G1-CLOSE-FRONT-LATE-HOLD] case=${case_name} suite_stamp=${suite_stamp}"
  ...
  env \
    ...
    MIN_AGILE_COMMAND_HOLD_FINAL_ACTIVE_STEPS=300 \
...
run_case late_final_180

run_case late_final_180_freeze \
  AGILE_COMMAND_HOLD_FINAL_FREEZE_IN_TARGET_WINDOW=1 \
  AGILE_COMMAND_HOLD_FINAL_FREEZE_MAX_TILT=0.38 \
  AGILE_COMMAND_HOLD_FINAL_FREEZE_MAX_BOX_TILT=0.48 \
  AGILE_COMMAND_HOLD_FINAL_ZERO_CORRECTIONS=1

run_case late_final_180_brake \
  AGILE_COMMAND_HOLD_FINAL_BRAKE_COMMAND_X=-0.012 \
  AGILE_COMMAND_HOLD_FINAL_BRAKE_DELAY_STEPS=0 \
  AGILE_COMMAND_HOLD_FINAL_BRAKE_STEPS=80 \
  MAX_FINAL_HOLD_COMMAND_X=0.013
