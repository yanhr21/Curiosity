...
if [[ "$(hostname)" == mgmtserver* ]]; then
  echo "Refusing to run G1 close-front escape-suppression suite on login/management node: $(hostname)" >&2
  exit 2
fi

ROOT_DIR="${ROOT_DIR:-/public/home/yanhongru/Curiosity}"
SUITE_STAMP_PREFIX="${SUITE_STAMP_PREFIX:-20260707_g1_lowcarry_close_front_escape_suppression}"
OUTPUT_ROOT="${OUTPUT_ROOT:-${ROOT_DIR}/experiments/outputs/core_world_g1_lowcarry_close_front_escape_suppression/${SUITE_STAMP_PREFIX}}"
CASE_SET="${ESCAPE_SUPPRESSION_CASE_SET:-default}"

cd "${ROOT_DIR}"
mkdir -p "${OUTPUT_ROOT}"

status_file="${OUTPUT_ROOT}/close_front_escape_suppression_status.tsv"
summary_out="${OUTPUT_ROOT}/close_front_escape_suppression_summary.json"
...
run_case() {
  ...
  echo "[G1-CLOSE-FRONT-ESCAPE-SUPPRESS] case=${case_name} suppress_streak=${suppress_streak} suite_stamp=${suite_stamp}"
  ...
  env \
    ...
    AGILE_COMMAND_HOLD_FINAL_TILT_ESCAPE_SUPPRESS_AFTER_TARGET_WINDOW_STREAK="${suppress_streak}" \
...
case "${CASE_SET}" in
  default)
    run_case suppress_after_streak60 60
    run_case suppress_after_streak80 80
    ;;
  quick)
    run_case suppress_after_streak80 80
    ;;
  *)
    echo "Unknown ESCAPE_SUPPRESSION_CASE_SET=${CASE_SET}; expected default or quick" >&2
    exit 2
    ;;
esac
