...
env "${COMMON_ENV[@]}" \
  STAMP=20260705_prismatic_cradle_walklike_high_close_x045_z018_drive023_retry15a \
  PAYLOAD_LOCAL_X=0.45 \
  GAIT_DRIVE_TARGET_X=-0.23 \
  bash scripts/isaac/run_core_world_prismatic_carrier_stand.sh

env "${COMMON_ENV[@]}" \
  STAMP=20260705_prismatic_cradle_walklike_high_mid_x050_z018_drive031_retry15b \
  PAYLOAD_LOCAL_X=0.50 \
  GAIT_DRIVE_TARGET_X=-0.31 \
  bash scripts/isaac/run_core_world_prismatic_carrier_stand.sh

env "${COMMON_ENV[@]}" \
  STAMP=20260705_prismatic_cradle_walklike_high_close_x045_z018_drive027_retry15c \
  PAYLOAD_LOCAL_X=0.45 \
  GAIT_DRIVE_TARGET_X=-0.27 \
  bash scripts/isaac/run_core_world_prismatic_carrier_stand.sh
