...
if [[ "$(hostname)" == mgmtserver* ]]; then
  echo "Refusing to run staged-gait refinement on login/management node: $(hostname)" >&2
  exit 2
fi
...
run_diag() {
  ...
  local ramp_start="$4"
  local ramp_end="$5"
  local recovery_threshold="$6"
  local stamp="20260705_core_world_g1_staged_refine_${diag}_mass${cradle_mass}_amp${amp}_ramp${ramp_start}_${ramp_end}"
  local output_dir="experiments/outputs/core_world_g1_box_scene/${stamp}"
  echo "[BATCH] ${diag} cradle_mass=${cradle_mass} amp=${amp} ramp=${ramp_start}-${ramp_end} recovery=${recovery_threshold}"
...
run_diag diag71 0.85 0.12 110 220 0.10
run_diag diag72 0.90 0.11 100 210 0.10
run_diag diag73 0.95 0.10 90 190 0.10
run_diag diag74 1.00 0.10 80 180 0.10
