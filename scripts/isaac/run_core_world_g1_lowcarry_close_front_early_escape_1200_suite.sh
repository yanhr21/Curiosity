...
if [[ "$(hostname)" == mgmtserver* ]]; then
  echo "Refusing to run G1 close-front early-escape 1200-step suite on login/management node: $(hostname)" >&2
  exit 2
fi

ROOT_DIR="${ROOT_DIR:-/public/home/yanhongru/Curiosity}"
SUITE_STAMP_PREFIX="${SUITE_STAMP_PREFIX:-20260707_g1_lowcarry_close_front_early_escape_1200}"
OUTPUT_ROOT="${OUTPUT_ROOT:-${ROOT_DIR}/experiments/outputs/core_world_g1_lowcarry_close_front_early_escape_1200/${SUITE_STAMP_PREFIX}}"
CASE_SET="${EARLY_ESCAPE_1200_CASE_SET:-default}"

cd "${ROOT_DIR}"
mkdir -p "${OUTPUT_ROOT}"

status_file="${OUTPUT_ROOT}/close_front_early_escape_1200_status.tsv"
summary_out="${OUTPUT_ROOT}/close_front_early_escape_1200_summary.json"
...
run_case() {
  ...
  echo "[G1-CLOSE-FRONT-EARLY-ESCAPE-1200] case=${case_name} suite_stamp=${suite_stamp}"
...
case "${CASE_SET}" in
  default)
    run_case baseline_target_window_pad

    run_case baseline_box_tilt_pad \
      CRADLE_CHEST_PAD_ENABLE_ON_BOX_TILT=1 \
      CRADLE_CHEST_PAD_BOX_TILT_THRESHOLD=0.28 \
  ...
  quick)
    ...
    ;;
  *)
    echo "Unknown EARLY_ESCAPE_1200_CASE_SET=${CASE_SET}; expected default or quick" >&2
    exit 2
    ;;
esac
