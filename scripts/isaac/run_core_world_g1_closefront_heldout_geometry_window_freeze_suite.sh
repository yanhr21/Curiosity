...
if [[ "$(hostname)" == mgmtserver* ]]; then
  echo "Refusing to run G1 held-out geometry window-freeze suite on login/management node: $(hostname)" >&2
  exit 2
fi

ROOT_DIR="${ROOT_DIR:-/public/home/yanhongru/Curiosity}"
SUITE_STAMP_PREFIX="${SUITE_STAMP_PREFIX:-20260707_g1_closefront_heldout_geometry_window_freeze}"
OUTPUT_ROOT="${OUTPUT_ROOT:-${ROOT_DIR}/experiments/outputs/core_world_g1_closefront_heldout_geometry_window_freeze/${SUITE_STAMP_PREFIX}}"

mkdir -p "${OUTPUT_ROOT}"
status_file="${OUTPUT_ROOT}/closefront_heldout_geometry_window_freeze_status.tsv"
summary_out="${OUTPUT_ROOT}/closefront_heldout_geometry_window_freeze_summary.json"
...
run_case() {
  ...
  echo "[G1-HELDOUT-GEOMETRY-WINDOW-FREEZE] case=${case_name} suite_stamp=${suite_stamp}"
...
run_case wide_y012_hold_guard_freeze \
...
run_case tall_z009_window_freeze \
