...
if [[ "$(hostname)" == mgmtserver* ]]; then
  echo "Refusing to run G1 close-front window-arrest suite on login/management node: $(hostname)" >&2
  exit 2
fi

ROOT_DIR="${ROOT_DIR:-/public/home/yanhongru/Curiosity}"
SUITE_STAMP_PREFIX="${SUITE_STAMP_PREFIX:-20260707_g1_lowcarry_close_front_window_arrest}"
OUTPUT_ROOT="${OUTPUT_ROOT:-${ROOT_DIR}/experiments/outputs/core_world_g1_lowcarry_close_front_window_arrest/${SUITE_STAMP_PREFIX}}"

cd "${ROOT_DIR}"
mkdir -p "${OUTPUT_ROOT}"

status_file="${OUTPUT_ROOT}/close_front_window_arrest_status.tsv"
summary_out="${OUTPUT_ROOT}/close_front_window_arrest_summary.json"
...
run_case() {
  ...
  echo "[G1-CLOSE-FRONT-WINDOW-ARREST] case=${case_name} suite_stamp=${suite_stamp}"
  ...
  env \
    ...
    AGILE_COMMAND_STOP_TARGET_WINDOW_MIN_STEP=620 \
    ...
    AGILE_COMMAND_HOLD_TERMINAL_BOX_TARGET_TRAVEL=-1.0 \
    ...
    CRADLE_CHEST_PAD_TARGET_WINDOW_MIN_STEP=620 \
...
run_case stop_window_pad620

run_case stop_window_pad650_soft_hold \
  AGILE_COMMAND_STOP_TARGET_WINDOW_MIN_STEP=650 \
  CRADLE_CHEST_PAD_TARGET_WINDOW_MIN_STEP=650 \
  AGILE_COMMAND_HOLD_SCALE=0.05

run_case stop_window_pad600 \
  AGILE_COMMAND_STOP_TARGET_WINDOW_MIN_STEP=600 \
  CRADLE_CHEST_PAD_TARGET_WINDOW_MIN_STEP=600
