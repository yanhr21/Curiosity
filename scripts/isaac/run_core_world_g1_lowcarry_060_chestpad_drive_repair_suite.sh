...
if [[ "$(hostname)" == mgmtserver* ]]; then
  echo "Refusing to run G1 low-carry 0.60 kg chest-pad drive repair suite on login/management node: $(hostname)" >&2
  exit 2
fi

ROOT_DIR="${ROOT_DIR:-/public/home/yanhongru/Curiosity}"
SUITE_STAMP_PREFIX="${SUITE_STAMP_PREFIX:-20260707_g1_lowcarry_060_chestpad_drive_repair}"
OUTPUT_ROOT="${OUTPUT_ROOT:-${ROOT_DIR}/experiments/outputs/core_world_g1_lowcarry_060_chestpad_drive_repair/${SUITE_STAMP_PREFIX}}"

mkdir -p "${OUTPUT_ROOT}"
status_file="${OUTPUT_ROOT}/lowcarry_060_chestpad_drive_repair_status.tsv"
summary_out="${OUTPUT_ROOT}/lowcarry_060_chestpad_drive_repair_summary.json"
...
run_repair_case() {
  ...
  echo "[G1-060-CHEST-DRIVE] case=${case_name} command_x=${command_x} terminal=${terminal_travel} final=${final_travel} suite_stamp=${suite_stamp}"
...
run_repair_case cmd012_final090 0.12 0.95 0.90 || overall_status=1
run_repair_case cmd014_final090 0.14 0.95 0.90 || overall_status=1
run_repair_case cmd012_final110 0.12 1.15 1.10 || overall_status=1
run_repair_case cmd014_final110 0.14 1.15 1.10 || overall_status=1
