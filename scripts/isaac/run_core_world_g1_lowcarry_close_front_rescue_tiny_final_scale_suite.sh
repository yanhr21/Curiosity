...
if [[ "$(hostname)" == mgmtserver* ]]; then
  echo "Refusing to run G1 close-front rescue tiny-final-scale suite on login/management node: $(hostname)" >&2
  exit 2
fi

ROOT_DIR="${ROOT_DIR:-/public/home/yanhongru/Curiosity}"
SUITE_STAMP_PREFIX="${SUITE_STAMP_PREFIX:-20260707_g1_lowcarry_close_front_rescue_tiny_final_scale}"
OUTPUT_ROOT="${OUTPUT_ROOT:-${ROOT_DIR}/experiments/outputs/core_world_g1_lowcarry_close_front_rescue_tiny_final_scale/${SUITE_STAMP_PREFIX}}"

cd "${ROOT_DIR}"
mkdir -p "${OUTPUT_ROOT}"

status_file="${OUTPUT_ROOT}/close_front_rescue_tiny_final_scale_status.tsv"
summary_out="${OUTPUT_ROOT}/close_front_rescue_tiny_final_scale_summary.json"
...
run_case() {
  ...
  echo "[G1-CLOSE-FRONT-RESCUE-TINY-FINAL-SCALE] case=${case_name} suite_stamp=${suite_stamp}"
  ...
  env \
    ...
    MAX_FINAL_HOLD_COMMAND_X=0.011 \
    ...
    AGILE_COMMAND_HOLD_RESCUE_KNEE=0.62 \
    AGILE_COMMAND_HOLD_RESCUE_ANKLE_PITCH=-0.34 \
    AGILE_COMMAND_HOLD_RESCUE_WAIST_PITCH=-0.08 \
...
run_case final_scale_003 \
  AGILE_COMMAND_HOLD_FINAL_SCALE=0.003

run_case final_scale_006 \
  AGILE_COMMAND_HOLD_FINAL_SCALE=0.006

run_case final_scale_010 \
  AGILE_COMMAND_HOLD_FINAL_SCALE=0.010
