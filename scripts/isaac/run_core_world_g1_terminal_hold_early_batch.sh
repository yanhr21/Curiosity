...
if [[ "$(hostname)" == mgmtserver* ]]; then
  echo "Refusing to run early terminal-hold validation on login/management node: $(hostname)" >&2
  exit 2
fi
...
run_diag() {
  ...
  local hip_offset="$4"
  local knee_offset="$5"
  local ankle_offset="$6"
  local waist_offset="$7"
  local stamp="20260705_core_world_g1_terminal_hold_early_${diag}_${trigger_mode}${trigger_value}"
  ...
  echo "[BATCH] ${diag} trigger=${trigger_mode}:${trigger_value} offsets=${hip_offset},${knee_offset},${ankle_offset},${waist_offset}"
...
run_diag diag85 pitch 0.20 0.12 -0.10 -0.12 0.10
run_diag diag86 pitch 0.20 0.20 -0.16 -0.18 0.16
run_diag diag87 step 320 0.12 -0.10 -0.12 0.10
run_diag diag88 travel 0.18 0.12 -0.10 -0.12 0.10
