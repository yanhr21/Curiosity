...
if [[ "$(hostname)" == mgmtserver* ]]; then
  echo "Refusing to run G1 close-front hold-delay suite on login/management node: $(hostname)" >&2
  exit 2
fi

ROOT_DIR="${ROOT_DIR:-/public/home/yanhongru/Curiosity}"
SUITE_STAMP_PREFIX="${SUITE_STAMP_PREFIX:-20260707_g1_lowcarry_close_front_hold_delay}"
OUTPUT_ROOT="${OUTPUT_ROOT:-${ROOT_DIR}/experiments/outputs/core_world_g1_lowcarry_close_front_hold_delay/${SUITE_STAMP_PREFIX}}"

mkdir -p "${OUTPUT_ROOT}"
status_file="${OUTPUT_ROOT}/close_front_hold_delay_status.tsv"
summary_out="${OUTPUT_ROOT}/close_front_hold_delay_summary.json"
...
run_case() {
  ...
  echo "[G1-CLOSE-HOLD-DELAY] case=${case_name} steps=${steps} terminal=${terminal_travel} final=${final_travel} suite_stamp=${suite_stamp}"
...
run_case steps950_final080 950 0.85 0.80 || overall_status=1
run_case steps1000_final100 1000 1.05 1.00 || overall_status=1
run_case steps1050_final120 1050 1.25 1.20 || overall_status=1
