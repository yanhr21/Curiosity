...
if [[ "$(hostname)" == mgmtserver* ]]; then
  echo "Refusing to run G1 boxtilt heavy window-freeze suite on login/management node: $(hostname)" >&2
  exit 2
fi

ROOT_DIR="${ROOT_DIR:-/public/home/yanhongru/Curiosity}"
SUITE_STAMP_PREFIX="${SUITE_STAMP_PREFIX:-20260707_g1_boxtilt_heavy_window_freeze}"
OUTPUT_ROOT="${OUTPUT_ROOT:-${ROOT_DIR}/experiments/outputs/core_world_g1_boxtilt_heavy_window_freeze/${SUITE_STAMP_PREFIX}}"

cd "${ROOT_DIR}"
mkdir -p "${OUTPUT_ROOT}"

status_file="${OUTPUT_ROOT}/boxtilt_heavy_window_freeze_status.tsv"
summary_out="${OUTPUT_ROOT}/boxtilt_heavy_window_freeze_summary.json"
...
run_case() {
  ...
  echo "[G1-BOXTILT-FREEZE] case=${case_name} suite_stamp=${suite_stamp}"
  ...
  env \
    ...
    AGILE_COMMAND_HOLD_FINAL_FREEZE_MAX_TILT=0.55 \
    AGILE_COMMAND_HOLD_FINAL_FREEZE_MAX_BOX_TILT=0.75 \
    "$@" \
...
run_case freeze_165_185_s010 \
  AGILE_COMMAND_HOLD_TERMINAL_BOX_TARGET_TRAVEL=1.65 \
  AGILE_COMMAND_HOLD_TERMINAL_SCALE=0.010 \
  AGILE_COMMAND_HOLD_FINAL_BOX_TARGET_TRAVEL=1.85 \
  AGILE_COMMAND_HOLD_FINAL_SCALE=0.0

run_case freeze_170_190_s008 \
  AGILE_COMMAND_HOLD_TERMINAL_BOX_TARGET_TRAVEL=1.70 \
  AGILE_COMMAND_HOLD_TERMINAL_SCALE=0.008 \
  AGILE_COMMAND_HOLD_FINAL_BOX_TARGET_TRAVEL=1.90 \
  AGILE_COMMAND_HOLD_FINAL_SCALE=0.0

run_case freeze_165_180_brake \
  AGILE_COMMAND_HOLD_TERMINAL_BOX_TARGET_TRAVEL=1.65 \
  AGILE_COMMAND_HOLD_TERMINAL_SCALE=0.012 \
  AGILE_COMMAND_HOLD_FINAL_BOX_TARGET_TRAVEL=1.80 \
  AGILE_COMMAND_HOLD_FINAL_SCALE=0.0 \
  AGILE_COMMAND_HOLD_FINAL_BRAKE_COMMAND_X=-0.015 \
  AGILE_COMMAND_HOLD_FINAL_BRAKE_DELAY_STEPS=8 \
  AGILE_COMMAND_HOLD_FINAL_BRAKE_STEPS=55
