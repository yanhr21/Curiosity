...
STAMP="${STAMP:-20260705_core_world_g1_stand_height_sweep}"
ROOT_OUTPUT="experiments/outputs/core_world_g1_stand_height_sweep/${STAMP}"
LOG_DIR="logs/core_world_g1_stand_height_sweep"
mkdir -p "${ROOT_OUTPUT}" "${LOG_DIR}"

HEIGHTS=(${HEIGHTS:-0.78 0.84 0.90 0.96})
ATTACH_BOX_MODE="${ATTACH_BOX_MODE:-fixed_torso}"
EXPECT_ATTACH_BOX="${EXPECT_ATTACH_BOX:-${ATTACH_BOX_MODE}}"
MAX_BOX_DROP_EVENTS="${MAX_BOX_DROP_EVENTS:-0}"
MIN_BOX_Z="${MIN_BOX_Z:-0.35}"

for height in "${HEIGHTS[@]}"; do
  case_name="z_${height/./p}"
  output_dir="${ROOT_OUTPUT}/${case_name}"
  mkdir -p "${output_dir}"
  echo "[CASE] height=${height} output=${output_dir}"
  set +e
  ...
  ATTACH_LOCAL_POS0_X="${ATTACH_LOCAL_POS0_X:-0.20}" \
  ...
  if [[ "${run_status}" -ne 0 ]]; then
    echo "[WARN] G1 stand case ${case_name} runner exited ${run_status}; preserving result and continuing." >&2
  fi
  ...
  if [[ "${check_status}" -ne 0 ]]; then
    echo "[WARN] G1 stand case ${case_name} strict checker failed; continuing." >&2
  fi
done

python3 scripts/isaac/summarize_core_world_g1_stand_height_sweep.py \
  --root "${ROOT_OUTPUT}" \
  --output "${ROOT_OUTPUT}/core_world_g1_stand_height_sweep_summary.json"

echo "[INFO] G1 stand height sweep output: ${ROOT_OUTPUT}"
