...
if [[ "$(hostname)" == mgmtserver* ]]; then
  echo "Refusing to run G1 close-front final-reset probe on login/management node: $(hostname)" >&2
  exit 2
fi

ROOT_DIR="${ROOT_DIR:-/public/home/yanhongru/Curiosity}"
SUITE_STAMP_PREFIX="${SUITE_STAMP_PREFIX:-20260707_g1_lowcarry_close_front_final_reset_probe}"
OUTPUT_ROOT="${OUTPUT_ROOT:-${ROOT_DIR}/experiments/outputs/core_world_g1_lowcarry_close_front_final_reset_probe/${SUITE_STAMP_PREFIX}}"

cd "${ROOT_DIR}"
mkdir -p "${OUTPUT_ROOT}"

status_file="${OUTPUT_ROOT}/close_front_final_reset_probe_status.tsv"
summary_out="${OUTPUT_ROOT}/close_front_final_reset_probe_summary.json"
...
run_case() {
  ...
  echo "[G1-CLOSE-FRONT-FINAL-RESET] case=${case_name} suite_stamp=${suite_stamp}"
  ...
  env \
    ...
    AGILE_COMMAND_HOLD_FINAL_RESET_POLICY_STATE=1 \
...
run_case steps1050_final120_reset
