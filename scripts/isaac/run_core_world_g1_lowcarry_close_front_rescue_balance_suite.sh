...
if [[ "$(hostname)" == mgmtserver* ]]; then
  echo "Refusing to run G1 close-front rescue/balance suite on login/management node: $(hostname)" >&2
  exit 2
fi

ROOT_DIR="${ROOT_DIR:-/public/home/yanhongru/Curiosity}"
SUITE_STAMP_PREFIX="${SUITE_STAMP_PREFIX:-20260707_g1_lowcarry_close_front_rescue_balance}"
OUTPUT_ROOT="${OUTPUT_ROOT:-${ROOT_DIR}/experiments/outputs/core_world_g1_lowcarry_close_front_rescue_balance/${SUITE_STAMP_PREFIX}}"

cd "${ROOT_DIR}"
mkdir -p "${OUTPUT_ROOT}"

status_file="${OUTPUT_ROOT}/close_front_rescue_balance_status.tsv"
summary_out="${OUTPUT_ROOT}/close_front_rescue_balance_summary.json"
...
run_case() {
  ...
  echo "[G1-CLOSE-FRONT-RESCUE-BALANCE] case=${case_name} suite_stamp=${suite_stamp}"
...
run_case rescue_crouch_abs055 \
  AGILE_COMMAND_HOLD_RESCUE_ENABLE=1 \
  AGILE_COMMAND_HOLD_RESCUE_ABS_ROLL_THRESHOLD=0.55 \
  AGILE_COMMAND_HOLD_RESCUE_BLEND_RATE=0.12 \
  AGILE_COMMAND_HOLD_RESCUE_HIP_PITCH=-0.26 \
  AGILE_COMMAND_HOLD_RESCUE_KNEE=0.70 \
  AGILE_COMMAND_HOLD_RESCUE_ANKLE_PITCH=-0.40 \
  AGILE_COMMAND_HOLD_RESCUE_WAIST_PITCH=-0.10

run_case balance_roll_avg_pos \
  BALANCE_ROLL_TARGET_FROM_LATERAL=1 \
  BALANCE_ROLL_TARGET_LATERAL_SOURCE=average \
  BALANCE_ROLL_TARGET_LATERAL_GAIN=0.16 \
  BALANCE_ROLL_TARGET_LATERAL_LIMIT=0.08 \
  BALANCE_ROLL_TARGET_LATERAL_DEADBAND=0.10 \
  BALANCE_ROLL_TARGET_LATERAL_SIGN=1.0 \
  BALANCE_ROLL_TARGET_LATERAL_START_AFTER_HOLD_STEPS=420 \
  BALANCE_ROLL_TARGET_LATERAL_RAMP_STEPS=120 \
  BALANCE_ROLL_TARGET_LATERAL_MAX_TILT=0.75 \
  BALANCE_ROLL_TARGET_LATERAL_MAX_BOX_TILT=0.85

run_case balance_roll_avg_neg \
  BALANCE_ROLL_TARGET_FROM_LATERAL=1 \
  BALANCE_ROLL_TARGET_LATERAL_SOURCE=average \
  BALANCE_ROLL_TARGET_LATERAL_GAIN=0.16 \
  BALANCE_ROLL_TARGET_LATERAL_LIMIT=0.08 \
