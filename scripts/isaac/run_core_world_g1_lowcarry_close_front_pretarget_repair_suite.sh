...
if [[ "$(hostname)" == mgmtserver* ]]; then
  echo "Refusing to run G1 close-front pretarget repair suite on login/management node: $(hostname)" >&2
  exit 2
fi

ROOT_DIR="${ROOT_DIR:-/public/home/yanhongru/Curiosity}"
SUITE_STAMP_PREFIX="${SUITE_STAMP_PREFIX:-20260707_g1_lowcarry_close_front_pretarget_repair}"
OUTPUT_ROOT="${OUTPUT_ROOT:-${ROOT_DIR}/experiments/outputs/core_world_g1_lowcarry_close_front_pretarget_repair/${SUITE_STAMP_PREFIX}}"

cd "${ROOT_DIR}"
mkdir -p "${OUTPUT_ROOT}"

status_file="${OUTPUT_ROOT}/close_front_pretarget_repair_status.tsv"
summary_out="${OUTPUT_ROOT}/close_front_pretarget_repair_summary.json"
...
run_case() {
  ...
  echo "[G1-CLOSE-FRONT-PRETARGET] case=${case_name} suite_stamp=${suite_stamp}"
...
run_case progress_conservative \
...
run_case progress_mid_no_hold_lat \
  ...
  AGILE_COMMAND_BOX_LATERAL_LIMIT=0.0020 \
  AGILE_COMMAND_BOX_LATERAL_SIGN=1.0 \
  AGILE_COMMAND_BOX_LATERAL_SCALE_ON_HOLD=1
