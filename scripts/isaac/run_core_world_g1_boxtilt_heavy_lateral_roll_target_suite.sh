...
if [[ "$(hostname)" == mgmtserver* ]]; then
  echo "Refusing to run G1 boxtilt heavy lateral-roll-target suite on login/management node: $(hostname)" >&2
  exit 2
fi

ROOT_DIR="${ROOT_DIR:-/public/home/yanhongru/Curiosity}"
SUITE_STAMP_PREFIX="${SUITE_STAMP_PREFIX:-20260707_g1_boxtilt_heavy_lateral_roll_target}"
OUTPUT_ROOT="${OUTPUT_ROOT:-${ROOT_DIR}/experiments/outputs/core_world_g1_boxtilt_heavy_lateral_roll_target/${SUITE_STAMP_PREFIX}}"

cd "${ROOT_DIR}"
mkdir -p "${OUTPUT_ROOT}"

status_file="${OUTPUT_ROOT}/boxtilt_heavy_lateral_roll_target_status.tsv"
summary_out="${OUTPUT_ROOT}/boxtilt_heavy_lateral_roll_target_summary.json"
...
run_case() {
  ...
  echo "[G1-BOXTILT-ROLLTARGET] case=${case_name} suite_stamp=${suite_stamp}"
  ...
  env \
    ...
    BALANCE_ROLL_TARGET_LATERAL_GAIN=0.05 \
    BALANCE_ROLL_TARGET_LATERAL_LIMIT=0.08 \
