...
if [[ "$(hostname)" == mgmtserver* ]]; then
  echo "Refusing to run G1 low-carry load repair suite on login/management node: $(hostname)" >&2
  exit 2
fi

ROOT_DIR="${ROOT_DIR:-/public/home/yanhongru/Curiosity}"
SUITE_STAMP_PREFIX="${SUITE_STAMP_PREFIX:-20260707_g1_lowcarry_load_repair}"
OUTPUT_ROOT="${OUTPUT_ROOT:-${ROOT_DIR}/experiments/outputs/core_world_g1_lowcarry_load_repair/${SUITE_STAMP_PREFIX}}"

mkdir -p "${OUTPUT_ROOT}"
status_file="${OUTPUT_ROOT}/lowcarry_load_repair_status.tsv"
summary_out="${OUTPUT_ROOT}/lowcarry_load_repair_summary.json"
...
run_case() {
  ...
  echo "[G1-LOWCARRY-REPAIR] case=${case_name} suite_stamp=${suite_stamp}"
...
run_case light025_final_freeze \
  ...
  "${common_target_window[@]}" || overall_status=1

run_case light025_policy_then_stand \
  LARGERBOX_STRICT_MODE=lowcarry \
  FREE_BOX_MASS=0.25 \
  FREE_STEPS=819 \
  AGILE_COMMAND_HOLD_FINAL_ZERO_CORRECTIONS=1 \
  AGILE_COMMAND_HOLD_MODE=policy_then_stand \
  AGILE_COMMAND_HOLD_POLICY_THEN_STAND_DELAY_STEPS=80 \
  AGILE_COMMAND_HOLD_STAND_BLEND_RATE=0.03 \
  MIN_AGILE_COMMAND_HOLD_FINAL_STAND_ACTIVE_STEPS=250 \
...
run_case heavy075_chestpad_retention_slow \
  LARGERBOX_STRICT_MODE=chestpad \
  FREE_BOX_MASS=0.75 \
  FREE_STEPS=1200 \
  BOX_RETENTION_POSTURE_CONTROLLER=1 \
  AGILE_COMMAND_HOLD_ADAPTIVE_MIN_SCALE=0.08 \
  AGILE_COMMAND_HOLD_ADAPTIVE_MAX_SCALE=0.25 \
  AGILE_COMMAND_HOLD_ADAPTIVE_SCALE_SMOOTHING=0.12 \
  AGILE_COMMAND_HOLD_TERMINAL_BOX_TARGET_TRAVEL=0.80 \
  AGILE_COMMAND_HOLD_TERMINAL_SCALE=0.012 \
  AGILE_COMMAND_HOLD_TERMINAL_LATCH=1 \
  AGILE_COMMAND_HOLD_FINAL_BOX_TARGET_TRAVEL=0.72 \
  AGILE_COMMAND_HOLD_FINAL_SCALE=0.0 \
  AGILE_COMMAND_HOLD_FINAL_LATCH=1 \
  FREE_MAX_FINAL_ROBOT_TARGET_DIRECTED_TRAVEL=2.35 \
  FREE_MAX_FINAL_BOX_TARGET_DIRECTED_TRAVEL=2.35 \
  ...
  MAX_FINAL_HOLD_BOX_TILT=0.45 \
  MAX_FINAL_HOLD_FALL_EVENTS=0 \
  MAX_FINAL_HOLD_BOX_DROP_EVENTS=0 || overall_status=1
