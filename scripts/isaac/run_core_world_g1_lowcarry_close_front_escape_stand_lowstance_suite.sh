...
if [[ "$(hostname)" == mgmtserver* ]]; then
  echo "Refusing to run G1 close-front escape-stand low-stance suite on login/management node: $(hostname)" >&2
  exit 2
fi

ROOT_DIR="${ROOT_DIR:-/public/home/yanhongru/Curiosity}"
SUITE_STAMP_PREFIX="${SUITE_STAMP_PREFIX:-20260707_g1_lowcarry_close_front_escape_stand_lowstance}"
OUTPUT_ROOT="${OUTPUT_ROOT:-${ROOT_DIR}/experiments/outputs/core_world_g1_lowcarry_close_front_escape_stand_lowstance/${SUITE_STAMP_PREFIX}}"

cd "${ROOT_DIR}"
mkdir -p "${OUTPUT_ROOT}"

status_file="${OUTPUT_ROOT}/close_front_escape_stand_lowstance_status.tsv"
summary_out="${OUTPUT_ROOT}/close_front_escape_stand_lowstance_summary.json"
...
run_case() {
  ...
  echo "[G1-CLOSE-FRONT-ESCAPE-STAND-LOWSTANCE] case=${case_name} hip=${hip_pitch} knee=${knee} ankle=${ankle_pitch} waist=${waist_pitch} suite_stamp=${suite_stamp}"
  ...
  env \
    ...
    AGILE_COMMAND_HOLD_STAND_HIP_PITCH="${hip_pitch}" \
    AGILE_COMMAND_HOLD_STAND_KNEE="${knee}" \
    AGILE_COMMAND_HOLD_STAND_ANKLE_PITCH="${ankle_pitch}" \
    AGILE_COMMAND_HOLD_STAND_WAIST_PITCH="${waist_pitch}" \
...
run_case lowstance_soft -0.12 0.30 -0.16 -0.05
run_case lowstance_deeper -0.18 0.42 -0.22 -0.08
