...
if [[ "$(hostname)" == mgmtserver* ]]; then
  echo "Refusing to run G1 close-front freeze-balance refine suite on login/management node: $(hostname)" >&2
  exit 2
fi

ROOT_DIR="${ROOT_DIR:-/public/home/yanhongru/Curiosity}"
SUITE_STAMP_PREFIX="${SUITE_STAMP_PREFIX:-20260707_g1_lowcarry_close_front_freeze_balance_refine}"
OUTPUT_ROOT="${OUTPUT_ROOT:-${ROOT_DIR}/experiments/outputs/core_world_g1_lowcarry_close_front_freeze_balance_refine/${SUITE_STAMP_PREFIX}}"

cd "${ROOT_DIR}"
mkdir -p "${OUTPUT_ROOT}"

status_file="${OUTPUT_ROOT}/close_front_freeze_balance_refine_status.tsv"
summary_out="${OUTPUT_ROOT}/close_front_freeze_balance_refine_summary.json"
...
run_case() {
  ...
  echo "[G1-CLOSE-FRONT-FREEZE-BALANCE] case=${case_name} suite_stamp=${suite_stamp}"
  ...
  env \
    ...
    BALANCE_ROLL_ACTIVATION_THRESHOLD=0.02 \
    BALANCE_ROLL_RATE_ACTIVATION_THRESHOLD=0.08 \
    BALANCE_PITCH_ACTIVATION_THRESHOLD=0.03 \
    BALANCE_PITCH_RATE_ACTIVATION_THRESHOLD=0.12 \
...
run_case roll_gain_010 \
  BALANCE_ROLL_GAIN=0.10 \
  BALANCE_ROLL_RATE_GAIN=0.006 \
  BALANCE_ADJUSTMENT_LIMIT=0.11

run_case roll_gain_014 \
  BALANCE_ROLL_GAIN=0.14 \
  BALANCE_ROLL_RATE_GAIN=0.008 \
  BALANCE_ADJUSTMENT_LIMIT=0.14

run_case roll_pitch_gain \
  BALANCE_ROLL_GAIN=0.10 \
  BALANCE_ROLL_RATE_GAIN=0.006 \
  BALANCE_PITCH_GAIN=0.14 \
  BALANCE_PITCH_RATE_GAIN=0.010 \
  BALANCE_ADJUSTMENT_LIMIT=0.12
