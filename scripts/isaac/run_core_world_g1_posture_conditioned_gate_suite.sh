...
if [[ "$(hostname)" == mgmtserver* ]]; then
  echo "Refusing to run G1 posture-conditioned gate suite on login/management node: $(hostname)" >&2
  exit 2
fi

ROOT_DIR="${ROOT_DIR:-/public/home/yanhongru/Curiosity}"
SUITE_STAMP_PREFIX="${SUITE_STAMP_PREFIX:-20260707_g1_posture_conditioned_gate}"
OUTPUT_ROOT="${OUTPUT_ROOT:-${ROOT_DIR}/experiments/outputs/core_world_g1_posture_conditioned_gate/${SUITE_STAMP_PREFIX}}"

mkdir -p "${OUTPUT_ROOT}"
status_file="${OUTPUT_ROOT}/posture_conditioned_gate_status.tsv"
summary_out="${OUTPUT_ROOT}/posture_conditioned_gate_summary.json"
...
run_common_case() {
  ...
  echo "[G1-POSTURE-GATE] case=${case_name} suite_stamp=${suite_stamp}"
...
run_common_case low_front_060 \
  ...
  AGILE_COMMAND_HOLD_FINAL_BOX_TARGET_TRAVEL=0.6 || overall_status=1

run_common_case close_front_060_conditioned \
  FREE_STEPS=1200 \
  FREE_BOX_POS_X=-0.14 \
