...
if [[ "$(hostname)" == mgmtserver* ]]; then
  echo "Refusing to run G1 boxtilt scaled-terminal suite on login/management node: $(hostname)" >&2
  exit 2
fi

ROOT_DIR="${ROOT_DIR:-/public/home/yanhongru/Curiosity}"
SUITE_STAMP_PREFIX="${SUITE_STAMP_PREFIX:-20260707_g1_boxtilt_scaled_terminal}"
OUTPUT_ROOT="${OUTPUT_ROOT:-${ROOT_DIR}/experiments/outputs/core_world_g1_boxtilt_scaled_terminal/${SUITE_STAMP_PREFIX}}"

cd "${ROOT_DIR}"
mkdir -p "${OUTPUT_ROOT}"

status_file="${OUTPUT_ROOT}/boxtilt_scaled_terminal_status.tsv"
summary_out="${OUTPUT_ROOT}/boxtilt_scaled_terminal_summary.json"
...
case_enabled() {
  local case_name="$1"
  local requested="${SCALED_TERMINAL_CASES:-all}"
...
run_case() {
  ...
  if ! case_enabled "${case_name}"; then
    echo "[G1-BOXTILT-SCALED-TERMINAL] skip case=${case_name} SCALED_TERMINAL_CASES=${SCALED_TERMINAL_CASES:-all}"
    return 0
  fi
  ...
  echo "[G1-BOXTILT-SCALED-TERMINAL] case=${case_name} suite_stamp=${suite_stamp}"
...
run_case early_terminal_finalzero \
  AGILE_COMMAND_STOP_BOX_TARGET_TRAVEL=1.45 \
  AGILE_COMMAND_HOLD_TERMINAL_BOX_TARGET_TRAVEL=1.45 \
  AGILE_COMMAND_HOLD_TERMINAL_SCALE=0.22 \
  AGILE_COMMAND_HOLD_FINAL_BOX_TARGET_TRAVEL=1.82 \
...
run_case later_terminal_finalzero \
  AGILE_COMMAND_STOP_BOX_TARGET_TRAVEL=1.60 \
  AGILE_COMMAND_HOLD_TERMINAL_BOX_TARGET_TRAVEL=1.60 \
  AGILE_COMMAND_HOLD_TERMINAL_SCALE=0.28 \
  AGILE_COMMAND_HOLD_FINAL_BOX_TARGET_TRAVEL=1.92 \
  AGILE_COMMAND_HOLD_FINAL_SCALE=0.0 \
  AGILE_COMMAND_HOLD_FINAL_ZERO_CORRECTIONS=1 \
  AGILE_COMMAND_HOLD_FINAL_FREEZE_IN_TARGET_WINDOW=1 \
  AGILE_COMMAND_HOLD_FINAL_FREEZE_MAX_TILT=0.50 \
...
run_case later_terminal_brake \
  AGILE_COMMAND_STOP_BOX_TARGET_TRAVEL=1.60 \
  AGILE_COMMAND_HOLD_TERMINAL_BOX_TARGET_TRAVEL=1.60 \
  AGILE_COMMAND_HOLD_TERMINAL_SCALE=0.30 \
  AGILE_COMMAND_HOLD_FINAL_BOX_TARGET_TRAVEL=1.90 \
  AGILE_COMMAND_HOLD_FINAL_SCALE=0.0 \
  AGILE_COMMAND_HOLD_FINAL_ZERO_CORRECTIONS=1 \
  AGILE_COMMAND_HOLD_FINAL_BRAKE_COMMAND_X=-0.018 \
  AGILE_COMMAND_HOLD_FINAL_BRAKE_STEPS=100
