...
if [[ "$(hostname)" == mgmtserver* ]]; then
  echo "Refusing to run online probe-adaptive hold posture suite on login/management node: $(hostname)" >&2
  exit 2
fi

ROOT_DIR="${ROOT_DIR:-/public/home/yanhongru/Curiosity}"
SUITE_STAMP="${SUITE_STAMP:-20260706_direct_carry_online_probe_adaptive_hold_postures_64cm_8kg}"
...
SUPPORT_AUDIT_ARGS=()
if [[ "${REQUIRE_PLANTED_STANCE_RAIL_PROPULSION:-0}" == "1" ]]; then
  SUPPORT_AUDIT_ARGS+=(--require-planted-stance-rail-propulsion)
fi
if [[ "${REQUIRE_FREEZE_COMMANDED_STANCE_FOOT_TARGETS:-0}" == "1" ]]; then
  SUPPORT_AUDIT_ARGS+=(--require-freeze-commanded-stance-foot-targets)
fi
...
case_enabled() {
  ...
  local item
  IFS=',' read -ra filter_items <<< "${CASE_FILTER}"
  for item in "${filter_items[@]}"; do
    item="${item//[[:space:]]/}"
    if [[ "${item}" == "${case_id}" || "${item}" == "${posture}" ]]; then
      return 0
    fi
  done
  return 1
...
run_case() {
  local condition="$1"
  local carry_posture="$2"
  local probe_mode="$3"
  local probe_x="$4"
  local probe_z="$5"
  local expected_bucket="$6"
  local expected_support_profile="$7"
  local expected_hold_profile="$8"
  local expected_closure="$9"
  local expected_collision_enabled="${10}"
  local case_id="${condition}_${carry_posture}"
  local expected_step_height="0.120"
  local expected_double_support="0.12"
  local stamp="${SUITE_STAMP}_${condition}_${carry_posture}"
  ...
  if ! case_enabled "${case_id}" "${carry_posture}"; then
    echo "[ONLINE_HOLD_POSTURE_SUITE] skip case=${case_id} CASE_FILTER=${CASE_FILTER}"
    return 0
  fi
  RUN_CASE_COUNT=$((RUN_CASE_COUNT + 1))

  if [[ "${expected_bucket}" == "medium" ]]; then
    expected_step_height="0.100"
    expected_double_support="0.18"
  elif [[ "${expected_bucket}" == "high" ]]; then
    expected_step_height="0.080"
    expected_double_support="0.24"
  fi

  echo "[ONLINE_HOLD_POSTURE_SUITE] condition=${condition} posture=${carry_posture} probe_mode=${probe_mode}"
  STAMP="${stamp}" \
  ...
  python3 scripts/isaac/check_direct_carry_task_summary.py \
    ...
    --expect-online-probe-adaptive-support-profile "${expected_support_profile}" \
    ...
    --expect-online-probe-adaptive-hold-profile "${expected_hold_profile}" \
...
run_case vertical_probe front_mid vertical_micro_lift 0.0 0.030 medium compact_medium_double_support reinforced_contact_closure 1.0 1
run_case vertical_probe close_mid vertical_micro_lift 0.0 0.030 medium compact_medium_double_support reinforced_contact_closure 1.0 1
run_case vertical_probe chest_high vertical_micro_lift 0.0 0.030 medium compact_medium_double_support reinforced_contact_closure 1.0 1
run_case horizontal_probe front_reach horizontal_push_pull 0.050 0.0 low nominal_reach_support light_contact_closure 0.75 0
run_case horizontal_probe low_front horizontal_push_pull 0.050 0.0 low nominal_reach_support light_contact_closure 0.75 0

if [[ "${RUN_CASE_COUNT}" -le 0 ]]; then
  echo "No cases selected; CASE_FILTER=${CASE_FILTER}" >&2
  exit 6
fi

python3 scripts/isaac/summarize_direct_carry_posture_suite.py \
  "${SUMMARY_ARGS[@]}" \
  --output "${SUITE_DIR}/online_probe_adaptive_hold_posture_summary.json" \
  --min-postures "${MIN_POSTURES:-5}" \
  --min-steps 3620 \
...

{
  ...
  echo "summary=${SUITE_DIR}/online_probe_adaptive_hold_posture_summary.json"
