...
if [[ "$(hostname)" == mgmtserver* ]]; then
  echo "Refusing to run G1 close-front rescue final-latch suite on login/management node: $(hostname)" >&2
  exit 2
fi

ROOT_DIR="${ROOT_DIR:-/public/home/yanhongru/Curiosity}"
SUITE_STAMP_PREFIX="${SUITE_STAMP_PREFIX:-20260707_g1_lowcarry_close_front_rescue_final_latch}"
OUTPUT_ROOT="${OUTPUT_ROOT:-${ROOT_DIR}/experiments/outputs/core_world_g1_lowcarry_close_front_rescue_final_latch/${SUITE_STAMP_PREFIX}}"

cd "${ROOT_DIR}"
mkdir -p "${OUTPUT_ROOT}"

status_file="${OUTPUT_ROOT}/close_front_rescue_final_latch_status.tsv"
summary_out="${OUTPUT_ROOT}/close_front_rescue_final_latch_summary.json"
...
run_case() {
  ...
  echo "[G1-CLOSE-FRONT-RESCUE-FINAL-LATCH] case=${case_name} suite_stamp=${suite_stamp}"
  ...
  env \
    ...
    MIN_AGILE_COMMAND_HOLD_FINAL_ACTIVE_STEPS=350 \
...
run_case final135 \
  AGILE_COMMAND_HOLD_TERMINAL_BOX_TARGET_TRAVEL=1.30 \
  AGILE_COMMAND_HOLD_FINAL_BOX_TARGET_TRAVEL=1.35

run_case final145 \
  AGILE_COMMAND_HOLD_TERMINAL_BOX_TARGET_TRAVEL=1.38 \
  AGILE_COMMAND_HOLD_FINAL_BOX_TARGET_TRAVEL=1.45

run_case final155 \
  AGILE_COMMAND_HOLD_TERMINAL_BOX_TARGET_TRAVEL=1.48 \
  AGILE_COMMAND_HOLD_FINAL_BOX_TARGET_TRAVEL=1.55
