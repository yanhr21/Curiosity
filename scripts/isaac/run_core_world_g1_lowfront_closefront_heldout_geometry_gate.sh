...
if [[ "$(hostname)" == mgmtserver* ]]; then
  echo "Refusing to run G1 held-out geometry gate suite on login/management node: $(hostname)" >&2
  exit 2
fi

ROOT_DIR="${ROOT_DIR:-/public/home/yanhongru/Curiosity}"
SUITE_STAMP_PREFIX="${SUITE_STAMP_PREFIX:-20260707_g1_lowfront_closefront_heldout_geometry_gate}"
OUTPUT_ROOT="${OUTPUT_ROOT:-${ROOT_DIR}/experiments/outputs/core_world_g1_lowfront_closefront_heldout_geometry_gate/${SUITE_STAMP_PREFIX}}"
CASE_SET="${HELDOUT_GEOMETRY_CASE_SET:-default}"

mkdir -p "${OUTPUT_ROOT}"
status_file="${OUTPUT_ROOT}/lowfront_closefront_heldout_geometry_gate_status.tsv"
summary_out="${OUTPUT_ROOT}/lowfront_closefront_heldout_geometry_gate_summary.json"
...
run_case() {
  ...
  echo "[G1-HELDOUT-GEOMETRY-GATE] case=${case_name} suite_stamp=${suite_stamp}"
...
case "${CASE_SET}" in
  quick)
    run_close_front_lowmass close_front_060_lowmass_lx19 || overall_status=1
    run_close_front_lowmass close_front_060_wide_y012 FREE_BOX_SIZE_Y=0.12 || overall_status=1
    ;;
  default)
    run_low_front_pass || overall_status=1
    run_close_front_lowmass close_front_060_lowmass_lx19 || overall_status=1
    run_close_front_lowmass close_front_060_wide_y012 FREE_BOX_SIZE_Y=0.12 || overall_status=1
    run_close_front_lowmass close_front_060_tall_z009 FREE_BOX_SIZE_Z=0.09 || overall_status=1
    ;;
  *)
    echo "Unknown HELDOUT_GEOMETRY_CASE_SET=${CASE_SET}; expected default or quick" >&2
    exit 2
    ;;
esac
