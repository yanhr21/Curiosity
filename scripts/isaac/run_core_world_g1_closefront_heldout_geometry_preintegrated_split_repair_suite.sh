...
if [[ "$(hostname)" == mgmtserver* ]]; then
  echo "Refusing to run G1 held-out geometry preintegrated split-repair suite on login/management node: $(hostname)" >&2
  exit 2
fi

ROOT_DIR="${ROOT_DIR:-/public/home/yanhongru/Curiosity}"
SUITE_STAMP_PREFIX="${SUITE_STAMP_PREFIX:-20260708_g1_closefront_heldout_geometry_preintegrated_split_repair}"
OUTPUT_ROOT="${OUTPUT_ROOT:-${ROOT_DIR}/experiments/outputs/core_world_g1_closefront_heldout_geometry_preintegrated_split_repair/${SUITE_STAMP_PREFIX}}"

mkdir -p "${OUTPUT_ROOT}"
status_file="${OUTPUT_ROOT}/closefront_heldout_geometry_preintegrated_split_repair_status.tsv"
summary_out="${OUTPUT_ROOT}/closefront_heldout_geometry_preintegrated_split_repair_summary.json"
...
run_case() {
  ...
  echo "[G1-HELDOUT-GEOMETRY-PREINTEGRATED-SPLIT-REPAIR] case=${case_name} suite_stamp=${suite_stamp}"
  ...
  env \
    ...
    AGILE_COMMAND_TERMINAL_SUPPORT_START_BOX_TARGET_TRAVEL=1.05 \
    AGILE_COMMAND_TERMINAL_SUPPORT_TARGET=2.0 \
    AGILE_COMMAND_TERMINAL_SUPPORT_DEADBAND=0.06 \
    AGILE_COMMAND_TERMINAL_SUPPORT_GAIN=0.045 \
    AGILE_COMMAND_TERMINAL_SUPPORT_MAX_FORWARD=0.030 \
    AGILE_COMMAND_TERMINAL_SUPPORT_MAX_REVERSE=0.014 \
    AGILE_COMMAND_TERMINAL_SUPPORT_FINAL_ZERO_COMMAND=0 \
    AGILE_COMMAND_TERMINAL_SUPPORT_LATERAL_SOURCE=average \
    AGILE_COMMAND_TERMINAL_SUPPORT_LATERAL_DEADBAND=0.04 \
    AGILE_COMMAND_TERMINAL_SUPPORT_LATERAL_GAIN=0.018 \
    AGILE_COMMAND_TERMINAL_SUPPORT_LATERAL_LIMIT=0.010 \
    AGILE_COMMAND_TERMINAL_SUPPORT_LATERAL_SIGN=1.0 \
    AGILE_COMMAND_TERMINAL_SUPPORT_YAW_GAIN=0.012 \
    AGILE_COMMAND_TERMINAL_SUPPORT_YAW_LIMIT=0.016 \
...
run_case wide_y012_relaxed_geometry \
  FREE_BOX_SIZE_Y=0.12 \
  AGILE_COMMAND_TERMINAL_SUPPORT_CONTROLLER=0 \
  CRADLE_CHEST_PAD_ENABLED=0 \
  CRADLE_FINAL_SIDE_GUARD_HALF_SPACING=0.18 \
  CRADLE_FINAL_SIDE_GUARD_MASS_SCALE=0.12 \
  CRADLE_FINAL_CROSS_BRACE=0 || overall_status=1

run_case tall_z009_drive_lateral_refine \
  FREE_BOX_SIZE_Z=0.09 \
  AGILE_COMMAND_X=0.13 \
  AGILE_COMMAND_Y=-0.04 \
  AGILE_COMMAND_TERMINAL_SUPPORT_LATERAL_SIGN=-1.0 \
  AGILE_COMMAND_TERMINAL_SUPPORT_MAX_FORWARD=0.040 || overall_status=1
