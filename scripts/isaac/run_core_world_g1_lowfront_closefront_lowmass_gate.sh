...
if [[ "$(hostname)" == mgmtserver* ]]; then
  echo "Refusing to run G1 two-posture gate suite on login/management node: $(hostname)" >&2
  exit 2
fi

ROOT_DIR="${ROOT_DIR:-/public/home/yanhongru/Curiosity}"
SUITE_STAMP_PREFIX="${SUITE_STAMP_PREFIX:-20260707_g1_lowfront_closefront_lowmass_gate}"
OUTPUT_ROOT="${OUTPUT_ROOT:-${ROOT_DIR}/experiments/outputs/core_world_g1_lowfront_closefront_lowmass_gate/${SUITE_STAMP_PREFIX}}"

mkdir -p "${OUTPUT_ROOT}"
status_file="${OUTPUT_ROOT}/lowfront_closefront_lowmass_gate_status.tsv"
summary_out="${OUTPUT_ROOT}/lowfront_closefront_lowmass_gate_summary.json"
...
run_case() {
  ...
  echo "[G1-LOWFRONT-CLOSEFRONT-GATE] case=${case_name} suite_stamp=${suite_stamp}"
...
run_case close_front_060_lowmass_lx19 \
  ...
  CRADLE_FINAL_SIDE_GUARD_MASS_SCALE=0.25 || overall_status=1
