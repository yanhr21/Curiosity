...
if [[ "$(hostname)" == mgmtserver* ]]; then
  echo "Refusing to run G1 low-carry 0.60 kg box-tilt repair suite on login/management node: $(hostname)" >&2
  exit 2
fi

ROOT_DIR="${ROOT_DIR:-/public/home/yanhongru/Curiosity}"
SUITE_STAMP_PREFIX="${SUITE_STAMP_PREFIX:-20260707_g1_lowcarry_060_box_tilt_repair}"
OUTPUT_ROOT="${OUTPUT_ROOT:-${ROOT_DIR}/experiments/outputs/core_world_g1_lowcarry_060_box_tilt_repair/${SUITE_STAMP_PREFIX}}"

mkdir -p "${OUTPUT_ROOT}"
status_file="${OUTPUT_ROOT}/lowcarry_060_box_tilt_repair_status.tsv"
summary_out="${OUTPUT_ROOT}/lowcarry_060_box_tilt_repair_summary.json"
...
run_repair_case() {
  ...
  echo "[G1-060-TILT] case=${case_name} suite_stamp=${suite_stamp}"
...
run_repair_case lid_lower \
  CRADLE_TOP_LID_LOCAL_Z=0.145 \
  CRADLE_TOP_LID_THICKNESS=0.018 \
  CRADLE_TOP_LID_X_SCALE=1.15 \
  CRADLE_TOP_LID_Y_SCALE=1.10 || overall_status=1

run_repair_case lid_lower_wide \
  CRADLE_TOP_LID_LOCAL_Z=0.145 \
  CRADLE_TOP_LID_THICKNESS=0.018 \
  CRADLE_TOP_LID_X_SCALE=1.25 \
  CRADLE_TOP_LID_Y_SCALE=1.25 \
  CRADLE_SIDE_RAIL_HEIGHT=0.14 \
  CRADLE_END_STOP_HEIGHT=0.15 || overall_status=1

run_repair_case lid_very_low \
  CRADLE_TOP_LID_LOCAL_Z=0.132 \
  CRADLE_TOP_LID_THICKNESS=0.020 \
  CRADLE_TOP_LID_X_SCALE=1.20 \
  CRADLE_TOP_LID_Y_SCALE=1.20 \
  CRADLE_SIDE_RAIL_HEIGHT=0.13 \
  CRADLE_END_STOP_HEIGHT=0.14 || overall_status=1

run_repair_case final_chest_pad \
  CRADLE_TOP_LID_LOCAL_Z=0.145 \
  CRADLE_TOP_LID_THICKNESS=0.018 \
  ...
  CRADLE_CHEST_PAD_SIZE_Z=0.22 || overall_status=1
