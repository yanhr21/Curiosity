...
if [[ "$(hostname)" == mgmtserver* ]]; then
  echo "Refusing to run G1 clean boxtilt box-progress suite on login/management node: $(hostname)" >&2
  exit 2
fi

ROOT_DIR="${ROOT_DIR:-/public/home/yanhongru/Curiosity}"
SUITE_STAMP_PREFIX="${SUITE_STAMP_PREFIX:-20260707_g1_boxtilt_clean_box_progress}"
OUTPUT_ROOT="${OUTPUT_ROOT:-${ROOT_DIR}/experiments/outputs/core_world_g1_boxtilt_clean_box_progress/${SUITE_STAMP_PREFIX}}"

cd "${ROOT_DIR}"
mkdir -p "${OUTPUT_ROOT}"

status_file="${OUTPUT_ROOT}/boxtilt_clean_box_progress_status.tsv"
summary_out="${OUTPUT_ROOT}/boxtilt_clean_box_progress_summary.json"
...
case_enabled() {
  local case_name="$1"
  local requested="${CLEAN_BOX_PROGRESS_CASES:-all}"
...
run_case() {
  ...
  if ! case_enabled "${case_name}"; then
    echo "[G1-BOXTILT-CLEAN-PROGRESS] skip case=${case_name} CLEAN_BOX_PROGRESS_CASES=${CLEAN_BOX_PROGRESS_CASES:-all}"
    return 0
  fi
  ...
  echo "[G1-BOXTILT-CLEAN-PROGRESS] case=${case_name} suite_stamp=${suite_stamp}"
...
run_case clean_default \
  AGILE_COMMAND_BOX_PROGRESS_GAIN=0.07 \
  AGILE_COMMAND_BOX_PROGRESS_MAX_FORWARD=0.095 \
  AGILE_COMMAND_BOX_PROGRESS_MAX_REVERSE=0.020 \
  AGILE_COMMAND_BOX_PROGRESS_MAX_TILT=0.55 \
  AGILE_COMMAND_BOX_PROGRESS_MAX_BOX_TILT=0.70
...
run_case clean_slow_lateral_pos \
  AGILE_COMMAND_BOX_PROGRESS_GAIN=0.040 \
  AGILE_COMMAND_BOX_PROGRESS_MAX_FORWARD=0.055 \
  AGILE_COMMAND_BOX_PROGRESS_MAX_REVERSE=0.015 \
  AGILE_COMMAND_BOX_PROGRESS_MAX_TILT=0.45 \
