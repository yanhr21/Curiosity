...
if [[ "$(hostname)" == mgmtserver* ]]; then
  echo "Refusing to run G1 close-front rescue freeze suite on login/management node: $(hostname)" >&2
  exit 2
fi

ROOT_DIR="${ROOT_DIR:-/public/home/yanhongru/Curiosity}"
SUITE_STAMP_PREFIX="${SUITE_STAMP_PREFIX:-20260707_g1_lowcarry_close_front_rescue_freeze}"
OUTPUT_ROOT="${OUTPUT_ROOT:-${ROOT_DIR}/experiments/outputs/core_world_g1_lowcarry_close_front_rescue_freeze/${SUITE_STAMP_PREFIX}}"

cd "${ROOT_DIR}"
mkdir -p "${OUTPUT_ROOT}"

status_file="${OUTPUT_ROOT}/close_front_rescue_freeze_status.tsv"
summary_out="${OUTPUT_ROOT}/close_front_rescue_freeze_summary.json"
...
run_case() {
  ...
  echo "[G1-CLOSE-FRONT-RESCUE-FREEZE] case=${case_name} suite_stamp=${suite_stamp}"
