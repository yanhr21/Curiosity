...
if [[ "$(hostname)" == mgmtserver* ]]; then
  echo "Refusing to run low-CG rear-anchor velocity batch on login/management node: $(hostname)" >&2
  exit 2
fi
...
run_diag() {
  ...
  local target_x="$2"
  local x_velocity="$3"
  local stamp_prefix="${STAMP_PREFIX:-20260705_lowcg_rear_anchor_velocity}"
  local stamp="${stamp_prefix}_${diag}_target${target_x}_vel${x_velocity}"
  local output_dir="experiments/outputs/core_world_prismatic_carrier_stand/${stamp}"
  echo "[BATCH] ${diag} rear_anchor_velocity target=${target_x} x_velocity=${x_velocity}"
  "${ISAAC_VENV}/bin/python" scripts/isaac/build_core_world_prismatic_carrier_stand.py \
    ...
    --tray-lid-mass 0.3 \
    --motion-mode rear_anchor_velocity_push \
    --enable-horizontal-legs \
    --target-x "${target_x}" \
    --x-slide-velocity "${x_velocity}" \
    --step-height 0.035 \
    --settle-steps 350 \
    --ramp-steps 300 \
    ...
    --x-slide-limit 0.08 \
    --x-slide-stiffness 300000.0 \
    --x-slide-damping 12000.0 \
...
run_diag "${DIAG_A:-diag12}" 0.030 0.030
run_diag "${DIAG_B:-diag13}" -0.030 0.030
run_diag "${DIAG_C:-diag14}" 0.030 0.080
