...
if [[ "$(hostname)" == mgmtserver* ]]; then
  echo "Refusing to run G1 held-out geometry early-support suite on login/management node: $(hostname)" >&2
  exit 2
fi

ROOT_DIR="${ROOT_DIR:-/public/home/yanhongru/Curiosity}"
SUITE_STAMP_PREFIX="${SUITE_STAMP_PREFIX:-20260707_g1_closefront_heldout_geometry_early_support}"
OUTPUT_ROOT="${OUTPUT_ROOT:-${ROOT_DIR}/experiments/outputs/core_world_g1_closefront_heldout_geometry_early_support/${SUITE_STAMP_PREFIX}}"

mkdir -p "${OUTPUT_ROOT}"
status_file="${OUTPUT_ROOT}/closefront_heldout_geometry_early_support_status.tsv"
summary_out="${OUTPUT_ROOT}/closefront_heldout_geometry_early_support_summary.json"
...
run_case() {
  ...
  echo "[G1-HELDOUT-GEOMETRY-EARLY] case=${case_name} suite_stamp=${suite_stamp}"
...
run_case tall_z009_early_stand40 \
