...
if [[ "$(hostname)" == mgmtserver* ]]; then
  echo "Refusing to run G1 low-creep latched-brake tuning on login/management node: $(hostname)" >&2
  exit 2
fi
...
SUITE_STAMP="${SUITE_STAMP:-$(date +%Y%m%d_core_world_g1_low_creep_latched_brake_tune)}"
SUITE_DIR="${SUITE_DIR:-${ROOT_DIR}/experiments/outputs/core_world_g1_low_creep_latched_brake_tune/${SUITE_STAMP}}"
DEVICE="${DEVICE:-cpu}"
STRICT="${STRICT:-0}"
STEPS="${STEPS:-700}"
...
status_file="${SUITE_DIR}/low_creep_latched_brake_tune_status.tsv"
...
base_python=(
  ...
  --creep-pitch-brake-amplitude-scale 0.0
  --creep-pitch-brake-push-scale 0.0
)
...
run_case() {
  ...
  mkdir -p "${out}"
  echo "[LATCH-BRAKE] ${case_id} brake_pitch=${brake_pitch} brake_bias=${brake_bias}"
  ...
  if [[ "${STRICT}" == "1" && ( "${build_status}" != "0" || "${check_status}" != "0" ) ]]; then
    echo "[LATCH-BRAKE] strict mode stopping after ${case_id}: build=${build_status} check=${check_status}" >&2
    exit 1
  fi
}

run_case latch012_bias0 0.12 0.0 0.08 0.25 0.14 0.10
run_case latch014_bias0 0.14 0.0 0.10 0.28 0.16 0.12
run_case latch016_bias0 0.16 0.0 0.12 0.32 0.18 0.14
run_case latch018_bias0 0.18 0.0 0.14 0.36 0.20 0.16
run_case latch016_bias02 0.16 0.2 0.12 0.32 0.18 0.14
run_case latch018_bias02 0.18 0.2 0.14 0.36 0.20 0.16

echo "[LATCH-BRAKE] status file: ${status_file}"
