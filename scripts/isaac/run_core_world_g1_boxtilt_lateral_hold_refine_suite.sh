...
if [[ "$(hostname)" == mgmtserver* ]]; then
  echo "Refusing to run G1 boxtilt lateral-hold refine suite on login/management node: $(hostname)" >&2
  exit 2
fi

ROOT_DIR="${ROOT_DIR:-/public/home/yanhongru/Curiosity}"
SUITE_STAMP_PREFIX="${SUITE_STAMP_PREFIX:-20260707_g1_boxtilt_lateral_hold_refine_760}"
OUTPUT_ROOT="${OUTPUT_ROOT:-${ROOT_DIR}/experiments/outputs/core_world_g1_boxtilt_lateral_hold_refine/${SUITE_STAMP_PREFIX}}"

cd "${ROOT_DIR}"
mkdir -p "${OUTPUT_ROOT}"

status_file="${OUTPUT_ROOT}/boxtilt_lateral_hold_refine_status.tsv"
summary_out="${OUTPUT_ROOT}/boxtilt_lateral_hold_refine_summary.json"
...
run_case() {
  ...
  echo "[G1-BOXTILT-LATERAL-HOLD-REFINE] case=${case_name} suite_stamp=${suite_stamp}"
  ...
  env \
    ...
    AGILE_COMMAND_HOLD_FINAL_ZERO_CORRECTIONS=0 \
...
run_case hold_sign_neg_l006 \
  AGILE_COMMAND_HOLD_LATERAL_SIGN=-1.0 \
  AGILE_COMMAND_HOLD_LATERAL_GAIN=0.025 \
  AGILE_COMMAND_HOLD_LATERAL_LIMIT=0.006

run_case hold_sign_neg_l012 \
  AGILE_COMMAND_HOLD_LATERAL_SIGN=-1.0 \
  AGILE_COMMAND_HOLD_LATERAL_GAIN=0.035 \
  AGILE_COMMAND_HOLD_LATERAL_LIMIT=0.012

run_case hold_sign_pos_l006 \
  AGILE_COMMAND_HOLD_LATERAL_SIGN=1.0 \
  AGILE_COMMAND_HOLD_LATERAL_GAIN=0.025 \
  AGILE_COMMAND_HOLD_LATERAL_LIMIT=0.006

run_case boxlat_sign_neg_l010 \
  AGILE_COMMAND_BOX_LATERAL_CONTROLLER=1 \
  AGILE_COMMAND_BOX_LATERAL_SIGN=-1.0 \
  AGILE_COMMAND_BOX_LATERAL_GAIN=0.030 \
