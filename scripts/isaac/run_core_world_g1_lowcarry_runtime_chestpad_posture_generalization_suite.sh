...
if [[ "$(hostname)" == mgmtserver* ]]; then
  echo "Refusing to run G1 posture generalization suite on login/management node: $(hostname)" >&2
  exit 2
fi

ROOT_DIR="${ROOT_DIR:-/public/home/yanhongru/Curiosity}"
SUITE_STAMP_PREFIX="${SUITE_STAMP_PREFIX:-20260707_g1_lowcarry_runtime_chestpad_posture_generalization}"
OUTPUT_ROOT="${OUTPUT_ROOT:-${ROOT_DIR}/experiments/outputs/core_world_g1_lowcarry_runtime_chestpad_posture_generalization/${SUITE_STAMP_PREFIX}}"

mkdir -p "${OUTPUT_ROOT}"
status_file="${OUTPUT_ROOT}/posture_generalization_status.tsv"
summary_out="${OUTPUT_ROOT}/posture_generalization_summary.json"
...
run_posture_case() {
  ...
  echo "[G1-POSTURE-GEN] case=${case_name} suite_stamp=${suite_stamp}"
...
run_posture_case low_front_060 || overall_status=1
run_posture_case close_front_060 \
  FREE_BOX_POS_X=-0.14 \
  FREE_CRADLE_LOCAL_X=-0.14 || overall_status=1
run_posture_case forward_reach_060 \
  FREE_BOX_POS_X=-0.22 \
  FREE_CRADLE_LOCAL_X=-0.22 || overall_status=1
run_posture_case wide_box_060 \
  FREE_BOX_SIZE_Y=0.14 \
  CRADLE_TOP_LID_Y_SCALE=1.35 \
  CRADLE_CHEST_PAD_SIZE_Y=0.46 || overall_status=1
run_posture_case low_front_080 \
  FREE_BOX_MASS=0.80 || overall_status=1
