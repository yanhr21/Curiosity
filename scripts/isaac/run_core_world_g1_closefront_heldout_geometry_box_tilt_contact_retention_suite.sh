...
if [[ "$(hostname)" == mgmtserver* ]]; then
  echo "Refusing to run G1 held-out geometry box-tilt contact-retention suite on login/management node: $(hostname)" >&2
  exit 2
fi

ROOT_DIR="${ROOT_DIR:-/public/home/yanhongru/Curiosity}"
SUITE_STAMP_PREFIX="${SUITE_STAMP_PREFIX:-20260708_g1_closefront_heldout_geometry_box_tilt_contact_retention}"
OUTPUT_ROOT="${OUTPUT_ROOT:-${ROOT_DIR}/experiments/outputs/core_world_g1_closefront_heldout_geometry_box_tilt_contact_retention/${SUITE_STAMP_PREFIX}}"

mkdir -p "${OUTPUT_ROOT}"
status_file="${OUTPUT_ROOT}/closefront_heldout_geometry_box_tilt_contact_retention_status.tsv"
summary_out="${OUTPUT_ROOT}/closefront_heldout_geometry_box_tilt_contact_retention_summary.json"
...
run_case() {
  ...
  echo "[G1-HELDOUT-GEOMETRY-BOX-TILT-CONTACT-RETENTION] case=${case_name} suite_stamp=${suite_stamp}"
  ...
  env \
    ...
    CRADLE_CHEST_PAD_BOX_TILT_MIN_STEP=380 \
    CRADLE_CHEST_PAD_BOX_TILT_THRESHOLD=0.18 \
    ...
    CRADLE_FINAL_SIDE_GUARD_ENABLE_ON_BOX_TILT=1 \
    CRADLE_FINAL_SIDE_GUARD_BOX_TILT_MIN_STEP=420 \
    CRADLE_FINAL_SIDE_GUARD_BOX_TILT_MIN_BOX_TARGET_TRAVEL=0.70 \
    CRADLE_FINAL_SIDE_GUARD_BOX_TILT_THRESHOLD=0.24 \
    ...
    CRADLE_FINAL_CROSS_BRACE_ENABLE_ON_BOX_TILT=1 \
    CRADLE_FINAL_CROSS_BRACE_BOX_TILT_MIN_STEP=460 \
    CRADLE_FINAL_CROSS_BRACE_BOX_TILT_MIN_BOX_TARGET_TRAVEL=0.90 \
    CRADLE_FINAL_CROSS_BRACE_BOX_TILT_THRESHOLD=0.34 \
