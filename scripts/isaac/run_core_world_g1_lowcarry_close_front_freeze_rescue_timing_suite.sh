...
if [[ "$(hostname)" == mgmtserver* ]]; then
  echo "Refusing to run G1 close-front freeze-rescue timing suite on login/management node: $(hostname)" >&2
  exit 2
fi

ROOT_DIR="${ROOT_DIR:-/public/home/yanhongru/Curiosity}"
SUITE_STAMP_PREFIX="${SUITE_STAMP_PREFIX:-20260707_g1_lowcarry_close_front_freeze_rescue_timing}"
OUTPUT_ROOT="${OUTPUT_ROOT:-${ROOT_DIR}/experiments/outputs/core_world_g1_lowcarry_close_front_freeze_rescue_timing/${SUITE_STAMP_PREFIX}}"

cd "${ROOT_DIR}"
mkdir -p "${OUTPUT_ROOT}"

status_file="${OUTPUT_ROOT}/close_front_freeze_rescue_timing_status.tsv"
summary_out="${OUTPUT_ROOT}/close_front_freeze_rescue_timing_summary.json"
...
run_case() {
  ...
  echo "[G1-CLOSE-FRONT-FREEZE-RESCUE-TIMING] case=${case_name} suite_stamp=${suite_stamp}"
...
run_case freeze_no_rescue \
  AGILE_COMMAND_HOLD_RESCUE_ENABLE=0

run_case freeze_rescue_late055 \
  AGILE_COMMAND_HOLD_RESCUE_ABS_ROLL_THRESHOLD=0.55 \
  AGILE_COMMAND_HOLD_RESCUE_BLEND_RATE=0.10

run_case freeze_rescue_soft035 \
  AGILE_COMMAND_HOLD_RESCUE_ABS_ROLL_THRESHOLD=0.35 \
  AGILE_COMMAND_HOLD_RESCUE_BLEND_RATE=0.06 \
  AGILE_COMMAND_HOLD_RESCUE_HIP_PITCH=-0.20 \
  AGILE_COMMAND_HOLD_RESCUE_KNEE=0.54 \
  AGILE_COMMAND_HOLD_RESCUE_ANKLE_PITCH=-0.32 \
