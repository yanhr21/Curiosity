...
if [[ "$(hostname)" == mgmtserver* ]]; then
  echo "Refusing to run G1 tall preintegrated mild-command suite on login/management node: $(hostname)" >&2
  exit 2
fi

ROOT_DIR="${ROOT_DIR:-/public/home/yanhongru/Curiosity}"
SUITE_STAMP_PREFIX="${SUITE_STAMP_PREFIX:-20260708_g1_tall_preintegrated_mild_command}"
OUTPUT_ROOT="${OUTPUT_ROOT:-${ROOT_DIR}/experiments/outputs/core_world_g1_tall_preintegrated_mild_command/${SUITE_STAMP_PREFIX}}"

mkdir -p "${OUTPUT_ROOT}"
status_file="${OUTPUT_ROOT}/g1_tall_preintegrated_mild_command_status.tsv"
summary_out="${OUTPUT_ROOT}/g1_tall_preintegrated_mild_command_summary.json"
...
run_case() {
  ...
  echo "[G1-TALL-PREINTEGRATED-MILD-COMMAND] case=${case_name} suite_stamp=${suite_stamp}"
...
run_case terminal_forward025 \
  AGILE_COMMAND_TERMINAL_SUPPORT_MAX_FORWARD=0.025 \
  AGILE_COMMAND_TERMINAL_SUPPORT_GAIN=0.036 || overall_status=1

run_case command_y_neg010 \
  AGILE_COMMAND_Y=-0.010 || overall_status=1

run_case command_y_pos010 \
  AGILE_COMMAND_Y=0.010 || overall_status=1
