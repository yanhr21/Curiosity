...
if [[ "$(hostname)" == mgmtserver* ]]; then
  echo "Refusing to run G1 close-front escape-suppression brake suite on login/management node: $(hostname)" >&2
  exit 2
fi

ROOT_DIR="${ROOT_DIR:-/public/home/yanhongru/Curiosity}"
SUITE_STAMP_PREFIX="${SUITE_STAMP_PREFIX:-20260707_g1_lowcarry_close_front_escape_suppression_brake}"
OUTPUT_ROOT="${OUTPUT_ROOT:-${ROOT_DIR}/experiments/outputs/core_world_g1_lowcarry_close_front_escape_suppression_brake/${SUITE_STAMP_PREFIX}}"

cd "${ROOT_DIR}"
mkdir -p "${OUTPUT_ROOT}"

status_file="${OUTPUT_ROOT}/close_front_escape_suppression_brake_status.tsv"
summary_out="${OUTPUT_ROOT}/close_front_escape_suppression_brake_summary.json"
...
run_case() {
  ...
  echo "[G1-CLOSE-FRONT-ESCAPE-SUPPRESS-BRAKE] case=${case_name} brake_x=${brake_x} suite_stamp=${suite_stamp}"
  ...
  env \
    ...
    MAX_FINAL_HOLD_COMMAND_X=0.008 \
    ...
    AGILE_COMMAND_HOLD_FINAL_BRAKE_COMMAND_X="${brake_x}" \
    AGILE_COMMAND_HOLD_FINAL_BRAKE_DELAY_STEPS=240 \
    AGILE_COMMAND_HOLD_FINAL_BRAKE_STEPS=120 \
...
run_case suppress60_brake240_neg004 -0.004
run_case suppress60_brake240_neg008 -0.008
