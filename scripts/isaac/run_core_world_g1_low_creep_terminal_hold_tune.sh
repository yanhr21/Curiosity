...
if [[ "$(hostname)" == mgmtserver* ]]; then
  echo "Refusing to run G1 low-creep terminal-hold tuning on login/management node: $(hostname)" >&2
  exit 2
fi
...
SUITE_STAMP="${SUITE_STAMP:-$(date +%Y%m%d_core_world_g1_low_creep_terminal_hold_tune)}"
SUITE_DIR="${SUITE_DIR:-${ROOT_DIR}/experiments/outputs/core_world_g1_low_creep_terminal_hold_tune/${SUITE_STAMP}}"
DEVICE="${DEVICE:-cpu}"
STRICT="${STRICT:-0}"
STEPS="${STEPS:-700}"
...
status_file="${SUITE_DIR}/low_creep_terminal_hold_tune_status.tsv"
...
run_case() {
  ...
  local hip_offset="$3"
  local knee_offset="$4"
  local ankle_offset="$5"
  local waist_offset="$6"
  local min_final_travel="$7"
  local max_tilt="$8"
  local max_rel="$9"
  local max_final_rel="${10}"
  local drive_gain="${11}"
  local drive_force="${12}"
  local out="${SUITE_DIR}/${case_id}"
  local build_log="${out}/build.log"
  local check_log="${out}/check.json"
  mkdir -p "${out}"
  echo "[LOW-HOLD] ${case_id} hold_travel=${hold_travel} offsets=(${hip_offset},${knee_offset},${ankle_offset},${waist_offset})"

  local extra_args=()
  if [[ "${drive_gain}" != "none" ]]; then
    extra_args+=(--terminal-drive-gain-scale "${drive_gain}" --terminal-drive-force-scale "${drive_force}")
  fi
  ...
  "${base_python[@]}" \
    ...
    --terminal-hold-hip-pitch-offset "${hip_offset}" \
    --terminal-hold-knee-offset "${knee_offset}" \
    --terminal-hold-ankle-pitch-offset "${ankle_offset}" \
    --terminal-hold-waist-pitch-offset "${waist_offset}" \
    "${extra_args[@]}" \
  ...
  if [[ "${STRICT}" == "1" && ( "${build_status}" != "0" || "${check_status}" != "0" ) ]]; then
    echo "[LOW-HOLD] strict mode stopping after ${case_id}: build=${build_status} check=${check_status}" >&2
    exit 1
  fi
}

run_case hold010_mild 0.10 -0.12 0.14 0.12 -0.08 0.10 0.25 0.14 0.10 none none
run_case hold012_mild 0.12 -0.12 0.14 0.12 -0.08 0.12 0.25 0.14 0.10 none none
run_case hold015_mild 0.15 -0.12 0.14 0.12 -0.08 0.15 0.25 0.14 0.10 none none
run_case hold012_strong 0.12 -0.18 0.22 0.18 -0.12 0.12 0.25 0.14 0.10 none none
run_case hold012_strong_drive 0.12 -0.18 0.22 0.18 -0.12 0.12 0.25 0.14 0.10 1.2 1.1

echo "[LOW-HOLD] status file: ${status_file}"
