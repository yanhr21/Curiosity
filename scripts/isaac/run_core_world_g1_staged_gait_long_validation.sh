...
if [[ "$(hostname)" == mgmtserver* ]]; then
  echo "Refusing to run staged-gait long validation on login/management node: $(hostname)" >&2
  exit 2
fi
...
run_diag() {
  ...
  local ramp_start="$5"
  local ramp_end="$6"
  local stamp="20260705_core_world_g1_staged_long_${diag}_from${source_diag}_mass${cradle_mass}_amp${amp}_700"
  local output_dir="experiments/outputs/core_world_g1_box_scene/${stamp}"
  echo "[BATCH] ${diag} long validation source=${source_diag} cradle_mass=${cradle_mass} amp=${amp}"
...
run_diag diag75 diag72 0.90 0.11 100 210
run_diag diag76 diag73 0.95 0.10 90 190
