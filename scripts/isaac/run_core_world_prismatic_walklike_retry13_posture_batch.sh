...
env "${COMMON_ENV[@]}" \
  STAMP=20260705_prismatic_cradle_walklike_posture_high_z018_retry13a \
  PAYLOAD_LOCAL_X=0.50 \
  PAYLOAD_LOCAL_Z=0.18 \
  bash scripts/isaac/run_core_world_prismatic_carrier_stand.sh

env "${COMMON_ENV[@]}" \
  STAMP=20260705_prismatic_cradle_walklike_posture_close_x045_retry13b \
  PAYLOAD_LOCAL_X=0.45 \
  PAYLOAD_LOCAL_Z=0.16 \
  bash scripts/isaac/run_core_world_prismatic_carrier_stand.sh

env "${COMMON_ENV[@]}" \
  STAMP=20260705_prismatic_cradle_walklike_posture_low_z014_retry13c \
  PAYLOAD_LOCAL_X=0.50 \
