...
if [[ "$(hostname)" == mgmtserver* ]]; then
  echo "Refusing to run G1 close-front final-stabilize suite on login/management node: $(hostname)" >&2
  exit 2
fi
...
summary_out="${OUTPUT_ROOT}/close_front_final_stabilize_summary.json"
printf "case\tstatus\tsuite_stamp\n" > "${status_file}"
summary_case_roots=()

run_case() {
  local case_name="$1"
  local tilt_threshold="$2"
  local suite_stamp="${SUITE_STAMP_PREFIX}_${case_name}"
  echo "[G1-CLOSE-FINAL-STAB] case=${case_name} tilt_threshold=${tilt_threshold} suite_stamp=${suite_stamp}"
  ...
  env \
    ...
    CRADLE_CHEST_PAD_BOX_TILT_THRESHOLD="${tilt_threshold}" \
