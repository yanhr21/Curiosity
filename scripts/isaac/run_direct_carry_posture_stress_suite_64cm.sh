...
if [[ "$(hostname)" == mgmtserver* ]]; then
  echo "Refusing to run carry posture stress suite on login/management node: $(hostname)" >&2
  exit 2
fi
...
run_case() {
  ...
  echo "[STRESS_SUITE] posture=${posture} stamp=${stamp}"
  set +e
  STAMP="${stamp}" \
...
run_case low_front
run_case chest_high
run_case front_reach
run_case close_mid

python3 scripts/isaac/summarize_direct_carry_posture_suite.py \
  "${SUMMARY_ARGS[@]}" \
  --output "${SUITE_DIR}/direct_carry_posture_stress_suite_summary.json" \
  --min-postures 5 \
  --min-steps 3560 \
...
echo "[INFO] Stress suite output: ${SUITE_DIR}"
