...
if [[ "$(hostname)" == mgmtserver* ]]; then
  echo "Refusing to run conservative staged-gait validation on login/management node: $(hostname)" >&2
  exit 2
fi
...
run_diag() {
  ...
  local ramp_start="$4"
  local ramp_end="$5"
  local stamp="20260705_core_world_g1_staged_cons_long_${diag}_mass${cradle_mass}_amp${amp}_700"
  local output_dir="experiments/outputs/core_world_g1_box_scene/${stamp}"
  echo "[BATCH] ${diag} cradle_mass=${cradle_mass} amp=${amp} ramp=${ramp_start}-${ramp_end}"
...
run_diag diag77 0.80 0.12 110 220
run_diag diag78 0.85 0.12 110 220
run_diag diag79 0.80 0.10 90 190
run_diag diag80 0.85 0.10 90 190
