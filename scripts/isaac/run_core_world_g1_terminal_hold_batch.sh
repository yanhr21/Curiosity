...
if [[ "$(hostname)" == mgmtserver* ]]; then
  echo "Refusing to run terminal-hold validation on login/management node: $(hostname)" >&2
  exit 2
fi
...
run_diag() {
  ...
  local ramp_start="$4"
  local ramp_end="$5"
  local trigger_travel="$6"
  local hip_offset="$7"
  local knee_offset="$8"
  local ankle_offset="$9"
  local waist_offset="${10}"
  local stamp="20260705_core_world_g1_terminal_hold_${diag}_mass${cradle_mass}_amp${amp}_travel${trigger_travel}"
  local output_dir="experiments/outputs/core_world_g1_box_scene/${stamp}"
  echo "[BATCH] ${diag} cradle_mass=${cradle_mass} amp=${amp} ramp=${ramp_start}-${ramp_end} terminal_box_travel=${trigger_travel} offsets=${hip_offset},${knee_offset},${ankle_offset},${waist_offset}"
  "${ISAAC_VENV}/bin/python" scripts/isaac/build_core_world_g1_box_scene.py \
    ...
    --terminal-hold-box-target-travel "${trigger_travel}" \
    --terminal-hold-pitch-threshold 0.45 \
  ...
  python3 scripts/isaac/check_core_world_g1_box_scene_summary.py \
    ...
    --min-final-box-target-directed-travel 0.30 \
    --require-diagnostic-claim || true
}

run_diag diag81 0.90 0.11 100 210 0.35 0.12 -0.10 -0.12 0.10
run_diag diag82 0.90 0.11 100 210 0.35 0.20 -0.16 -0.18 0.16
run_diag diag83 0.95 0.10 90 190 0.45 0.12 -0.10 -0.12 0.10
run_diag diag84 0.95 0.10 90 190 0.45 -0.10 0.12 0.12 -0.08
