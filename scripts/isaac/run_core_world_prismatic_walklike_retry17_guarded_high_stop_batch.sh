...
env "${COMMON_ENV[@]}" \
  STAMP=20260705_prismatic_cradle_walklike_guarded_high_mid_x050_z018_stop_retry17a \
  PAYLOAD_LOCAL_X=0.50 \
  GUARDED_STEP_TARGET_TOLERANCE=0.018 \
  bash scripts/isaac/run_core_world_prismatic_carrier_stand.sh

env "${COMMON_ENV[@]}" \
  STAMP=20260705_prismatic_cradle_walklike_guarded_high_mid_x050_z018_stop_tol030_retry17b \
  PAYLOAD_LOCAL_X=0.50 \
  GUARDED_STEP_TARGET_TOLERANCE=0.030 \
  bash scripts/isaac/run_core_world_prismatic_carrier_stand.sh

env "${COMMON_ENV[@]}" \
  STAMP=20260705_prismatic_cradle_walklike_guarded_high_close_x045_z018_overdrive16_stop_retry17c \
