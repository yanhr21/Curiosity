...
SUITE_SUFFIX="${SUITE_SUFFIX:-foot_ik_roll_feedback}"
...
run_case() {
  ...
  echo "[CASE] ${stamp} roll_gain=${roll_gain} stop=${stop_after} host=$(hostname)"
  STAMP="${stamp}" \
  STEPS=3000 \
  BOX_MASS=2.0 \
  TARGET_SPEED=0.24 \
...
run_case roll_pos006 0.06 0.08
run_case roll_neg006 -0.06 0.08
run_case roll_pos012 0.12 0.08
run_case roll_neg012 -0.12 0.08

echo "[DONE] MuJoCo free-box foot-IK roll-feedback suite suffix=${SUITE_SUFFIX}"
