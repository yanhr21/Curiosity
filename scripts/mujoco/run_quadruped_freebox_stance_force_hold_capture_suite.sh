...
SUITE_SUFFIX="${SUITE_SUFFIX:-stance_force_holdcapture}"
...
run_case() {
  ...
  echo "[CASE] ${stamp} cap_x=${cap_x_gain} cap_y_hip=${cap_y_hip} cap_y_foot=${cap_y_foot} tau=${cap_tau} limits=${cap_x_limit}/${cap_y_limit} host=$(hostname)"
  STAMP="${stamp}" \
  STEPS=3000 \
  BOX_MASS=2.0 \
  TARGET_SPEED=0.16 \
  ...
  HOLD_CAPTURE_POINT_FOOT_PLACEMENT=1 \
  HOLD_CAPTURE_TIME_CONSTANT="${cap_tau}" \
  HOLD_CAPTURE_X_GAIN="${cap_x_gain}" \
  HOLD_CAPTURE_X_LIMIT="${cap_x_limit}" \
  HOLD_CAPTURE_Y_HIP_GAIN="${cap_y_hip}" \
  HOLD_CAPTURE_Y_FOOT_Z_GAIN="${cap_y_foot}" \
  HOLD_CAPTURE_Y_LIMIT="${cap_y_limit}" \
  ...
  "${PYTHON_BIN}" scripts/mujoco/check_quadruped_freebox_summary.py "${summary}" \
    ...
    --require-hold-capture-point-foot-placement \
    --min-hold-capture-active-steps 600 \
...
run_case capture_x_pos 1.0 0.0 0.0 0.18 0.06 0.08
run_case capture_x_neg -1.0 0.0 0.0 0.18 0.06 0.08
run_case capture_y_hip_pos 0.0 1.2 0.0 0.18 0.06 0.08
run_case capture_y_hip_neg 0.0 -1.2 0.0 0.18 0.06 0.08
run_case capture_xy_pos 1.0 1.2 -0.18 0.18 0.06 0.08
run_case capture_xy_neg -1.0 -1.2 0.18 0.18 0.06 0.08

echo "[DONE] MuJoCo free-box stance-force hold capture suite suffix=${SUITE_SUFFIX}"
