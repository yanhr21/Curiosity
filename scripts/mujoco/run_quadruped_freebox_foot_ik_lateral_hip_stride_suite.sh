...
SUITE_SUFFIX="${SUITE_SUFFIX:-foot_ik_lateral_hip_stride}"
...
run_case() {
  ...
  echo "[CASE] ${stamp} stride=${stride} hip_base=${hip_base} foot_roll=${foot_roll_gain} host=$(hostname)"
  STAMP="${stamp}" \
  STEPS=3000 \
  BOX_MASS=2.0 \
  TARGET_SPEED=0.24 \
...
run_case base010_pos12 0.12 0.10 -0.04
run_case base010_pos16 0.16 0.10 -0.04
run_case base008_pos12 0.12 0.08 -0.04
run_case base008_neg12 -0.12 0.08 -0.04
run_case base006_neg12 -0.12 0.06 -0.04

echo "[DONE] MuJoCo free-box foot-IK lateral-hip stride suite suffix=${SUITE_SUFFIX}"
