...
SUITE_SUFFIX="${SUITE_SUFFIX:-stance_force_holdworldy}"
...
run_case() {
  local tag="$1"
  local source="$2"
  local world_y_gain="$3"
  local world_vy_gain="$4"
  local max_fy="$5"
  local hip_feedback="$6"
  ...
  echo "[CASE] ${stamp} source=${source} world_y=${world_y_gain} world_vy=${world_vy_gain} max_fy=${max_fy} hip_fb=${hip_feedback} host=$(hostname)"
  STAMP="${stamp}" \
  STEPS=3000 \
  BOX_MASS=2.0 \
  TARGET_SPEED=0.16 \
  ...
  SUPPORT_FY_ROLL_GAIN=0.0 \
  SUPPORT_FY_ROLL_RATE_GAIN=0.0 \
  SUPPORT_FY_COM_Y_GAIN=0.0 \
  SUPPORT_FY_WORLD_Y_SOURCE="${source}" \
  SUPPORT_FY_WORLD_Y_GAIN="${world_y_gain}" \
  SUPPORT_FY_WORLD_VY_GAIN="${world_vy_gain}" \
...
run_case worldy_box_pos180 box 180.0 35.0 120.0 -0.30
run_case worldy_box_neg180 box -180.0 -35.0 120.0 -0.30
run_case worldy_torso_pos180 torso 180.0 35.0 120.0 -0.30
run_case worldy_torso_neg180 torso -180.0 -35.0 120.0 -0.30
run_case worldy_box_pos280 box 280.0 55.0 160.0 -0.30
run_case worldy_torso_pos280 torso 280.0 55.0 160.0 -0.30
run_case worldy_box_pos180_hip040 box 180.0 35.0 120.0 -0.40
run_case worldy_torso_pos180_hip040 torso 180.0 35.0 120.0 -0.40

echo "[DONE] MuJoCo free-box stance-force hold world-y suite suffix=${SUITE_SUFFIX}"
