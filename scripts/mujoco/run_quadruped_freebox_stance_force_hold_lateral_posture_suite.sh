...
SUITE_SUFFIX="${SUITE_SUFFIX:-stance_force_holdlatposture}"
...
run_case() {
  ...
  echo "[CASE] ${stamp} hip_fb=${hip_feedback} foot_roll_z=${foot_roll_z} fy_roll=${fy_roll} fy_rate=${fy_roll_rate} fy_com_y=${fy_com_y} max_fy=${max_fy} host=$(hostname)"
  STAMP="${stamp}" \
  STEPS=3000 \
  BOX_MASS=2.0 \
  TARGET_SPEED=0.16 \
  ...
  HOLD_FOOT_ROLL_Z_GAIN="${foot_roll_z}" \
  ...
  SUPPORT_FY_ROLL_GAIN="${fy_roll}" \
  SUPPORT_FY_ROLL_RATE_GAIN="${fy_roll_rate}" \
  SUPPORT_FY_COM_Y_GAIN="${fy_com_y}" \
  SUPPORT_MAX_TOTAL_FY="${max_fy}" \
  SUPPORT_FY_PRE_LATCH_SCALE=0.0 \
  HOLD_SUPPORT_FY_SCALE=1.0 \
...
run_case latpost_hip030_roll_pos120 -0.30 -0.04 120.0 20.0 0.0 80.0
run_case latpost_hip030_roll_neg120 -0.30 -0.04 -120.0 -20.0 0.0 80.0
run_case latpost_hip030_com_pos220 -0.30 -0.04 0.0 0.0 220.0 80.0
run_case latpost_hip030_com_neg220 -0.30 -0.04 0.0 0.0 -220.0 80.0
run_case latpost_hip030_combo_pos -0.30 -0.04 120.0 20.0 220.0 120.0
run_case latpost_hip030_combo_neg -0.30 -0.04 -120.0 -20.0 -220.0 120.0
run_case latpost_hip040_combo_pos -0.40 -0.04 120.0 20.0 220.0 120.0
run_case latpost_combofoot_pos -0.30 -0.08 120.0 20.0 220.0 120.0

echo "[DONE] MuJoCo free-box stance-force hold lateral posture suite suffix=${SUITE_SUFFIX}"
