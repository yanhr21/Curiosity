...
SUITE_SUFFIX="${SUITE_SUFFIX:-foot_ik_lateral_hip}"
...
run_case() {
  ...
  echo "[CASE] ${stamp} hip_base=${hip_base} hip_gain=${hip_gain} foot_roll=${foot_roll_gain} host=$(hostname)"
  STAMP="${stamp}" \
  STEPS=3000 \
  BOX_MASS=2.0 \
  TARGET_SPEED=0.24 \
  ...
  FOOT_ROLL_Z_GAIN="${foot_roll_gain}" \
  HIP_ROLL_BASE="${hip_base}" \
  HIP_ROLL_FEEDBACK_GAIN="${hip_gain}" \
...
run_case hip_base005 0.05 0.0 -0.04
run_case hip_base010 0.10 0.0 -0.04
run_case hip_base015 0.15 0.0 -0.04
run_case hip_base010_fbneg 0.10 -0.15 -0.04
run_case hip_base010_fbpos 0.10 0.15 -0.04

echo "[DONE] MuJoCo free-box foot-IK lateral-hip suite suffix=${SUITE_SUFFIX}"
