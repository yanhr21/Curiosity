...
SUITE_SUFFIX="${SUITE_SUFFIX:-lqr_additive_hold}"
...
run_case() {
  ...
  echo "[CASE] ${stamp} additive_lqr q=${q_pos}/${q_vel} r=${r_cost} lqr_max=${max_fx}/${max_fy} host=$(hostname)"
  STAMP="${stamp}" \
  STEPS=3000 \
  BOX_MASS=2.0 \
  TARGET_SPEED=0.16 \
...
run_case lqr_add_nominal 80.0 8.0 1.0 120.0 120.0
run_case lqr_add_soft 40.0 12.0 2.0 80.0 100.0
run_case lqr_add_ystrong 60.0 10.0 1.0 80.0 180.0
run_case lqr_add_xsoft_ystrong 30.0 12.0 2.0 60.0 180.0

echo "[DONE] MuJoCo free-box additive LQR stance-hold suite suffix=${SUITE_SUFFIX}"
