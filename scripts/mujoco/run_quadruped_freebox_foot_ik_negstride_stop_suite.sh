...
SUITE_SUFFIX="${SUITE_SUFFIX:-foot_ik_negstride_stop}"
...
run_case() {
  ...
  echo "[CASE] ${stamp} hold=${hold_speed} stride=${stride} stop=${stop_after} host=$(hostname)"
  STAMP="${stamp}" \
  STEPS=3000 \
  BOX_MASS=2.0 \
  TARGET_SPEED=0.24 \
...
run_case neg_high_stop0 0.00 -0.12 0.15
run_case neg_high_hold004 0.04 -0.12 0.15
run_case neg_high_hold008 0.08 -0.12 0.15
run_case neg_mid_stop0 0.00 -0.10 0.15

echo "[DONE] MuJoCo free-box foot-IK negative-stride stop suite suffix=${SUITE_SUFFIX}"
