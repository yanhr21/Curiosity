...
SUITE_SUFFIX="${SUITE_SUFFIX:-foot_ik_early_stop}"
...
run_case() {
  ...
  echo "[CASE] ${stamp} stop=${stop_after} stride=${stride} freq=${freq} host=$(hostname)"
  STAMP="${stamp}" \
  STEPS=3000 \
  BOX_MASS=2.0 \
  TARGET_SPEED=0.24 \
...
run_case early08 0.08 -0.12 1.1
run_case early10 0.10 -0.12 1.1
run_case early12 0.12 -0.12 1.1
run_case early10_slow 0.10 -0.10 0.9

echo "[DONE] MuJoCo free-box foot-IK early-stop suite suffix=${SUITE_SUFFIX}"
