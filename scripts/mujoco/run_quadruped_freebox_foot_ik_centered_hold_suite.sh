...
SUITE_SUFFIX="${SUITE_SUFFIX:-foot_ik_centered_hold}"
...
run_case() {
  ...
  echo "[CASE] ${stamp} hip_base=${hip_base} hold_hip=${hold_hip} retention_y=${retention_y} host=$(hostname)"
  STAMP="${stamp}" \
  STEPS=3000 \
  BOX_MASS=2.0 \
  TARGET_SPEED=0.24 \
...
run_case centered_nohip_y140 0.00 0.08 140.0
run_case centered_nohip_y220 0.00 0.08 220.0
run_case centered_hip003_y140 0.03 0.10 140.0
run_case centered_hip003_y220 0.03 0.10 220.0
run_case centered_hip004_y180 0.04 0.10 180.0
run_case centered_hip005_y180 0.05 0.10 180.0

echo "[DONE] MuJoCo free-box foot-IK centered hold suite suffix=${SUITE_SUFFIX}"
