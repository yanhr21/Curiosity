...
SUITE_SUFFIX="${SUITE_SUFFIX:-centroidal_support}"

cd "${ROOT_DIR}"

run_case() {
  local tag="$1"
  local support_force_scale="$2"
  local hip_feedback="$3"
  local max_foot_scale="$4"
  local max_torque_scale="$5"
  local max_fy="$6"
  ...
  echo "[CASE] ${stamp} support_force_scale=${support_force_scale} hip_fb=${hip_feedback} foot_fz_scale=${max_foot_scale} torque_scale=${max_torque_scale} max_fy=${max_fy} host=$(hostname)"
  STAMP="${stamp}" \
  STEPS=3000 \
  BOX_MASS=2.0 \
  TARGET_SPEED=0.16 \
  ...
  STRIDE_CLIP=0.12 \
  SUPPORT_CONTROLLER_MODE=centroidal_stance_force \
  SUPPORT_FORCE_SCALE="${support_force_scale}" \
  SUPPORT_KP_Z=4200.0 \
  SUPPORT_KD_Z=240.0 \
  ...
  "${PYTHON_BIN}" scripts/mujoco/check_quadruped_freebox_summary.py "${summary}" \
    ...
    --expect-support-controller-mode centroidal_stance_force \
...
run_case centroid_negscale_hip030 -0.36 -0.30 1.0 1.0 120.0
run_case centroid_posscale_hip030 0.36 -0.30 1.0 1.0 120.0
run_case centroid_negscale_hip040 -0.36 -0.40 1.0 1.0 120.0
run_case centroid_posscale_hip040 0.36 -0.40 1.0 1.0 120.0
run_case centroid_negscale_authority -0.36 -0.30 1.5 1.5 160.0
run_case centroid_posscale_authority 0.36 -0.30 1.5 1.5 160.0

echo "[DONE] MuJoCo free-box centroidal support suite suffix=${SUITE_SUFFIX}"
