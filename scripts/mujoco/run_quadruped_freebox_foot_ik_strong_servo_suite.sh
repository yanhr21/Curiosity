...
SUITE_SUFFIX="${SUITE_SUFFIX:-foot_ik_strong_servo}"
...
run_case() {
  ...
  echo "[CASE] ${stamp} kp=${actuator_kp} kv=${actuator_kv} hip_base=${hip_base} hip_feedback=${hip_feedback} retention_y=${retention_y} host=$(hostname)"
  STAMP="${stamp}" \
  STEPS=3000 \
  BOX_MASS=2.0 \
  TARGET_SPEED=0.24 \
...
run_case kp140_kv14_nohip 140.0 14.0 0.00 0.00 180.0
run_case kp180_kv18_nohip 180.0 18.0 0.00 0.00 180.0
run_case kp140_kv14_hip003 140.0 14.0 0.03 0.00 180.0
run_case kp180_kv18_hip003 180.0 18.0 0.03 0.00 180.0
run_case kp140_kv14_hip004_fbneg 140.0 14.0 0.04 -0.30 180.0
run_case kp180_kv18_hip004_fbneg 180.0 18.0 0.04 -0.30 180.0

echo "[DONE] MuJoCo free-box foot-IK strong-servo suite suffix=${SUITE_SUFFIX}"
