...
SUITE_SUFFIX="${SUITE_SUFFIX:-stance_force_holdauthority}"

cd "${ROOT_DIR}"

run_case() {
  local tag="$1"
  local hip_feedback="$2"
  local max_foot_scale="$3"
  local max_torque_scale="$4"
  local kd_roll_scale="$5"
  local kd_z_scale="$6"
  ...
  echo "[CASE] ${stamp} hip_fb=${hip_feedback} foot_fz_scale=${max_foot_scale} torque_scale=${max_torque_scale} kd_roll=${kd_roll_scale} kd_z=${kd_z_scale} height=${height_offset} host=$(hostname)"
  STAMP="${stamp}" \
  STEPS=3000 \
  BOX_MASS=2.0 \
  TARGET_SPEED=0.16 \
  ...
  HOLD_HIP_ROLL_FEEDBACK_GAIN="${hip_feedback}" \
  ...
  HOLD_SUPPORT_KD_Z_SCALE="${kd_z_scale}" \
  HOLD_SUPPORT_KD_ROLL_SCALE="${kd_roll_scale}" \
  HOLD_SUPPORT_KD_PITCH_SCALE=1.5 \
  HOLD_SUPPORT_MAX_FOOT_FZ_SCALE="${max_foot_scale}" \
  HOLD_SUPPORT_MAX_JOINT_TORQUE_SCALE="${max_torque_scale}" \
  HOLD_SUPPORT_HEIGHT_OFFSET="${height_offset}" \
...
run_case authority_torque15 -0.30 1.0 1.5 1.5 1.5 0.00
run_case authority_torque20 -0.30 1.0 2.0 1.5 1.5 0.00
run_case authority_fz15_torque15 -0.30 1.5 1.5 1.5 1.5 0.00
run_case authority_fz15_torque20 -0.30 1.5 2.0 1.5 1.5 0.00
run_case authority_high03 -0.30 1.5 1.5 1.8 1.8 0.03
run_case authority_high05 -0.30 1.8 2.0 2.0 2.0 0.05
run_case authority_hip040_high03 -0.40 1.5 1.5 1.8 1.8 0.03
run_case authority_hip020_high03 -0.20 1.5 1.5 1.8 1.8 0.03

echo "[DONE] MuJoCo free-box stance-force hold authority suite suffix=${SUITE_SUFFIX}"
