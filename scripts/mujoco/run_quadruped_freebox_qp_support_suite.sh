...
SUITE_SUFFIX="${SUITE_SUFFIX:-qp_support}"
...
run_case() {
  ...
  echo "[CASE] ${stamp} qp mu=${mu} angular=${angular_weight} reg=${qp_reg} recovery=${recovery} host=$(hostname)"
  STAMP="${stamp}" \
  STEPS=3000 \
  BOX_MASS=2.0 \
  TARGET_SPEED=0.16 \
...
run_case qp_nominal 0.90 1.0 0.001 0
run_case qp_roll_weight 0.90 2.5 0.001 0
run_case qp_recovery 0.90 1.5 0.001 1
run_case qp_conservative 0.65 2.0 0.010 1

echo "[DONE] MuJoCo free-box constrained QP support suite suffix=${SUITE_SUFFIX}"
