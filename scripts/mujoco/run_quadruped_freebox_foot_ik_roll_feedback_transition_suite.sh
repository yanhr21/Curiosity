...
SUITE_SUFFIX="${SUITE_SUFFIX:-foot_ik_roll_feedback_transition}"
...
run_case() {
  ...
  echo "[CASE] ${stamp} roll_gain=${roll_gain} stride=${stride} host=$(hostname)"
  STAMP="${stamp}" \
  STEPS=3000 \
  BOX_MASS=2.0 \
  TARGET_SPEED=0.24 \
...
run_case roll_neg0045 -0.045 -0.12
run_case roll_neg0050 -0.050 -0.12
run_case roll_neg0055 -0.055 -0.12
run_case roll_neg0050_stride10 -0.050 -0.10

echo "[DONE] MuJoCo free-box foot-IK roll-feedback transition suite suffix=${SUITE_SUFFIX}"
