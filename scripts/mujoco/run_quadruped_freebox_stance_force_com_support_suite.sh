...
SUITE_SUFFIX="${SUITE_SUFFIX:-stance_force_comsupport}"
...
run_case() {
  ...
  echo "[CASE] ${stamp} com_x=${com_x_gain} com_vx=${com_vx_gain} com_y=${com_y_gain} limit=${com_shift_limit} hold_scale=${hold_com_scale} feet=${front_x}/${rear_x} host=$(hostname)"
  STAMP="${stamp}" \
  STEPS=3000 \
  BOX_MASS=2.0 \
  TARGET_SPEED=0.16 \
...
run_case com_pos400 400.0 0.0 0.0 120.0 1.0 0.22 -0.22
run_case com_pos800 800.0 0.0 0.0 160.0 1.0 0.22 -0.22
run_case com_pos1200 1200.0 0.0 0.0 200.0 1.0 0.22 -0.22
run_case com_neg400 -400.0 0.0 0.0 120.0 1.0 0.22 -0.22
run_case com_neg800 -800.0 0.0 0.0 160.0 1.0 0.22 -0.22
run_case com_pos800_vx 800.0 80.0 0.0 180.0 1.0 0.22 -0.22
run_case com_pos800_y 800.0 80.0 250.0 180.0 1.0 0.22 -0.22
run_case com_pos800_basefeet 800.0 80.0 250.0 180.0 1.0 0.14 -0.10

echo "[DONE] MuJoCo free-box stance-force COM support suite suffix=${SUITE_SUFFIX}"
