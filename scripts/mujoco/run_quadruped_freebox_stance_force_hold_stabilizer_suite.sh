...
SUITE_SUFFIX="${SUITE_SUFFIX:-stance_force_holdstab}"

cd "${ROOT_DIR}"

run_case() {
  local tag="$1"
  local hold_fx_scale="$2"
  local hold_max_fx_scale="$3"
  local hold_kp_vx_scale="$4"
  local hold_kd_scale="$5"
  local hold_height_offset="$6"
  local stop_after="$7"
  ...
  echo "[CASE] ${stamp} hold_fx=${hold_fx_scale} hold_max_fx=${hold_max_fx_scale} hold_kp_vx=${hold_kp_vx_scale} hold_kd=${hold_kd_scale} height=${hold_height_offset} stop=${stop_after} host=$(hostname)"
  STAMP="${stamp}" \
  STEPS=3000 \
  BOX_MASS=2.0 \
  TARGET_SPEED=0.16 \
  ...
  HOLD_SUPPORT_KP_VX_SCALE="${hold_kp_vx_scale}" \
  HOLD_SUPPORT_KD_Z_SCALE="${hold_kd_scale}" \
  HOLD_SUPPORT_KD_ROLL_SCALE="${hold_kd_scale}" \
  HOLD_SUPPORT_KD_PITCH_SCALE="${hold_kd_scale}" \
  HOLD_SUPPORT_HEIGHT_OFFSET="${hold_height_offset}" \
...
run_case holdfx_zero -0.00 0.50 1.5 1.5 0.00 0.05
run_case holdfx_pos015 0.15 0.60 1.5 1.5 0.00 0.05
run_case holdfx_pos030 0.30 0.60 1.5 1.5 0.00 0.05
run_case holdfx_pos060 0.60 0.60 1.5 1.5 0.00 0.05
run_case holdfx_neg010 -0.10 0.60 1.5 1.5 0.00 0.05
run_case holdfx_pos030_high 0.30 0.60 1.5 1.8 0.03 0.05
run_case holdfx_pos030_stop04 0.30 0.60 1.5 1.5 0.00 0.04
run_case holdfx_pos030_soft 0.30 0.40 1.0 1.5 0.00 0.05

echo "[DONE] MuJoCo free-box stance-force hold stabilizer suite suffix=${SUITE_SUFFIX}"
