...
SUITE_SUFFIX="${SUITE_SUFFIX:-stance_force_holdlat}"
...
run_case() {
  ...
  echo "[CASE] ${stamp} roll_gain=${roll_gain} rate_gain=${rate_gain} com_y=${com_y_gain} max_fy=${max_fy} feet=${front_x}/${rear_x} host=$(hostname)"
  STAMP="${stamp}" \
  STEPS=3000 \
  BOX_MASS=2.0 \
  TARGET_SPEED=0.16 \
  ...
  SUPPORT_FY_ROLL_GAIN="${roll_gain}" \
  SUPPORT_FY_ROLL_RATE_GAIN="${rate_gain}" \
  SUPPORT_FY_COM_Y_GAIN="${com_y_gain}" \
...
run_case lat_pos80 80.0 12.0 0.0 80.0 0.22 -0.22
run_case lat_pos160 160.0 24.0 0.0 120.0 0.22 -0.22
run_case lat_pos240 240.0 36.0 0.0 160.0 0.22 -0.22
run_case lat_neg80 -80.0 -12.0 0.0 80.0 0.22 -0.22
run_case lat_neg160 -160.0 -24.0 0.0 120.0 0.22 -0.22
run_case lat_pos160_comy 160.0 24.0 180.0 140.0 0.22 -0.22
run_case lat_neg160_comy -160.0 -24.0 -180.0 140.0 0.22 -0.22
run_case lat_pos160_basefeet 160.0 24.0 180.0 140.0 0.14 -0.10

echo "[DONE] MuJoCo free-box stance-force hold lateral suite suffix=${SUITE_SUFFIX}"
