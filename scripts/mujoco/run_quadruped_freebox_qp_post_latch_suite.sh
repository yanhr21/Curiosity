...
SUITE_SUFFIX="${SUITE_SUFFIX:-qp_post_latch}"
...
run_case() {
  ...
  echo "[CASE] ${stamp} post_latch_qp mu=${mu} angular=${angular_weight} reg=${qp_reg} recovery=${recovery} host=$(hostname)"
  STAMP="${stamp}" \
  STEPS=3000 \
  BOX_MASS=2.0 \
  TARGET_SPEED=0.16 \
  ...
  SUPPORT_QP_REGULARIZATION="${qp_reg}" \
...
run_case qp_post_nominal 0.90 1.0 0.001 0
run_case qp_post_roll_weight 0.90 2.5 0.001 0
run_case qp_post_recovery 0.90 1.5 0.001 1
run_case qp_post_conservative 0.65 2.0 0.010 1

echo "[DONE] MuJoCo free-box post-latch constrained QP support suite suffix=${SUITE_SUFFIX}"
