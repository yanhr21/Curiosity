...
SUITE_SUFFIX="${SUITE_SUFFIX:-stance_force_brake}"
...
run_case() {
  ...
  echo "[CASE] ${stamp} stop=${stop_after} hold=${hold_speed} scale=${force_scale} kp_vx=${kp_vx} max_fx=${max_fx} hip_base=${hip_base} ret=${retention_x}/${retention_z} host=$(hostname)"
  STAMP="${stamp}" \
  STEPS=3000 \
  BOX_MASS=2.0 \
  TARGET_SPEED=0.18 \
  ...
  STRIDE_VELOCITY_GAIN=0.18 \
  STRIDE_POSITION_GAIN=0.45 \
  STRIDE_CLIP=0.14 \
  ...
  SUPPORT_MAX_FOOT_FZ=260.0 \
  SUPPORT_MAX_JOINT_TORQUE=220.0 \
  ...
  RETENTION_KP_X="${retention_x}" \
  RETENTION_KD_X=36.0 \
  RETENTION_KP_Y=180.0 \
  RETENTION_KD_Y=28.0 \
  RETENTION_KP_Z="${retention_z}" \
  RETENTION_KD_Z=28.0 \
...
run_case brake_s006_hn004 0.06 -0.04 -1.0 260.0 120.0 0.03 360.0 260.0
run_case brake_s006_hn008 0.06 -0.08 -1.0 260.0 120.0 0.03 360.0 260.0
run_case brake_s004_hn006 0.04 -0.06 -1.0 220.0 100.0 0.03 360.0 260.0
run_case brake_scale05_s006 0.06 -0.04 -0.5 260.0 120.0 0.03 360.0 260.0
run_case brake_nohip_s006 0.06 -0.04 -1.0 260.0 120.0 0.00 360.0 260.0
run_case brake_scale05_nohip 0.06 -0.04 -0.5 260.0 120.0 0.00 360.0 260.0

echo "[DONE] MuJoCo free-box stance-force brake suite suffix=${SUITE_SUFFIX}"
