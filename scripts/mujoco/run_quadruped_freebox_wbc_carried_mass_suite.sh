...
SUITE_SUFFIX="${SUITE_SUFFIX:-wbc_carried_mass}"

cd "${ROOT_DIR}"

run_case() {
  local tag="$1"
  local support_scale="$2"
  local box_com_weight="$3"
  ...
  echo "[CASE] ${stamp} wbc_carried_mass scale=${support_scale} box_com_weight=${box_com_weight} height=${target_height} recovery=${recovery} host=$(hostname)"
  STAMP="${stamp}" \
  STEPS=3000 \
  BOX_MASS=2.0 \
  TARGET_SPEED=0.16 \
...
run_case wbc_fullbox  -0.55 1.0 0.56 0
run_case wbc_halfcom  -0.55 0.5 0.56 0
run_case wbc_highhold -0.55 1.0 0.60 0
run_case wbc_recovery -0.55 1.0 0.56 1

echo "[DONE] MuJoCo free-box WBC carried-mass support suite suffix=${SUITE_SUFFIX}"
