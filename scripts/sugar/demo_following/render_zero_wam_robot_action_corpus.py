...
def main() -> None:
    ...
    for frame_index in range(EXPECTED_FRAMES):
        ...
        for env_index, image in enumerate(rgb):
            if int(image.max()) == int(image.min()):
                raise RuntimeError(
                    f"constant RGB for {task}:{int(source_ids[env_index])}"
                )
            path = frame_dirs[env_index] / f"{frame_index:03d}.png"
    ...
    for env_index, source_id_value in enumerate(source_ids.tolist()):
        ...
        if len(paths) != EXPECTED_FRAMES:
            raise RuntimeError(f"incomplete rendered sequence: {directory}")
        record = {
            "task": task,
            ...
            "trace_environment_index": env_index,
        }
    ...
    render_result = {
        ...
        "total_rgb_frames": EXPECTED_FRAMES * envs,
        ...
        "neighbor_geometry_gate": {
            "conservative_camera_to_neighbor_center_lower_bound_m": (
                ISOLATED_ENV_SPACING_M - float(np.hypot(3.6, 3.6))
            ),
