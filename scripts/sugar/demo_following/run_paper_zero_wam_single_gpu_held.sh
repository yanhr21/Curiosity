#!/usr/bin/env bash
# One retained compute child; the parent tmux/srun shell keeps its allocation.
set -euo pipefail
...
flock -n 9
for stage in overfit formal; do
    "$PYTHON_BIN" "$IMPORT_BOOTSTRAP" torch.distributed.run \
        ...
        --mode "$stage" --output-dir "$EXPERIMENT/$stage"
done
"$PYTHON_BIN" "$IMPORT_BOOTSTRAP" torch.distributed.run \
    --standalone --nproc_per_node=1 "$IMPORT_BOOTSTRAP" \
    scripts.sugar.demo_following.paper_zero_wam.evaluate_heldout \
    --single-gpu --checkpoint "$EXPERIMENT/formal/latest_checkpoint" \
...
"$PYTHON_BIN" "$IMPORT_BOOTSTRAP" \
    scripts.sugar.demo_following.paper_zero_wam.advance_single_gpu \
    --formal-root "$EXPERIMENT/formal"
