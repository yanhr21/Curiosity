#!/usr/bin/env python3
"""Test frozen Carry45/Kick21 scoring on held-out official Kick policies.

The nine source motions ending in 9 are the motion-disjoint predictor test
split. Their exact 121-D online observations were generated by the released
official SUGAR KickBox Tracker in IsaacLab/PhysX. The primary result uses the
deployed fixed 650-step clock. A source-duration-normalized phase is reported
only as an evaluation diagnostic and never enters a deployed policy.
"""
...
def load_heldout_kick(corpus_root: Path) -> dict[str, np.ndarray]:
...
def main() -> None:
    ...
    result = {
        ...
        "checks": checks,
        "claim_scope": (
            "Motion-disjoint official-Tracker Kick-domain scorer transfer under the "
            "deployed fixed clock. This is an independent policy/data gate, but it "
            "does not establish Refiner-plus-residual Kick transfer or policy following."
        ),
        "device": str(device),
