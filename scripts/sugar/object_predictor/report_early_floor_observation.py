...
from .render_state_early_floor_observation import comparison_baseline, read
...
def report(root):
    ...
    late = Path(json.loads((root/'PROTOCOL.json').read_text())['late_baseline'])
    from .overfit_floor_observation import verify_artifacts as verify_late
    verify_late(late)
    result = json.loads((root/'RESULT.json').read_text())
    if not result['execution_complete']:
        raise ValueError('No complete actual floor endpoint')
    sources = [root/'RESULT.json', root/'PROTOCOL.json', baseline/'RESULT.json', late/'RESULT.json', late/'PROTOCOL.json']
    roles = {}
    ...
    text = ['# 地面高度观测：同预算实际对照', '',
            '三组均恢复同一原2000步完整Utonia和Adam，固定100次整批更新。对照分别为未补高度、末端23权重读出接入、原SensorAffine后54权重早期接入；同一当前左手距公开地面高度，无物体真值高度。早期与末端接入还存在参数数目及新参数学习率单位差异，不能只归因于位置。', '',
    ...
    for stem in ('fit_02100', 'same_trajectory_interpolation'):
        paths = [base/(stem+'.npz') for base in (baseline, late, root)]
        arrays = [read(path) for path in paths]
        sources.extend(paths)
        if any(not np.array_equal(arrays[0][key], a[key]) for a in arrays[1:] for key in label_fields):
        ...
        text += [f'## {stem}', '', '|条件|候选中心 cm|候选网格 cm|可用质量均值 %|可用质量p95 %|力 RMSE N|不可用误判 %|',
                 '|---|---:|---:|---:|---:|---:|---:|']
        for title, m in zip(('未补高度', '末端接入高度', '早期接入高度'), roles[stem]['all']):
    ...
    text += ['[实际渲染](index.html)：未补高度2100端点对比早期接入高度2100端点，均保留原错误、未知阶段与物理失败。视频完成与否以 renders/RESULT.json 为准。', '',
             '三组完整骨干均实际训练。该固定训练轨迹对照不能建立未见物体泛化、触觉净增益或所有误判的唯一原因。负结果不意味着高度信息无用。', '']
    record = dict(complete=True, model_forwards=0, model_updates=0, physics_controls=0,
                  matched_baseline=str(baseline), late_baseline=str(late), roles=roles, acceptance_passed=result['acceptance_passed'],
