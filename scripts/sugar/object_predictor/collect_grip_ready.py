"""Force-readiness qualification; original failed fixed-clock acquisition retained."""
...
def main(args):
    ...
    import warp as wp
    from .grip_ready_scene import GripReadyScene
    from .collect_newton import observation,hand_sites
    out=Path(args.output);out.mkdir(parents=True,exist_ok=False)
    protocol=dict(frames=1800,dt=.02,mass_kg=args.mass,target_load_n=args.load,seed=args.seed,scale=args.scale,geometry_group=args.geometry_group,approach_angle_deg=args.approach_angle_deg,yaw_delta_deg=args.yaw_delta_deg,lift_height_m=args.lift_height_m,lateral_xy=args.lateral_xy,
                  phases={'grip':'until both pads within25%target for1sec; earliest16s/latest28s','lift_translate_rotate':'4sec after ready','hold':'remaining time through36s'},
                  ...
                  source_sha256={n:hashlib.sha256(Path('scripts/sugar/object_predictor',n).read_bytes()).hexdigest()
                                 for n in ('collect_grip_ready.py','grip_ready_scene.py','grip_motion_scene.py','grip_scene.py','probe_scene.py','support_scene.py','collect_newton.py','geometry.py')})
    (out/'PROTOCOL.json').write_text(json.dumps(protocol,indent=2))
    wp.init();scene=GripReadyScene(mass=args.mass,scale=args.scale,seed=args.seed,target_load_n=args.load,approach_angle_deg=args.approach_angle_deg,yaw_delta_deg=args.yaw_delta_deg,lift_height_m=args.lift_height_m,lateral_xy=args.lateral_xy)
    ...
    try:
        for frame in range(1800):
            ...
            row={**obs,'object_pose_w':pose,'object_mass_kg':np.float32(mass),
                 ...
                 'validation_lift_start_time_s':np.float64(scene.probe_record['lift_start_time_s']),
                 'validation_ready_streak_s':np.float64(scene.probe_record['ready_streak_s']),
    ...
    hold=(a['validation_lift_start_time_s']>=0)&(a['timestamp_s']>=a['validation_lift_start_time_s']+5.);baseline=(a['timestamp_s']>=1.)&(a['timestamp_s']<=2.)
    ...
    hold_count=int(hold.sum());hold_available=hold_count>0
    if not hold_available:hold[-1]=True
    rise=float(np.median(center[hold,2])-np.median(center[baseline,2]))
    values=dict(hold_frames=hold_count,lift_start_time_s=float(a['validation_lift_start_time_s'][-1]),bilateral_fraction=float((loads[hold]>.01).all(1).mean()),clearance_fraction=float((a['validation_full_mesh_min_z_m'][hold]>.1).mean()),
    ...
    checks=dict(hold_at_least2seconds=hold_count>=100,bilateral=values['bilateral_fraction']>=.8,clearance=values['clearance_fraction']>=.8,rise=rise>=args.lift_height_m-.04,peak=values['peak_hand_load_n']<=100.,
                load_target=bool(np.max(np.abs(loads[hold].mean(0)/args.load-1))<=.25))
    report=dict(episode=args.episode,split=args.split,acquisition_group='grip',sampling_stride=25,frames=1800,mass_kg=mass,scale=args.scale,seed=args.seed,geometry_group=args.geometry_group,grip_target_per_hand_n=args.load,approach_angle_deg=args.approach_angle_deg,yaw_delta_deg=args.yaw_delta_deg,lift_height_m=args.lift_height_m,lateral_xy=args.lateral_xy,
                passed=all(checks.values()),checks=checks,values=values,physics_steps=1800,optimizer_updates=0,
