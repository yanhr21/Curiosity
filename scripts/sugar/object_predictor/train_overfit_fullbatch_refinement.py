"""Exactly100 full-gradient refinement updates of the original complete Utonia.
...
from . import overfit_fullbatch_refinement as repair


def save_evaluation(root,stem,model,dataset,vertices,device):
    report,arrays=original.evaluate(model,dataset,vertices,device)
...
def validate_prepared(source,root):
    ...
    if declared!=repair.source_protocol(source):raise ValueError('Prepared refinement protocol/source changed')
    return declared
...
def run(args):
    ...
    dataset=original.FailureAwareOverfitDataset(protocol['source_data'],protocol['data'])
    ...
    if args.check_data:
        ...
        return 0
    interpolation=original.FailureAwareOverfitDataset(protocol['source_data'],protocol['data'],
    ...
    restored=repair.verify_restored_optimizer(model,optimizer,repair.SOURCE_UPDATES)
    repair.split_readout_group(optimizer,model.predictor.head.parameters(),model.auxiliary.parameters())
    after_split=repair.verify_restored_optimizer(model,optimizer,repair.SOURCE_UPDATES)
    ...
    original.save_json(root/'RESTORED_OPTIMIZER.json',dict(before_split=restored,after_split=after_split,
    ...
    with (root/'train.jsonl').open('x',buffering=1) as log:
        for update in range(1,repair.NEW_UPDATES+1):
            ...
            for micro_index,indices in enumerate(batches,1):
                batch=original.device_batch(original.collate_conditional([dataset[index] for index in indices]),device)
                output=model(batch['inputs'])
                parts=original.conditional_task_losses(output,batch,vertices)
                loss=repair.scaled_microbatch_loss(parts)
    ...
    final_optimizer=repair.verify_restored_optimizer(model,optimizer,total)
    torch.save(dict(model=model.state_dict(),optimizer=optimizer.state_dict(),step=total,
                    new_optimizer_updates=repair.NEW_UPDATES,protocol=protocol),root/'model.pt')
    update_report=original.parameter_update_report(model,optimizer,initial,total)
    ...
    del initial
    batch=original.device_batch(original.collate_conditional([dataset[0]]),device)
    model.eval()
    with torch.no_grad():
