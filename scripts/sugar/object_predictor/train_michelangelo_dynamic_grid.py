"""Fixed1000 dynamic-grid sampling experiment, full released official AE.

prepare/check are CPU only. Root queues qualify (four full backwards, zero
updates) and then run (fresh official model, fresh Adam, seed reset, 1000 steps)
under the retained-resource guard. Original architecture, KLNearFar, AdamW,
posterior sampling, learning rate, GT-near sequence, evaluation and full-mesh
gates are reused unchanged. Only volume-query selection changes. No toy model,
mesh cleanup, checkpoint selection, sweep, continuation or automatic next run.
"""
...
from . import michelangelo_dynamic_grid_data as sampling
...
REVISION='dynamic_extraction_grid_v1'
GRID_CHUNK=10000
...
def refresh(model,queries,cases,device,output,step):
    ...
    print('DYNAMIC_GRID_REFRESH',step,[(r['object_id'],r['fp_pool_count'],r['fn_pool_count']) for r in records],flush=True)
    return pools,report
...
def qualify(args):
    ...
    for index,oid in enumerate(OBJECT_IDS):
        ...
        try:
            ...
            loss,components=original.batch_loss(model,criterion,item,indices,device)
            ...
            row.update(passed=True,loss=float(loss),loss_components={k:float(v) for k,v in components.items()},
                       gradients=grad,sampling=info,sampling_file=path.name,sampling_sha256=digest(path))
        except Exception:row['error']=traceback.format_exc()
        records.append(row);original.write_json(args.output/'PROGRESS.json',records)
        print('DYNAMIC_GRID_FULL_GRADIENT',oid,row.get('loss'),row['passed'],flush=True)
...
def check_qualification(path,bound,resource):
    ...
    if (report['scope']!=SCOPE+'_qualification' or not report['passed']
            ...
            or not all(x['passed'] and x['gradients']['passed'] for x in report['cases'])):
...
def run(args):
    ...
    protocol=dict(scope=SCOPE,sampling_revision=REVISION,bindings=bound,resource=resource,
                  ...
                  train_posterior='sample',refresh_posterior='mode',eval_posterior='mode',abc_to_vae_scale=ABC_TO_VAE,
        ...
        with (args.output/'updates.jsonl').open('x') as log:
            for step in range(1,1001):
                ...
                if step%100==0:
                    ...
                    print('DYNAMIC_GRID_UPDATE',step,row,flush=True)
    finally:
