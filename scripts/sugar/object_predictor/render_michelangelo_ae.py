"""Raw actual-mesh qualification renders for the four native Michelangelo cases.

No model/physics execution, smoothing, welding, fitted alignment, or mesh repair.
Three panels show full original mesh, the actual reconstructed mesh, and the
same reconstructed triangles as wireframe. A rotating camera is not a physical
...
from .qualify_michelangelo_ae import OBJECT_IDS, CHECKPOINT_SHA
...
def render(root):
    ...
    try:
        with imageio.get_writer(movie, fps=30, codec='libx264', quality=8,
            ...
            for row in result['cases']:
                ...
                meshes = [(vertices, faces), None, None]
                if saved.exists():
                ...
                records.append(dict(object_id=oid, reconstruction_available=meshes[1] is not None,
                                    video_start_s=frame_count/30, frames=96))
                for frame in range(96):
                    im = canvas('完整官方 Michelangelo AE · 原生四网格重建资格',
                                f'{oid} | 真值表面 XYZ+法向输入 / 冻结发布权重 / 0次更新 | 相机环绕，非物理 rollout',
                                ['真实完整网格', '原始 AE 重建网格', '同一重建：三角线框'])
                    ...
                    for panel, (scene, camera) in enumerate(zip(scenes, cameras)):
                        ...
                        if scene is None:
                            ...
                            render_flags = flags | (pyrender.RenderFlags.ALL_WIREFRAME if panel == 2 else 0)
                        ...
                        if panel:
                            text(draw, (x, 883), '数值资格：'+('PASS' if row['checks']['passed'] else 'FAIL'), 20)
                    text(draw, (24, 924),
                         '不是触觉预测、未知位姿/质量估计或训练成功。所有原始三角形保留；未平滑、焊接或对齐。逐面探针不保证连续面误差上界。', 16)
                    if frame in (0, 32, 64):
    ...
    report = dict(complete=True, checkpoint_sha256=CHECKPOINT_SHA, records=records,
                  ...
                  scope='Native full-surface frozen AE, camera turntable; not tactile prediction',
    ...
    html = ['<!doctype html><meta charset="utf-8"><title>Michelangelo 原生AE资格</title>',
            ...
            '<h1>完整官方 AE：全部四个网格</h1><p>冻结发布权重；完整真实表面输入；0训练。相机环绕，不是触觉搬运。</p>',
            '<video controls src="renders/native_ae_reconstruction.mp4"></video>']
    for name in stills:
