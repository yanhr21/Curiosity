...
def predictions(root,initial):
    paths=([root/'initial_fit.npz',root/'initial_same_trajectory_interpolation.npz'] if initial else
           [root/'fit_02000.npz',root/'same_trajectory_interpolation.npz'])
...
def render(root, shard_index=None):
    ...
    from .train_overfit import verify_artifacts
        ...
        with imageio.get_writer(movie,fps=fps,codec='libx264',quality=8,pixelformat='yuv420p',macro_block_size=1,**writer_options) as writer:
            for episode in episodes:
                ...
                for frame in frames:
                    ...
                    contexts=[]
                    labels=['实际物体与双手','完整模型初始预测','完整模型 overfit 终点']
                    ...
                    if conditional:
                        ...
                        labels=['实际物体与双手', '完整模型初始 · '+evidence_label(contexts[0])[0],
                                '2000步终点 · '+evidence_label(contexts[1])[0]]
                        title='受控训练轨迹 · 条件监督前后'
                    im=canvas(title,
...
def write_delivery(root,out,rendered_result):
    ...
    introduction='4倍回放，实际物理全网格；固定训练轨迹及未拟合时刻，不是未见物体泛化。质量可用性为上次估计的学习概率；数值输出完整保留。青色虚线仅是真值评价参考。画面误差包含估计保持延迟，区别于2Hz同刻验收指标。'
    page_title='修复后overfit rollout';heading='固定16例：实际搬运与overfit前后'
    if conditional:
        page_title='受控轨迹：条件监督rollout';heading='固定16例：真实物体、先验与条件估计'
...
def render_parallel(root):
    ...
    try:
        for index in range(4):
            ...
            command=[sys.executable,'-P','-m','scripts.sugar.object_predictor.render_state_overfit',
