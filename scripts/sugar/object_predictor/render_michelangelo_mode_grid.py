...
Three panels show truth, dynamic-grid sampled-posterior training and matched mode-posterior training.
...
def render(root):
    ...
    if (protocol.get('sampling_revision') != 'dynamic_extraction_grid_mode_training_v1'
    ...
    if (tuple(r['object_id'] for r in result['cases']) != OBJECT_IDS
            or result['scope'] != 'full_surface_native_ae_mode_grid_overfit_only'
    ...
    baseline = root.parent/'michelangelo_dynamic_grid_overfit_v1'
    old_result = json.loads((baseline/'RESULT.json').read_text())
    old_protocol = json.loads((baseline/'PROTOCOL.json').read_text())
    if (old_result['scope'] != 'full_surface_native_ae_dynamic_grid_overfit_only'
            or old_protocol.get('sampling_revision') != 'dynamic_extraction_grid_v1'
            or old_protocol.get('train_posterior') != 'sample'
        ...
        raise ValueError('Previous dynamic sampled-posterior full-model endpoint differs')
    ...
    movie = out/'native_ae_mode_grid_comparison.mp4'
            ...
            for row in result['cases']:
                ...
                for frame in range(96):
                    im = canvas('完整官方 Michelangelo AE · 确定性几何训练对照',
                                f'{oid} | 真值表面 XYZ+法向输入 / 两轮各自完整模型1000步 / 仅改变训练latent取样 | 相机环绕，非物理 rollout',
                                ['真实完整网格', '随机latent训练：1000步', '确定性latent训练：1000步'])
    ...
    html = ['<!doctype html><meta charset="utf-8"><title>Michelangelo 原生AE资格</title>',
            ...
            '<h1>完整官方 AE：全部四个网格</h1><p>完整真实表面输入；相同动态网格采样规则，随机latent与确定性latent训练对比；两轮均从官方权重重置并完整训练1000步。相机环绕，不是触觉搬运。</p>',
            '<video controls src="renders/native_ae_mode_grid_comparison.mp4"></video>']
    for name in stills:
