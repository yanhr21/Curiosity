...
from .qualified_cop_response import (INTERVENTION, DESCRIPTION, TELEMETRY,
    QualifiedCoPResponseController as PreliftCoPAlignmentController, register_intervention,
    QualifiedObservedForceGain, command_evidence, ROUND_OFF_TOL)
...
BASELINE = Path('experiments/object_predictor_v1/overfit_repair_v1/prelift_cop_alignment_pilot_v1')
RESOURCE = BASELINE.parent/'ACTIVE_RESOURCE.json'
MODULE = 'scripts.sugar.object_predictor.run_qualified_cop_response_pilot'
...
def protocol():
    ...
    return dict(study='qualified_cop_response_pilot_v1', controller_intervention=INTERVENTION,
        baseline_root=str(BASELINE), baseline_intervention='prelift_cop_alignment_v1',
...
def source_paths():
    names=('run_qualified_cop_response_pilot','qualified_cop_response','run_prelift_cop_pilot','prelift_cop_alignment','freeze_postlift_alignment',
...
def collect_one(root,episode):
    ...
    actual.update(intervention_baseline='prelift_cop_alignment_v1',
        ...
        changed_rules=['complete_pure_normal_window_secant_eligibility','qualified_upper_context_and_expiry_validity'],
...
def run(root):
    ...
    for config in declared['configurations']:
        ...
        print('QUALIFIED_COP_RESPONSE_CASE',episode,row['complete'],row['passed'],flush=True)
