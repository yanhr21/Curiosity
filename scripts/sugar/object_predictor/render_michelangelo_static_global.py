...
Three panels show truth, source persistent replay2832 and full-model static global continuation2848.
...
def render(root):
    ...
    audit = json.loads((root/'FULL_ADAM_READBACK.json').read_text())
    reload = json.loads((root/'RELOAD.json').read_text())
    if (audit != result['full_adam_audit'] or not audit['passed']
            or audit['adam_parameter_tensors'] != 307
            or audit['adam_parameter_elements'] != 184659585
            or audit['every_adam_step'] != 2848
            or not reload['passed'] or not reload['all_four_query_metrics_exact']
            or not reload['all_four_logits_and_latents_exact']
            or tuple(r['object_id'] for r in reload['cases']) != OBJECT_IDS):
        raise ValueError('Require full307Adam2848 and exact all-four endpoint reload')
    if (protocol.get('sampling_revision') != 'frozen_train_global_objective_v1'
            or protocol.get('train_posterior') != 'mode'):
        raise ValueError('Require the reviewed frozen global TRAIN objective protocol')
    if (tuple(r['object_id'] for r in result['cases']) != OBJECT_IDS
            or result['scope'] != 'full_surface_native_ae_static_global_continuation_only'
            or result['initial_checkpoint_sha256'] != CHECKPOINT_SHA
            or result['model_updates'] != 16
            or result['source_model_updates'] != 2832
            or result['total_model_updates'] != 2848
            or result['checkpoint_reset'] is not False
            or digest(root/'endpoint.pt') != result['checkpoint_sha256']):
        raise ValueError('Require actual full-model 2832-to2848 continuation and all four cases')
    baseline = root.parent/'michelangelo_persistent_replay_v1'
    old_result = json.loads((baseline/'RESULT.json').read_text())
    old_protocol = json.loads((baseline/'PROTOCOL.json').read_text())
    if (old_result['scope'] != 'full_surface_native_ae_persistent_replay_continuation_only'
            or old_protocol.get('sampling_revision') != 'persistent_grid_and_train_near_replay_v1'
            or old_protocol.get('train_posterior') != 'mode'
            or old_result['initial_checkpoint_sha256'] != CHECKPOINT_SHA
            or old_result['model_updates'] != 832
            or old_result['total_model_updates'] != 2832
            or old_result['checkpoint_sha256'] != digest(baseline/'endpoint.pt')
            or result['source_checkpoint_sha256'] != old_result['checkpoint_sha256']):
        raise ValueError('Source full-model persistent2832 endpoint differs')
    ...
    movie = out/'native_ae_static_global.mp4'
        ...
        with imageio.get_writer(movie, fps=30, codec='libx264', quality=8,
            ...
            for row in result['cases']:
                ...
                for frame in range(96):
                    im = canvas('完整官方 Michelangelo AE · 固定全局目标对照',
                                f'{oid} | 真值表面 XYZ+法向输入 / 完整模型2832步 → 保留Adam全局续训到2848步 / 冻结查询、四物体累积 | 相机环绕，非物理 rollout',
                                ['真实完整网格', '持续覆盖：2832步', '固定全局目标：2848步'])
    ...
    report = dict(complete=True, sampling_revision=protocol['sampling_revision'],
                  ...
                  checkpoint_sha256=result['checkpoint_sha256'], initial_checkpoint_sha256=CHECKPOINT_SHA, bindings=bindings, training_model_updates=16, source_model_updates=2832, total_model_updates=2848, records=records,
                  ...
                  scope='Native full-surface full-model static global TRAIN comparison, camera turntable; not tactile prediction',
    ...
    html = ['<!doctype html><meta charset="utf-8"><title>Michelangelo 原生AE资格</title>',
            ...
            '<h1>完整官方 AE：全部四个网格</h1><p>完整真实表面输入；保留2832步模型和完整Adam，用固定1e-5步长进行16次全局更新到2848；固定全部历史均匀/错误查询及TRAIN近表面集合，累积四物体后才更新。原官方loss、确定性latent与验收门槛不变；独立heldout不参与采样。不挑选中间最佳端点。相机环绕，不是触觉搬运。</p>',
            '<video controls src="renders/native_ae_static_global.mp4"></video>']
    for name in stills:
