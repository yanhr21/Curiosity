...
def render(root, shard_index=None):
        ...
        with imageio.get_writer(movie,fps=fps,codec='libx264',quality=8,pixelformat='yuv420p',macro_block_size=1,**writer_options) as writer:
            for episode in episodes:
                ...
                for frame in frames:
                    ...
                    contexts=[]
                    labels=['实际物体与双手','同预算：未补地面高度','同预算：补入地面高度']
                    title='固定轨迹 · 地面高度观测对照'
                    if conditional:
                        ...
                        labels=['实际物体与双手', '未补高度 · '+evidence_label(contexts[0])[0],
                                '补入高度 · '+evidence_label(contexts[1])[0]]
                        title='受控训练轨迹 · 地面高度观测对照'
                    im=canvas(title,
    ...
    rendered_result=dict(complete=True,frames=count,fps=fps,
        ...
        compared_arms='matched fullbatch100 without/with explicit observed floor-height branch',
        floor_observation_only=True, matched_baseline=str(comparison_baseline(root)),
...
def write_delivery(root,out,rendered_result):
    ...
    introduction='中列为未补高度的整批精修，右列补入当前左手相对公开地面的高度；两组均从同一2000步端点和完整Adam出发，固定100次新整批更新。4倍回放，实际物理全网格；固定训练轨迹及未拟合时刻，不是未见物体泛化。质量可用性为上次估计的学习概率；数值输出完整保留。青色虚线仅是真值评价参考。画面误差包含估计保持延迟，区别于2Hz同刻验收指标。'
    page_title='地面高度观测对照';heading='固定16例：实际搬运与高度观测对照'
    if conditional:
        page_title='受控轨迹：条件监督rollout';heading='固定16例：真实物体、未补与补入地面高度'
...
def render_parallel(root):
    ...
    try:
        for index in range(4):
            ...
            command=[sys.executable,'-P','-m','scripts.sugar.object_predictor.render_state_floor_observation',
