"""Cached-input r1 of the fixed20 complete official Utonia semantic pair.
...
from . import semantic_state_cached as study
...
def run(args):
    ...
    print('SEMANTIC_CACHED_FIT_LOAD_BEGIN',flush=True)
    dataset=study.load_dataset(p,'fit')
    ...
    print('SEMANTIC_CACHED_DEVELOPMENT_LOAD_BEGIN',flush=True)
    dense=study.load_dataset(p,'development_interpolation')
    ...
    if args.check_data:
        if torch.cuda.is_initialized():raise RuntimeError('CPU cached preflight initialized CUDA')
        import socket
        combined=dict(passed=True,rows=1904,fit_rows=464,development_rows=1440,
            fit=report,development=dict(conditions=dense_reports,observation_readback=dense_cache),
            encoder_calls=0,model_forwards=0,cuda_initialized=False,host=socket.gethostname(),
            all_loaded_array_and_row_bytes_exact=True,hash_tolerance_relaxed=False,rounding_applied=False)
        original.save_json(root/'CACHED_INPUT_CHECK.json',combined)
        print(json.dumps({k:v for k,v in combined.items() if k not in ('fit','development')}),flush=True)
        return 0
    if original.require_active_resource()!=resource:raise RuntimeError('Retained resource changed')
...
def main():
    ...
    if args.prepare:
        protocol=study.source_protocol(args.source,args.qualification,args.cache)
    ...
    try:
        ...
        if (str(args.source.resolve())!=p['source_endpoint'] or str(args.qualification.resolve())!=p['qualification']
                or str(args.cache.resolve())!=p['cache']):
