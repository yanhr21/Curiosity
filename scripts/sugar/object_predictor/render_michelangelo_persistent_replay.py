...
Three panels show truth, source lowstep2000 and full-model persistent replay continuation2832.
...
def render(root):
    ...
    if (audit != result['full_adam_audit'] or not audit['passed']
        ...
        raise ValueError('Require full307Adam2832 and exact all-four endpoint reload')
    if (protocol.get('sampling_revision') != 'persistent_grid_and_train_near_replay_v1'
    ...
    if (tuple(r['object_id'] for r in result['cases']) != OBJECT_IDS
            or result['scope'] != 'full_surface_native_ae_persistent_replay_continuation_only'
            or result['initial_checkpoint_sha256'] != CHECKPOINT_SHA
            or result['model_updates'] != 832
            or result['source_model_updates'] != 2000
            or result['total_model_updates'] != 2832
            or result['checkpoint_reset'] is not False
            or digest(root/'endpoint.pt') != result['checkpoint_sha256']):
        raise ValueError('Require actual full-model 2000-to2832 continuation and all four cases')
    baseline = root.parent/'michelangelo_lowstep_continuation_v1'
    old_result = json.loads((baseline/'RESULT.json').read_text())
    old_protocol = json.loads((baseline/'PROTOCOL.json').read_text())
    if (old_result['scope'] != 'full_surface_native_ae_lowstep_continuation_only'
            or old_protocol.get('sampling_revision') != 'dynamic_extraction_grid_mode_lowstep_continuation_v1'
            ...
            or old_result['total_model_updates'] != 2000
        ...
        raise ValueError('Source full-model lowstep2000 endpoint differs')
    ...
    movie = out/'native_ae_persistent_replay.mp4'
        ...
        with imageio.get_writer(movie, fps=30, codec='libx264', quality=8,
            ...
            for row in result['cases']:
                ...
                for frame in range(96):
                    im = canvas('完整官方 Michelangelo AE · 持续覆盖修复对照',
                                f'{oid} | 真值表面 XYZ+法向输入 / 完整模型2000步 → 保留Adam续训到2832步 / 持续训练查询覆盖 | 相机环绕，非物理 rollout',
                                ['真实完整网格', '原模型：2000步', '持续覆盖续训：2832步'])
    ...
    report = dict(complete=True, sampling_revision=protocol['sampling_revision'],
                  ...
                  checkpoint_sha256=result['checkpoint_sha256'], initial_checkpoint_sha256=CHECKPOINT_SHA, bindings=bindings, training_model_updates=832, source_model_updates=2000, total_model_updates=2832, records=records,
                  ...
                  scope='Native full-surface full-model persistent query replay comparison, camera turntable; not tactile prediction',
    ...
    html = ['<!doctype html><meta charset="utf-8"><title>Michelangelo 原生AE资格</title>',
            ...
            '<h1>完整官方 AE：全部四个网格</h1><p>完整真实表面输入；保留2000步模型和完整Adam，用固定1e-5步长继续832步到2832；保留均匀查询锚点，加入历史网格错误持续覆盖及TRAIN近表面困难查询。原官方loss、确定性latent与验收门槛不变；独立heldout不参与采样。不挑选中间最佳端点。相机环绕，不是触觉搬运。</p>',
            '<video controls src="renders/native_ae_persistent_replay.mp4"></video>']
    for name in stills:
