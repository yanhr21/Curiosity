"""Actual matched rollout: whole-command floor rejection versus per-hand CoP allocation.
...
def main(root):
    ...
    if protocol['controller_intervention'] != 'allocated_floor_cop_v1' or [r['episode'] for r in result['cases']] != [5014,5012,5000]:
    ...
    if baseline_protocol['controller_intervention']!='floor_safe_qualified_cop_v1':
        raise ValueError('Require immediate complete floor-safe baseline')
    if not json.loads((baseline/'RESULT.json').read_text())['complete']:
        raise ValueError('Incomplete floor-safe baseline')
    saved={}
    ...
    try:
        for case in result['cases']:
            ...
            for frame in frames+[2399]:
                ...
                text(draw,(16,12),f'{episode} · 安全接触调整：固定三例对照',28)
                text(draw,(16,53),'真实完整网格回放｜相同地面约束、载荷与截止｜按手保留安全调整｜无模型预测',19)
                timestamp=float(datasets[0]['timestamp_s'][frame])
                for panel,data in enumerate(datasets):
                    x=8+panel*544
                    text(draw,(x+8,91),'不安全时整步保持' if panel==0 else '按手保留安全接触调整',22)
                    ...
                    status=f'整步保持 {count} 次'
                    if panel:
                        changed=bool(data['validation_controller_cop_allocation_changed'][frame])
                        changed_count=int(data['validation_controller_cop_allocation_changed_count'][frame])
                        keep=data['validation_controller_cop_allocation_selected_mask'][frame]
                        action='保持' if blocked else ('左'+('保留' if keep[0] else '取消')+' / 右'+('保留' if keep[1] else '取消') if changed else '完整执行')
                        status=f'{action}｜分配 {changed_count} / 保持 {count}'
                    else:
                        status=('本步保持' if blocked else '完整执行')+f' / 累计保持 {count}'
                ...
                if frame%400==0:print('ALLOCATED_FLOOR_RENDER',episode,frame,flush=True)
    finally:
        renderer.delete()
    movie=out/'allocated_floor_rollout.mp4'
    ...
    html=['<!doctype html><html lang="zh"><meta charset="utf-8"><title>安全接触调整对照</title>',
          '<style>body{max-width:1200px;margin:28px auto;font:18px/1.7 system-ui;background:#f2f6f9}video,img{width:100%}</style>',
          '<h1>按手分配安全接触调整：三例实际回放</h1>',
          '<p>两侧都使用完整手网格地面约束。左侧不安全时保持整步；右侧在起升前尝试按手保留安全的接触调整，实际行程只累计执行部分，无安全组合则保持整步。相同初始朝向、载荷、每手行程、截止及物体门槛；两侧全部8子步全手检查。两侧都是真实物理，2倍速，保留全部失败，无模型预测。</p>',
          '<p><a href="RESULT.json">全部三例、原物体门槛与新增全手门槛</a></p>',
          '<video controls preload="metadata" src="renders/allocated_floor_rollout.mp4"></video>']
