...
from .shared_budget_cop import (INTERVENTION, DESCRIPTION, TELEMETRY,
    SharedBudgetCoPController as PreliftCoPAlignmentController, register_intervention, CLEARANCE_M, MASKS, add_cop)
...
BASELINE = Path('experiments/object_predictor_v1/overfit_repair_v1/allocated_floor_cop_pilot_v1')
RESOURCE = BASELINE.parent/'ACTIVE_RESOURCE.json'
MODULE = 'scripts.sugar.object_predictor.run_shared_budget_cop_pilot'
...
def protocol():
    ...
    retained={k:source[k] for k in keep}
    retained['cop_rule']=dict(source['cop_rule'],per_hand_accumulated_travel_max_m=None,
        original_per_hand_quota_m=.1,total_actual_accumulated_travel_max_m=.2,
        quota_transfer='Only original allocated transaction cancels the opposite nonzero CoP for floor and this hand was quota-truncated. Recompute safe4masks. Never increase200mm total or reset either cumulative travel.',
        original_cop_budget_exhausted_flag='Diagnostic original100mm quota only; not new total-budget exhaustion.')
    return dict(**retained,study='shared_budget_cop_pilot_v1',
        controller_intervention=INTERVENTION,baseline_root=str(BASELINE),
        baseline_intervention='allocated_floor_cop_v1',description=DESCRIPTION,known_hand_geometry=hand_geometry,
        ...
        shared_budget=dict(total_actual_m=.2,original_quota_per_hand_m=.1,
            narrow_trigger='Original allocated committed safe command cancels a nonzero donor CoP because its full proposed hand sweep is unsafe; opposite raw CoP is truncated by100mm quota; donor has unexecuted quota and shared balance positive.',
            preference='Keep original allocated outcome unless narrow-trigger additional recipient CoP is swept-safe and retained total displacement is not smaller. After any borrow every future combination obeys200mm actual total.',
            final_balance='Actual executed path lengths only; no resets/refunds. Recipient excess over100mm is cumulativeborrowed telemetry, not extra total allowance.',
            old_static_evidence='At old36.56s unused5.6mm versus local needed5.145636mm; .454364mm margin, worsening contact drift.168 eligible old clocks is not a new trajectory or success guarantee.',
            proposed_per_hand_110mm_not_executed=True),
        unchanged='Original configurations, clock2400/48s, 12checks, late2381/H32, CoP5deg/40s/.004mps, total200mm and original qualified gain formula. Only narrow fixed-per-hand quota transfer changes.',
        scope='Narrow conserved-budget transfer on completed allocated-floor baseline2/3; no static-counterfactual trajectory or grasp success claim. Original15/16 and all negative interventions retained.',
...
def source_paths():
    names=('run_shared_budget_cop_pilot','shared_budget_cop','shared_budget_cop_scene','collect_shared_budget_cop','render_shared_budget_cop_pilot','allocated_floor_cop','floor_safe_cop','hand_floor_feasibility','run_floor_safe_cop_pilot','run_qualified_cop_response_pilot','qualified_cop_response','run_prelift_cop_pilot','prelift_cop_alignment','freeze_postlift_alignment',
...
def collect_one(root,episode):
    ...
    from .collect_shared_budget_cop import main as collect
    ...
    actual.update(intervention_baseline='allocated_floor_cop_v1',
        ...
        hand_floor=declared['hand_floor'], allocation=declared['allocation'], shared_budget=declared['shared_budget'], changed_rules=['narrow_floor_cancelled_quota_transfer_with_conserved_total200mm'],
...
"""
    ...
    target=chosen['targets'][selected] if committed else previous
    if not np.array_equal(v('floor_executed_target_pose_w')[i],target):raise ValueError('Executed shared target differs')
    if not np.allclose(a['hand_pose_w'][i],target,rtol=0,atol=2e-7):raise ValueError('Actual hand differs from shared target')
    if committed:
        for actual,proposed in [('distance_m','floor_requested_distance_m'),('ready_seconds','floor_requested_ready_seconds'),
            ...
            ('actual_alignment_active','floor_requested_alignment_active')]:
            if not np.array_equal(v(actual)[i],v(proposed)[i]):raise ValueError('Committed parent state differs: '+actual)
    return chosen['heights'][selected] if committed else initial
