"""Actual matched rollout: shared CoP controller versus selective rotation coordination.
...
def main(root):
    ...
    if protocol['controller_intervention'] != 'coordinated_rotation_cop_v1' or [r['episode'] for r in result['cases']] != [5014,5012,5000]:
    ...
    if baseline_protocol['controller_intervention']!='shared_budget_cop_v1':
        raise ValueError('Require immediate complete shared-budget baseline')
    if not json.loads((baseline/'RESULT.json').read_text())['complete']:
        raise ValueError('Incomplete shared-budget baseline')
    saved={}
    ...
    try:
        for case in result['cases']:
            ...
            for frame in frames+[2399]:
                im=Image.new('RGB',(1088,1008),'#f2f6f9');draw=ImageDraw.Draw(im)
                text(draw,(16,12),f'{episode} · 旋转与平移协调：固定三例对照',28)
                text(draw,(16,53),'真实完整网格回放｜双手总行程上限仍200mm｜地面、载荷及截止不变｜无模型预测',19)
                timestamp=float(datasets[0]['timestamp_s'][frame])
                for panel,data in enumerate(datasets):
                    x=8+panel*544
                    text(draw,(x+8,91),'原共享行程控制' if panel==0 else '选择性暂停抵消纠偏的旋转',22)
                    ...
                    if panel:
                        flags=data['validation_controller_rotation_coord_selected_suppressed'][frame].astype(bool)
                        count=int(data['validation_controller_rotation_coord_suppressed_count'][frame])
                        text(draw,(x+8,898),f'本步暂停旋转 L/R {int(flags[0])}/{int(flags[1])} · 累计 {count} 次',18)
                    text(draw,(x+8,934),f'地面限制整步保持 {int(data["validation_controller_floor_blocked_count"][frame])} 次',18)
                text(draw,(16,976),'载荷/最低点：本步后；CoP错位/旋转决定：本步前观测。两侧均核全部8子步，失败保留。',16)
                if frame==2399:
                ...
                if frame%400==0:print('ROTATION_COORDINATION_RENDER',episode,frame,flush=True)
    finally:
        renderer.delete()
    movie=out/'coordinated_rotation_rollout.mp4'
    ...
    html=['<!doctype html><html lang="zh"><meta charset="utf-8"><title>旋转与平移协调对照</title>',
          '<style>body{max-width:1200px;margin:28px auto;font:18px/1.7 system-ui;background:#f2f6f9}video,img{width:100%}</style>',
          '<h1>选择性旋转协调：三例实际回放</h1>',
          '<p>两侧使用相同完整手网格地面约束、原力和5度起升门槛、40秒截止与4mm/s速度。两侧保留同样的双手共享200mm总行程。右侧仅在接触错位未达标且父控制器旋转会抵消平移纠偏时，选择性暂停相应侧旋转；力控制与原物理门槛保留。真实全部三例，2倍速，失败完整保留，无模型预测。</p>',
          '<p><a href="RESULT.json">全部三例、原物体门槛与新增全手门槛</a></p>',
          '<video controls preload="metadata" src="renders/coordinated_rotation_rollout.mp4"></video>']
