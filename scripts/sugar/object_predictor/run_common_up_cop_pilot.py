...
from .common_up_cop_alignment import (INTERVENTION, DESCRIPTION, TELEMETRY,
    CommonUpCoPController as PreliftCoPAlignmentController, register_intervention)
...
BASELINE = Path('experiments/object_predictor_v1/overfit_repair_v1/qualified_cop_response_pilot_v1')
RESOURCE = BASELINE.parent/'ACTIVE_RESOURCE.json'
MODULE = 'scripts.sugar.object_predictor.run_common_up_cop_pilot'
...
def protocol():
    ...
    return dict(study='common_up_cop_pilot_v1', controller_intervention=INTERVENTION,
        baseline_root=str(BASELINE), baseline_intervention='qualified_cop_response_v1',
        configurations=[next(c for c in source['configurations'] if c['episode']==e) for e in EPISODES],
        description=DESCRIPTION, initial_frame_rule='Both projected CAD+X tangents point world+Z; original support triangle centers z.14 and inward normals/world approach yaw unchanged. Full initial mesh geometry qualification required; not grasp success.', planned_cases=3, planned_controls=7200,
        ...
        response_gain='Two declared estimator repairs: complete5 pure-command intervals for secants; mixed intervals clear qualified slopes and upper requires >=5 qualified slopes within original2s window, otherwise upper0/candidate original base. Actual gain preserves original1.05 upward cap; zero qualified is fallback, not identification.',
        sample_rule=dict(roundoff_tolerance=ROUND_OFF_TOL, interval_count=5, observation_count=6, baseline_gain=.000025, unknown_initial_interval_rejected=True, no_GT=True, historical_stale_upper_rule_preserved=False, mixed_interval_clears_slopes=True, upper_requires_qualified_slopes=5, upper_window_s=2., insufficient_slopes_upper=0., candidate_fallback='original base', actual_upward_gain_cap=1.05),
...
def source_paths():
    names=('run_common_up_cop_pilot','common_up_cop_alignment','qualify_common_up_geometry','collect_common_up_cop','qualified_cop_response','run_prelift_cop_pilot','prelift_cop_alignment','freeze_postlift_alignment',
...
def prepare(root):
    ...
    from .qualify_common_up_geometry import qualify
    geometry=[qualify(c) for c in declared['configurations']]
    if not all(c['passed'] for c in geometry):raise ValueError('Initial full mesh geometry qualification failed')
    root.mkdir(parents=True,exist_ok=False)
    write(root/'INITIAL_GEOMETRY_CPU.json',dict(passed=True,cases=geometry,physics_controls=0,model_forwards=0))
    declared.update(planned_resource={k:resource[k] for k in ('job_id','step_id','host')},
        ...
        initial_geometry_cpu_sha256=sha(root/'INITIAL_GEOMETRY_CPU.json'))
...
def validate_protocol(root):
    ...
    if sha(root/'INITIAL_GEOMETRY_CPU.json')!=declared['initial_geometry_cpu_sha256']:raise ValueError('Static geometry receipt changed')
    geometry=json.loads((root/'INITIAL_GEOMETRY_CPU.json').read_text())
    if not geometry['passed'] or [c['episode'] for c in geometry['cases']]!=list(EPISODES):raise ValueError('Incomplete geometry qualification')
...
def collect_one(root,episode):
    ...
    from .collect_common_up_cop import main as collect
    ...
    actual.update(intervention_baseline='qualified_cop_response_v1',
        ...
        changed_rules=['initial_hand_tangent_roll_only'],initial_frame_rule=declared['initial_frame_rule'],
        unchanged_rules=['qualified_secant_window_and_upper_validity','cop_alignment_and_budget_and_deadline','base_and_candidate_gain_formula_and_caps','load_target','original_load_and_angle_bands',
...
def validate_case(folder,config,declared):
    ...
    initial=json.loads((folder/'INITIAL_GEOMETRY.json').read_text())
    if not initial['passed'] or initial['frame']!=0 or initial['time_s']!=0:raise ValueError('Actual initial geometry not qualified')
    from .qualify_common_up_geometry import initial_poses
    _,expected=initial_poses(config)
    if not np.allclose(initial['actual_hand_poses_w'],expected,rtol=0,atol=2e-7):raise ValueError('Actual common-up poses differ')
    initial_q=Rotation.from_quat(np.asarray(initial['actual_hand_poses_w'])[:,3:])
    first_q=Rotation.from_quat(a['validation_controller_response_command_target_quat_xyzw'][0])
    if ((first_q*initial_q.inv()).magnitude()>1e-7).any():raise ValueError('First command contains initial roll jump')
    ...
    return dict(complete=True,initial_geometry=initial,physical_passed=bool(result['passed']),physical_checks=result['checks'],
        response_readback=response,physical_values=result['values'],late_mass_available=bool(mass['mass_available']),
...
def run(root):
    ...
    for config in declared['configurations']:
        ...
        print('COMMON_UP_COP_CASE',episode,row['complete'],row['passed'],flush=True)
