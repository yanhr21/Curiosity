"""Early-floor controlled observation experiment: same100 full-gradient updates.
...
from . import overfit_early_floor_observation as repair
...
def validate_prepared(source,root,matched_baseline,late_baseline):
    declared=json.loads((Path(root)/'PROTOCOL.json').read_text())
    if declared!=repair.source_protocol(source,matched_baseline,late_baseline):raise ValueError('Prepared refinement protocol/source changed')
    return declared


def run(args):
    root=Path(args.output);source=Path(args.source).resolve()
    protocol=validate_prepared(source,root,args.matched_baseline,args.late_baseline)
    ...
    model=repair.EarlyFloorObservationOverfit(protocol['checkpoint'],history=32).to(device)
    ...
    if model.floor_embedding.out_features!=protocol['floor_observation']['embedding_width']:
        raise RuntimeError('Actual source stem width differs from prepared checkpoint')
    ...
    original.save_json(root/'RESTORED_OPTIMIZER.json',dict(before_split=restored,after_split=after_split,after_floor_embedding=added,
    ...
    if model.zero_stem_items!=1520 or model.zero_stem_calls!=1520:
        raise RuntimeError('Require all80+1440 actual original-stem zero-effect checks')
    original.save_json(root/'ZERO_STEM_REPLAY.json',dict(passed=True,fit_items=80,dense_items=1440,
        same_forward_original_stem_comparisons=model.zero_stem_items,stem_max_abs_difference=0.,
        point_rows_compared=model.zero_stem_points,final_output_pairs_directly_compared=False,
        source_checkpoint=protocol['source_bindings']['model.pt'],
        scope='Direct original SensorAffine output versus output plus zero for every initial item, followed by unchanged official downstream computation; no dual final-output or historical cross-process bitidentity claim'))
    ...
    with (root/'train.jsonl').open('x',buffering=1) as log:
        for update in range(1,repair.NEW_UPDATES+1):
            ...
            floor_before=model.floor_embedding.weight.detach().clone()
            floor_grad=model.floor_embedding.weight.grad
            if floor_grad is None or not bool(torch.isfinite(floor_grad).all()):
                raise RuntimeError('Missing/nonfinite floor embedding gradient')
            optimizer.step()
            floor_update=float((model.floor_embedding.weight.detach()-floor_before).norm())
            del floor_before
            ...
            record=dict(new_optimizer_update=update,total_optimizer_updates=repair.SOURCE_UPDATES+update,
                ...
                actual_floor_parameter_update_l2=floor_update,
                floor_parameter_l2=float(model.floor_embedding.weight.detach().norm()),
                floor_gradient_nonzero=bool((floor_grad!=0).any()),
                gradient_clip_scale=min(1.,100./(float(norm)+1e-6)),
    ...
    validate_prepared(source,root,args.matched_baseline,args.late_baseline)
    ...
    result=dict(execution_complete=True,study=repair.STUDY,steps=repair.NEW_UPDATES,
        ...
        floor_observation_only=True,early_floor_observation=True,
        zero_stem_all_initial_items_exact=True,initial_final_output_pairs_directly_compared=False,
        matched_baseline=protocol['matched_baseline'],late_baseline=protocol['late_baseline'],
...
def main():
    ...
    parser.add_argument('--late-baseline',type=Path,required=True)
    ...
    if args.prepare:
        ...
        original.save_json(args.output/'PROTOCOL.json',repair.source_protocol(args.source,args.matched_baseline,args.late_baseline))
        return
    try:code=run(args)
