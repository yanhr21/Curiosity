...
import numpy as np
from . import run_shared_budget_cop_pilot as pilot
...
def controller_summary(a,folder):
    ...
    changed=v('cop_allocation_changed').astype(bool)
    attempted=v('cop_allocation_attempted').astype(bool)
    ...
    for mask in (changed,attempted,(v('cop_travel_m')>=.10-1e-12).any(1),v('shared_borrow_applied').any(1),v('shared_borrow_attempted'),v('shared_total_travel_m')>=.2-1e-12,v('lift_start_s')>=0):
        ix=np.flatnonzero(mask)
        if len(ix):
            first,last=int(ix[0]),int(ix[-1])
            diagnostic_indices.update((max(0,first-1),first,min(2399,first+1),min(2399,first+10),last))
    ...
    for i in sorted(diagnostic_indices):
        events.append(dict(frame=i,time_s=float(a['timestamp_s'][i]),blocked=bool(blocked[i]),
            allocation_changed=bool(changed[i]),selected_mask=v('cop_allocation_selected_mask')[i].tolist(),
            original_proposed_step_m=v('shared_original_proposed_step_m')[i].tolist(),
            actual_candidate_step_m=v('cop_allocation_proposed_step_m')[i].tolist(),
            shared_total_m=float(v('shared_total_travel_m')[i]),
            borrowed_m=v('shared_borrowed_travel_m')[i].tolist(),
            borrow_eligible=v('shared_borrow_eligible')[i].tolist(),
            borrow_applied=v('shared_borrow_applied')[i].tolist(),
            actual_travel_before_m=(v('cop_travel_m')[i-1] if i else np.zeros(2)).tolist(),
            actual_travel_after_m=v('cop_travel_m')[i].tolist(),
            alternative_safe=v('cop_allocation_safe')[i].tolist(),
            alternative_swept_min_z_m=v('cop_allocation_swept_min_z_m')[i].tolist(),
    ...
    borrowing=v('shared_borrow_applied').any(1)
    return dict(borrow_attempts=int(v('shared_borrow_attempted').sum()),borrow_applied_controls=int(borrowing.sum()),
        first_borrow_s=float(a['timestamp_s'][np.flatnonzero(borrowing)[0]]) if borrowing.any() else None,
        last_borrow_s=float(a['timestamp_s'][np.flatnonzero(borrowing)[-1]]) if borrowing.any() else None,
        total_travel_m=float(v('shared_total_travel_m')[-1]),maximum_total_travel_m=float(v('shared_total_travel_m').max()),
        borrowed_travel_m=v('shared_borrowed_travel_m')[-1].tolist(),
        lift_start_s=float(v('lift_start_s')[-1]),lift_complete_s=float(v('lift_complete_s')[-1]),
        maximum_prelift_ready_s=float(v('ready_seconds')[v('lift_start_s')<0].max(initial=0)),
        allocation_attempts=int(attempted.sum()),partial_commands=int(changed.sum()),
        changed_with_nonzero_retained_cop=int(np.count_nonzero(changed & (np.linalg.norm(v('cop_servo_velocity_m_s'),axis=(1,2))>0))),
        changed_parent_only=int(np.count_nonzero(changed & (np.linalg.norm(v('cop_servo_velocity_m_s'),axis=(1,2))==0))),
        first_partial_s=float(a['timestamp_s'][np.flatnonzero(changed)[0]]) if changed.any() else None,
        last_partial_s=float(a['timestamp_s'][np.flatnonzero(changed)[-1]]) if changed.any() else None,
        selected_mask_counts={str(k):int(np.count_nonzero((v('cop_allocation_selected_mask')==k).all(1)&attempted)) for k in ([1,1],[1,0],[0,1],[0,0])},
        first_budget_exhausted_s=[float(a['timestamp_s'][ix[0]]) if len(ix) else None for ix in [np.flatnonzero(v('cop_travel_m')[:,side]>=.10-1e-12) for side in (0,1)]],
        blocked_commands=int(blocked.sum()),blocked_wall_time_s=float(blocked.sum()*.02),
...
def run(root,output):
    ...
    for config,record in zip(declared['configurations'],actual['cases'],strict=True):
        ...
        for key in ('floor_blocked','floor_blocked_count','floor_requested_swept_min_z_m','floor_executed_swept_min_z_m',
                    'shared_total_travel_m','shared_borrowed_travel_m','shared_borrow_applied','shared_borrow_eligible','shared_floor_donor','shared_quota_limited','cop_line_angle_deg','fit_valid','alignment_error_deg','lift_start_s','lift_complete_s','cop_allocation_attempted','cop_allocation_changed','cop_allocation_selected_mask','cop_allocation_proposed_step_m','cop_travel_m','response_command_pure','response_gain_m_per_ns','response_upper_n_m','response_samples','motion_elapsed_s','ready_seconds'):
    ...
    np.savez_compressed(output/'ACTUAL_ALLOCATION_TRANSACTION_READBACK.npz',**arrays)
    summary=dict(complete=True,actual_cases=3,controls_per_case=2400,substeps_per_case=19200,
        ...
        limits='Accepted geometry and estimator replay certify implementation consistency, not successful grasp. All three outcomes remain; no automatic16, changed force/clock/gates or new interventions.')
    pilot.write(output/'RESULT.json',summary)
    lines=['# Conserved shared CoP quota: actual fixed-three readback','',
        '|Case|Original12|Late mass|Actual8substep floor|Joint|Partial controls|Blocked controls|First partial s|First block s|',
        '|---|---|---|---|---|---|---|---|---|']
    for r in rows:
        c=r['controller'];v=r['revalidated']
        lines.append(f"|{r['episode']}|{v['physical_passed']}|{v['late_mass_available']}|{v['hand_floor_passed']}|{v['passed']}|{c['partial_commands']}|{c['blocked_commands']}|{c['first_partial_s']}|{c['first_blocked_s']}|")
    lines+=['',f"Original12 {summary['original12_passes']}/3; full-hand floor {summary['full_hand_substep_floor_passes']}/3; joint {summary['joint_passes']}/3.",
        '',summary['limits'],'','|Case|Borrow controls|Borrowed L/R mm|Total mm|Lift start/complete s|Max prelift ready s|',
        '|---|---|---|---|---|---|']
    for r in rows:
        c=r['controller']
        lines.append(f"|{r['episode']}|{c['borrow_applied_controls']}|{[1000*x for x in c['borrowed_travel_m']]}|{1000*c['total_travel_m']:.9f}|{c['lift_start_s']}/{c['lift_complete_s']}|{c['maximum_prelift_ready_s']}|")
    lines+=['','Readback uses all 2400 control transactions and all 19200 actual FK/solver hand poses per case. Observed load rows follow original previous-observation timing. Video/frame inspection is separate.']
