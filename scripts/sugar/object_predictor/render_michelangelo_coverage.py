...
Three panels show truth, original finite-pool overfit and revised volume-sampling overfit.
...
def render(root):
    ...
    if (protocol.get('sampling_revision') != 2
            or protocol.get('batch_surface_composition') != dict(
                centers=256, negative_offsets=128, positive_offsets=128)):
        raise ValueError('Require the reviewed center-plus-offset sampling revision 2')
    if (tuple(r['object_id'] for r in result['cases']) != OBJECT_IDS
            or result['scope'] != 'full_surface_native_ae_coverage_overfit_only'
    ...
    baseline = root.parent/'michelangelo_native_overfit_v1'
    old_result = json.loads((baseline/'RESULT.json').read_text())
    if (old_result['initial_checkpoint_sha256'] != CHECKPOINT_SHA
            or old_result['model_updates'] != 1000
            or old_result['checkpoint_sha256'] != digest(baseline/'endpoint.pt')):
        raise ValueError('Original finite-pool full-model endpoint differs')
    ...
    movie = out/'native_ae_coverage_comparison.mp4'
        ...
        with imageio.get_writer(movie, fps=30, codec='libx264', quality=8,
            ...
            for row in result['cases']:
                ...
                for frame in range(96):
                    im = canvas('完整官方 Michelangelo AE · 固定四物体体积监督采样修复',
                                f'{oid} | 真值表面 XYZ+法向输入 / 两轮各自完整模型1000步 / 仅改变volume采样 | 相机环绕，非物理 rollout',
                                ['真实完整网格', '原体积采样：1000步', '修复体积采样：1000步'])
    ...
    report = dict(complete=True, sampling_revision=protocol['sampling_revision'],
                  batch_surface_composition=protocol['batch_surface_composition'],
    ...
    html = ['<!doctype html><meta charset="utf-8"><title>Michelangelo 原生AE资格</title>',
            ...
            '<h1>完整官方 AE：全部四个网格</h1><p>完整真实表面输入；原体积采样与修复采样对比；两轮均从官方权重重置并完整训练1000步。相机环绕，不是触觉搬运。</p>',
            '<video controls src="renders/native_ae_coverage_comparison.mp4"></video>']
    for name in stills:
