...
from . import run_qualified_cop_response_pilot as pilot
...
def run(root,output):
    ...
    if baseline['qualification_passed'] or any(c['physical_passed'] for c in baseline['cases']):
        raise ValueError('Declared completed0/3 CoP baseline changed')
    ...
    report=dict(complete=True,planned_cases=3,actual_cases=3,controls_per_case=2400,
        baseline_physical_passes=0,new_physical_passes=sum(r['candidate']['physical_passed'] for r in rows),
        ...
        all_original12_and_late_decisions_preserved=True,model_forwards=0,physics_controls=0,optimizer_updates=0,
        source_sha256={str(p):pilot.sha(p) for p in (Path(__file__),
            Path(__file__).with_name('analyze_prelift_cop_pilot.py'),root/'PROTOCOL.json',root/'RESULT.json',
            pilot.BASELINE/'PROTOCOL.json',pilot.BASELINE/'RESULT.json')},
        limits='Two estimator repairs, not an isolated filter-only ablation. No automatic16 qualification, model training, force-target changes, timeout relaxation or deleted failed cases. The original15/16 corpus and blind-control failures remain separate.')
    pilot.write(output/'RESULT.json',report)
    lines=['# Qualified response: actual three-case comparison','',
    ...
    lines+=['',f"Physical {report['new_physical_passes']}/3; physical plus fixed late mass {report['new_physical_and_late_passes']}/3. All three complete cases retained.",'',
        'All2400 gain/upper/sample values and command-window flags were replayed from actual previous measurements. All12 original physical decisions and fixed frame2381/H32 mass availability were recomputed. Actual command rotation is replayed for2399 saved input rows; the initial unsaved input remains explicitly outside that geometry replay.',
