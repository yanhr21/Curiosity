...
import numpy as np
from . import run_allocated_floor_cop_pilot as pilot
...
def controller_summary(a,folder):
    ...
    for mask in (changed,attempted,(v('cop_travel_m')>=.10-1e-12).any(1)):
    ...
    for i in sorted(diagnostic_indices):
        events.append(dict(frame=i,time_s=float(a['timestamp_s'][i]),blocked=bool(blocked[i]),
            ...
            original_proposed_step_m=v('cop_allocation_proposed_step_m')[i].tolist(),
    ...
    return dict(allocation_attempts=int(attempted.sum()),partial_commands=int(changed.sum()),
...
def run(root,output):
    ...
    for config,record in zip(declared['configurations'],actual['cases'],strict=True):
        ...
        for key in ('floor_blocked','floor_blocked_count','floor_requested_swept_min_z_m','floor_executed_swept_min_z_m',
                    'cop_allocation_attempted','cop_allocation_changed','cop_allocation_selected_mask','cop_allocation_proposed_step_m','cop_travel_m','response_command_pure','response_gain_m_per_ns','response_upper_n_m','response_samples','motion_elapsed_s','ready_seconds'):
    ...
    summary=dict(complete=True,actual_cases=3,controls_per_case=2400,substeps_per_case=19200,
        ...
        limits='Accepted geometry and estimator replay certify implementation consistency, not successful grasp. All three outcomes, including the failed case, remain; no automatic16, changed force/clock/gates or new interventions.')
    pilot.write(output/'RESULT.json',summary)
    lines=['# Per-hand CoP allocation: actual fixed-three readback','',
