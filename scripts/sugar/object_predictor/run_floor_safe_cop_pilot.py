...
from .floor_safe_cop import (INTERVENTION, DESCRIPTION, TELEMETRY,
    FloorSafeCoPController as PreliftCoPAlignmentController, register_intervention, CLEARANCE_M)
...
MODULE = 'scripts.sugar.object_predictor.run_floor_safe_cop_pilot'
...
def protocol():
    ...
    return dict(**{k:source[k] for k in keep},study='floor_safe_cop_pilot_v1',
        controller_intervention=INTERVENTION,baseline_root=str(BASELINE),
        baseline_intervention='qualified_cop_response_v1',description=DESCRIPTION,known_hand_geometry=hand_geometry,
        hand_floor=dict(public_floor_z_m=0.,reserved_clearance_m=CLEARANCE_M,
            ...
            execution='Full accept or full reject. No partial progress. Same original initial hand orientation.',
            ...
            response='Original update exactly once on real current observation and prior executed distance/command evidence; keep this observed state, replace next incoming_pure from actual hold.',
        ...
        scope='Known-hand floor constraint on qualified-CoP baseline; not a demonstrated grasp repair or learned model.',
...
def source_paths():
    names=('run_floor_safe_cop_pilot','floor_safe_cop','floor_safe_cop_scene','hand_floor_feasibility','collect_floor_safe_cop','render_floor_safe_cop_pilot','run_qualified_cop_response_pilot','qualified_cop_response','run_prelift_cop_pilot','prelift_cop_alignment','freeze_postlift_alignment',
...
def collect_one(root,episode):
    ...
    from .collect_floor_safe_cop import main as collect
    ...
    actual.update(intervention_baseline='qualified_cop_response_v1',
        ...
        motion_progress='Original4s path after original+CoP admission; unsafe full commands rejected with actual phase frozen. No deadline extension.',
        ...
        hand_floor=declared['hand_floor'], changed_rules=['known_full_hand_swept_floor_transaction'],
...
def floor_readback(a,folder):
    ...
    if not np.array_equal(v('floor_blocked_count'),np.cumsum(blocked)) or not np.array_equal(v('floor_executed_fraction'),(~blocked).astype(float)):
        raise ValueError('Floor transaction counts/fraction differ')
    ...
    for i in range(2400):
        ...
        if blocked[i]!=bool((requested<CLEARANCE_M).any()):raise ValueError('Floor decision differs from actual complete sweep')
        expected=initial if blocked[i] else requested
...
def run(root):
    ...
    for config in declared['configurations']:
        ...
        print('FLOOR_SAFE_COP_CASE',episode,row['complete'],row['passed'],flush=True)
