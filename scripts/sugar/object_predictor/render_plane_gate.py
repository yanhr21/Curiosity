"""Matched full-duration original-mesh replay of the two plane-gate cases."""
import os
...
def main(root):
    ...
    result=json.loads((root/'COMPARISON.json').read_text()); assert result['complete']
    ...
    try:
        for case in result['cases']:
            ...
            for frame in frames+[2399]:
                ...
                text(draw,(16,12),f'{episode} · 接触面准入：固定单因素对照',28)
                ...
                for panel,data in enumerate(datasets):
                    x=8+panel*544
                    text(draw,(x+8,91),'原平面载荷准入 2.0 N' if panel==0 else '候选平面载荷准入 0.2 N',22)
                ...
                if frame%400==0:print('PLANE_GATE_RENDER',episode,frame,flush=True)
    finally:
        renderer.delete()
    movie=out/'plane_gate_comparison.mp4'
    ...
    html=['<!doctype html><html lang="zh"><meta charset="utf-8"><title>平面准入对照</title>',
          '<style>body{max-width:1200px;margin:28px auto;font:18px/1.7 system-ui;background:#f2f6f9}video,img{width:100%}</style>',
          '<h1>接触面准入：固定两例完整回放</h1>',
          '<p>只改变平面拟合载荷准入；左右均为实际物理轨迹，2倍速。所有成功/失败结果保留，无模型预测。</p>',
          '<p><a href="REPORT.md">报告</a> · <a href="COMPARISON.json">全部结果</a></p>',
          '<video controls preload="metadata" src="renders/plane_gate_comparison.mp4"></video>']
