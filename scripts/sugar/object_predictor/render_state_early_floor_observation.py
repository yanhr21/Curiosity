...
def comparison_baseline(root):
    from .overfit_fullbatch_refinement import verify_artifacts
    protocol=json.loads((root/'PROTOCOL.json').read_text())
    baseline=Path(protocol['matched_baseline'])
    verify_artifacts(baseline)
    other=json.loads((baseline/'PROTOCOL.json').read_text())
    for key in ('source_bindings','source_data','loss_scales','loss_weights',
                'fit_limits','interpolation_limits','steps','effective_batch',
                'microbatches_per_update','learning_rate_schedule'):
        if protocol[key]!=other[key]:raise ValueError('Matched baseline differs: '+key)
    if any(protocol['learning_rates'].get(key)!=value
           for key,value in other['learning_rates'].items()):
        raise ValueError('Original parameter learning rates differ from baseline')
    ...
    if (result.get('execution_complete') is not True or result.get('optimizer_updates')!=100
        ...
        raise ValueError('Require complete matched100 baseline endpoint')
    return baseline


def predictions(root,initial):
    # Both displayed arms have 2100 original-parameter updates. The floor run's
    # own source2000 initial predictions are checked by training, not displayed.
    source=comparison_baseline(root) if initial else root
    parts=[read(source/name) for name in ('fit_02100.npz','same_trajectory_interpolation.npz')]
...
def render(root, shard_index=None):
    ...
    from .overfit_early_floor_observation import verify_artifacts
        ...
        with imageio.get_writer(movie,fps=fps,codec='libx264',quality=8,pixelformat='yuv420p',macro_block_size=1,**writer_options) as writer:
            for episode in episodes:
                ...
                for frame in frames:
                    ...
                    contexts=[]
                    labels=['实际物体与双手','同预算：未补地面高度','同预算：早期接入地面高度']
                    title='固定轨迹 · 早期地面高度观测对照'
                    if conditional:
                        ...
                        labels=['实际物体与双手', '未补高度 · '+evidence_label(contexts[0])[0],
                                '早期接入高度 · '+evidence_label(contexts[1])[0]]
                        title='受控训练轨迹 · 早期地面高度观测对照'
                    im=canvas(title,
    ...
    rendered_result=dict(complete=True,frames=count,fps=fps,
        ...
        before_optimizer_updates=2100, after_optimizer_updates=2100,
        compared_arms='matched fullbatch100 baseline versus early SensorAffine floor-height insertion',
        floor_observation_only=True, floor_insertion_location='original SensorAffine output before official LayerNorm', matched_baseline=str(comparison_baseline(root)),
...
def write_delivery(root,out,rendered_result):
    ...
    introduction='中列为未补高度的整批精修，右列在原SensorAffine输出处接入当前左手相对公开地面的高度；新支路为54个零初始化权重，后续仍经过完整官方骨干；两组均从同一2000步端点和完整Adam出发，固定100次新整批更新。4倍回放，实际物理全网格；固定训练轨迹及未拟合时刻，不是未见物体泛化。质量可用性为上次估计的学习概率；数值输出完整保留。青色虚线仅是真值评价参考。画面误差包含估计保持延迟，区别于2Hz同刻验收指标。'
    page_title='早期地面高度观测对照';heading='固定16例：实际搬运与高度观测对照'
    if conditional:
        page_title='受控轨迹：条件监督rollout';heading='固定16例：真实物体、未补与早期接入地面高度'
...
def render_parallel(root):
    ...
    try:
        for index in range(4):
            ...
            command=[sys.executable,'-P','-m','scripts.sugar.object_predictor.render_state_early_floor_observation',
