"""Actual full-mesh before/after renders for four native Michelangelo overfit cases.

No model/physics execution, smoothing, welding, fitted alignment, or mesh repair.
Three panels show truth, frozen official reconstruction and full-model overfit.
...
def render(root):
    ...
    if (tuple(r['object_id'] for r in result['cases']) != OBJECT_IDS
            or result['scope'] != 'full_surface_native_ae_overfit_only'
    ...
    baseline = root.parent/'michelangelo_native_ae_v1_r1'
    old_result = json.loads((baseline/'RESULT.json').read_text())
    if old_result['checkpoint_sha256'] != CHECKPOINT_SHA:
        raise ValueError('Frozen comparison checkpoint differs')
    corrected_readback = json.loads((root.parent/'michelangelo_grid_coordinate_readback/RESULT.json').read_text())
    old_rows = {r['object_id']:r for r in corrected_readback['cases']}
    bindings = {}
    for oid in OBJECT_IDS:
        old_path=baseline/f'{oid}.npz'
        if digest(old_path) != old_rows[oid]['source_sha256']:
            raise ValueError('Frozen geometry/metric source changed')
        bindings[oid]=dict(frozen_sha256=digest(old_path), endpoint_case_sha256=digest(root/f'{oid}.npz') if (root/f'{oid}.npz').exists() else None)
    out = root/'renders'; out.mkdir(exist_ok=False)
    device = configure_retained_egl()
    renderer = pyrender.OffscreenRenderer(PW, PH)
    movie = out/'native_ae_overfit_comparison.mp4'
        ...
        with imageio.get_writer(movie, fps=30, codec='libx264', quality=8,
            ...
            for row in result['cases']:
                ...
                with np.load(baseline/f'{oid}.npz',allow_pickle=False) as old:
                    ...
                    frozen=(correct_extracted_coordinates(old['reconstruction_vertices_vae'])/ABC_TO_VAE,old['reconstruction_faces'])
                ...
                for frame in range(96):
                    im = canvas('完整官方 Michelangelo AE · 固定四物体微调前后',
                                f'{oid} | 真值表面 XYZ+法向输入 / 完整模型 1000 次更新 / 几何专用微调 | 相机环绕，非物理 rollout',
                                ['真实完整网格', '官方冻结权重', '完整模型微调后'])
                    ...
                    for panel, (scene, camera) in enumerate(zip(scenes, cameras)):
                        ...
                        if panel:
                            text(draw, (x, 883), '数值资格：'+('PASS' if panel==2 and row['checks']['passed'] else 'FAIL'), 20)
                    text(draw, (24, 924),
    ...
    report = dict(complete=True, checkpoint_sha256=result['checkpoint_sha256'], initial_checkpoint_sha256=CHECKPOINT_SHA, bindings=bindings, training_model_updates=1000, records=records,
                  ...
                  scope='Native full-surface full-model overfit comparison, camera turntable; not tactile prediction',
    ...
    html = ['<!doctype html><meta charset="utf-8"><title>Michelangelo 原生AE资格</title>',
            ...
            '<h1>完整官方 AE：全部四个网格</h1><p>完整真实表面输入；官方冻结权重与完整模型1000步微调对比。相机环绕，不是触觉搬运。</p>',
            '<video controls src="renders/native_ae_overfit_comparison.mp4"></video>']
    for name in stills:
