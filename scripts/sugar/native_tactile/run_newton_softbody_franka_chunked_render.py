#!/usr/bin/env python3
"""Render the Newton Franka/soft-duck tactile run in short EGL workers."""
...
def _pad_raw(array: np.ndarray, width: int, value: int) -> np.ndarray:
    pad_width = [(0, 0), (0, width - array.shape[1])] + [(0, 0)] * (array.ndim - 2)
...
def main() -> None:
    ...
    parser.add_argument("--frame-stop", type=int, default=720)
    ...
    parser.add_argument(
        "--worker",
        type=Path,
        default=Path(__file__).with_name("run_newton_softbody_franka_tactile.py"),
    )
    ...
    maximum_raw = max(part["raw_contact_kind"].shape[1] for part in trace_parts)
    trace: dict[str, np.ndarray] = {}
    for key in trace_parts[0]:
        if key == "patch_names":
            trace[key] = trace_parts[0][key]
        elif key in RAW_PAD_VALUES:
            trace[key] = np.concatenate(
                [_pad_raw(part[key], maximum_raw, RAW_PAD_VALUES[key]) for part in trace_parts], axis=0
            )
        else:
    ...
    summary["contact_frames_per_patch"] = np.sum(
        np.asarray([item["contact_frames_per_patch"] for item in summaries], dtype=np.int64), axis=0
    ).tolist()
    summary["frames_with_any_finger_contact"] = int(np.count_nonzero(trace["raw_count"]))
    ...
    concat_file.write_text(
        "".join(
            f"file '{(chunk_dir / 'newton_softbody_franka_native_tactile.mp4').resolve()}'\n"
            for chunk_dir in chunk_dirs
        ),
        encoding="utf-8",
    )
