...
# sugar_newton.validation.allegro_tactile.
#
...
source "$NT/renders/render_env_egl.sh"   # hardware GL; no Xvfb, so jobs do not fight over a display
...
while IFS= read -r cfg; do
  ...
  echo "GPU $i <- $cfg"
  CUDA_VISIBLE_DEVICES=$i nohup uv run python -m ${SN_MODULE:-sugar_newton.validation.allegro_tactile} $cfg \
