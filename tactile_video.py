...
# Object materials: kh ~ stiffness (force/penetration), mu friction, density [kg/m^3].
# Models contact-mechanics (stiff vs compliant, grippy vs slippery, heavy vs light);
# not microtexture/thermal. kh ~ scaled Young's modulus for the hydroelastic contact.
# Hard (rigid) materials: SAME rigid kh — stiffness doesn't differentiate rigid
# contact; they differ only by friction (mu), weight (density), and restitution.
# Genuinely soft/deformable materials are handled by the FEM scene (Young's modulus),
# not here. RIGID_KH is set well above the pad so the contact is pad-limited (rigid).
RIGID_KH = 1.0e12
MATERIALS = {
    "wood": dict(kh=RIGID_KH, mu=0.5, restitution=0.25, density=600.0),
    "metal": dict(kh=RIGID_KH, mu=0.3, restitution=0.4, density=7800.0),
    "porcelain": dict(kh=RIGID_KH, mu=0.2, restitution=0.6, density=2400.0),
    "glass": dict(kh=RIGID_KH, mu=0.2, restitution=0.5, density=2500.0),
    # somewhat-soft material: lower kh -> compliant contact (broader, lower-pressure patch)
    "rubber": dict(kh=1.0e10, mu=1.0, restitution=0.3, density=1100.0),
}
...
def install_push(ex, base, rate):
    ...
    def push():
        ...
        wp.launch(
            broadcast_ik_solution_kernel, dim=ex.world_count, inputs=[ex.joint_q_ik, ex.joint_targets_2d, 0.0]
...
def main():
    ...
    parser.add_argument(
        "--push-after", type=int, default=None, help="after this frame, hold grasp and push the gripper straight down"
    )
    parser.add_argument("--push-frames", type=int, default=100)
    parser.add_argument("--push-rate", type=float, default=0.001)
    parser.add_argument(
        "--material", default=None, choices=list(MATERIALS), help="override the grasped object's material (pen scene)"
    )
    ...
    global KH
    mat = MATERIALS[args.material] if args.material else None
    if mat:
        KH = mat["kh"]  # pressure scale follows the material stiffness
        _ocap = newton.ModelBuilder.add_shape_capsule  # pen object is the only capsule
        ...
        def _pcap(self, *a, **k):
            ...
            if cfg is not None:
                cfg.kh = mat["kh"]
                cfg.mu = mat["mu"]
                cfg.restitution = mat["restitution"]
                cfg.density = mat["density"]
            return _ocap(self, *a, **k)

        newton.ModelBuilder.add_shape_capsule = _pcap
    ...
    pen_cos = np.where((_nL > 1e-6) & (_nR > 1e-6), np.einsum("ij,ij->i", sLarr, sRarr) / (_nL * _nR + 1e-12), np.nan)
    for f in range(args.frames):
        ...
        for ax, (name, _, _), fi in ((axL, fingers[0], 0), (axR, fingers[1], 1)):
            ...
            if Ln > 1e-9:
                ...
                for s in (1, -1):
                    ax.plot(
                        [q0[0] + s * perp[0], q1[0] + s * perp[0]],
                        [q0[1] + s * perp[1], q1[1] + s * perp[1]],
                        color="cyan",
                        lw=1.3,
                        zorder=7,
                    )
            ...
            ax.set_xlim(e[0], e[1])
            ax.set_ylim(e[2], e[3])  # pen would expand the view
        ...
        axS.set_ylabel("shear on pen · up  [N]")
        ...
        mtxt = f"   [{args.material}: kh={KH:g}, μ={mat['mu']}, ρ={mat['density']:g} kg/m³]" if mat else ""
        fig.suptitle(f"panda_hydro tactile — frame {f}/{args.frames}  t={t[f]:.2f}s{mtxt}", fontsize=13)
