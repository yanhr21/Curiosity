...
class FrozenRecedingKnotRefiner(torch.nn.Module):
    """Checkpoint knot actor causally replanned around the exact Refiner."""
    ...
    def __init__(
        ...
        super().__init__()
        from sugar_rl.utils.frozen_expert_transition_actor_critic import (
            RefinerRecedingKnotCausalTemporalActor,
        )

        self.env = env
        self.knot_actor = RefinerRecedingKnotCausalTemporalActor().to(device)
        payload = torch.load(checkpoint, map_location=device, weights_only=False)
        state = payload["model_state_dict"]
        actor_state = {
            key.removeprefix("actor."): value
        ...
        self.knot_actor.load_state_dict(actor_state, strict=True)
        self.official_refiner, hidden_dims = load_official_teacher(
            official_checkpoint, device
        )
        if hidden_dims != [512, 256, 128]:
            raise RuntimeError(f"official Refiner geometry drifted: {hidden_dims}")
        self.eval().requires_grad_(False)
        self.latched_correction = torch.zeros(env.num_envs, 29, device=device)
        self.correction_latched = torch.zeros(
            env.num_envs, dtype=torch.bool, device=device
        )
        self.last_latch_t = torch.full(
    ...
    def composition_terms(self, actor_input: torch.Tensor) -> dict[str, torch.Tensor]:
        if tuple(actor_input.shape) != (self.env.num_envs, 9790):
            raise ValueError(
                f"receding-knot actor input is {tuple(actor_input.shape)}, expected "
                f"{(self.env.num_envs, 9790)}"
            )
        raw_knot = self.knot_actor(actor_input)
        if tuple(raw_knot.shape) != (self.env.num_envs, self.ACTION_DIM):
            raise RuntimeError(
                f"receding-knot action shape drifted: {tuple(raw_knot.shape)}"
            )
        if not torch.isfinite(raw_knot).all():
            raise RuntimeError("frozen receding-knot action is non-finite")

        physical_t = self.env.t.detach().clone()
        reset = physical_t == 0
        self.latched_correction[reset] = 0.0
        self.correction_latched[reset] = False
        self.last_latch_t[reset] = -1
        chunk_active = (
            (physical_t >= self.PREFIX_STEPS)
            & (physical_t < self.PREFIX_STEPS + self.CHUNK_STEPS)
        )
        knot_boundary = chunk_active & (
            torch.remainder(
                physical_t - self.PREFIX_STEPS, self.STEPS_PER_KNOT
            )
            == 0
        )
        knot_decision = knot_boundary & (self.last_latch_t != physical_t)
        if bool(knot_decision.any()):
            correction = torch.tanh(raw_knot)
            self.latched_correction[knot_decision] = correction[knot_decision]
            self.correction_latched[knot_decision] = True
            self.last_latch_t[knot_decision] = physical_t[knot_decision]
            self.plan_latches += int(knot_decision.sum().item())
        if bool((chunk_active & ~self.correction_latched).any()):
            raise RuntimeError("receding-knot execution has no causal latch")

        correction = torch.zeros(self.env.num_envs, 29, device=actor_input.device)
        correction[chunk_active] = self.latched_correction[chunk_active]
        physical_obs = actor_input[:, :obs_890.OBS_DIM_890]
        official_action = self.official_refiner(physical_obs)
        composed_action = official_action + correction
        if not torch.isfinite(composed_action).all():
            raise RuntimeError("frozen receding-knot composed action is non-finite")
        ...
        self.nonzero_post_chunk_correction_calls += int(
            (nonzero & (physical_t >= self.PREFIX_STEPS + self.CHUNK_STEPS))
            .sum()
            .item()
        )
        self.action_calls += int(physical_t.numel())
        ...
        self.maximum_raw_plan = max(
            self.maximum_raw_plan, float(raw_knot.abs().max().item())
        )
        return {
            "expert_retention": torch.ones_like(raw_knot[:, :1]),
            "selected_endpoint_action": official_action,
            "bounded_residual_action": correction,
            "composed_action": composed_action,
            "raw_plan_action": raw_knot,
            "chunk_active": chunk_active[:, None],
        }
...
def main() -> int:
    ...
    parser.add_argument(
        "--receding-knot",
        action="store_true",
        help="evaluate seven causal five-step replans instead of one open-loop plan",
    )
    ...
    actor_class = (
        FrozenRecedingKnotRefiner if args.receding_knot else FrozenActionChunkRefiner
    )
    actor = actor_class(env, args.checkpoint, args.official_refiner, device)
    records: list[dict] = []
    batch_count = 0
    ...
    expected_latches = batch_count * args.num_envs * (
        actor.KNOT_COUNT if args.receding_knot else 1
    )
    checks = {
        ...
        (
            "seven_knots_latched_per_world_per_batch"
            if args.receding_knot
            else "one_plan_latched_per_world_per_batch"
        ): actor.plan_latches == expected_latches,
    ...
    result = {
        "protocol": (
            "sugar_newton_refiner_receding_knot_frozen_v1"
            if args.receding_knot
            else "sugar_newton_refiner_action_chunk_frozen_v1"
        ),
        "seed": args.seed,
        ...
        "receding_knot_recovery": args.receding_knot,
