...
- **Four reward terms are missing**: `feet_slide`, `feet_air_time`, `undesired_contacts`,
  `hoi_contact` (`rewards.OMITTED`). All four are per-body contact-force terms. The
  tactile work has since made those forces reachable -- `tactile/reducer.py` reduces
  Newton's contact array per patch -- so the blocker is now wiring, not availability.
  Before reinstating any of them, read the Plan-15 audit: in the IsaacLab line
  `hoi_contact` was dead and `undesired_contacts` cancelled the entire positive reward
  (`claude_context/findings.md`). Porting them naively reproduces that bug.
...
are numerically worse, and worlds do not collide. Newton's `replicate` docstring
recommends `approximate_meshes()` before replication so one simplified mesh is shared;
that is convex approximation of interacting geometry, which this project does not do,
because a convex box loses the graspable features the tactile channel exists to measure.

What it does instead is quadric decimation, which preserves concavity: the RL env defaults
to `box_tris=2000` and leaves the hands at full resolution (`hand_tris=0`), and the
