...
class VideoRecorder:
    ...
    def __init__(self, clip: str = "data_000", start: int = 0, frames: int = 400,
                 ...
                 teacher_motion_root: str | Path | None = None):
        ...
        self.teacher_motion_root = teacher_motion_root
    ...
    def _ensure(self) -> bool:
        ...
        try:
            ...
            if self.motion_root is not None:
                env_kwargs["motion_root"] = self.motion_root
            if self.teacher_motion_root is not None:
                env_kwargs["teacher_motion_root"] = self.teacher_motion_root
            self.env = CarryBoxEnv(
                num_envs=1, clip_names=[self.clip], episode_length=10**9,
                mu=self.mu, substeps=self.substeps, device=self.device,
                auto_reset=False, **env_kwargs,
            )
    ...
    def record(self, policy, iteration: int) -> tuple[str | None, dict]:
        ...
        env.reset(env_ids, motion_ids=0, start_frames=self.start)
        ...
        ref_end = min(self.start + len(frames), int(env.ref["length"][env.motion_id[0]]))
        ref = env.ref["obj_pos"][env.motion_id[0], self.start:ref_end, 2]
        stats = {
