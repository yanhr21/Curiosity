...
def parse_motion_id(folder_name: str) -> int:
    """Match SUGAR's ``data_{motion_id}[_env_id]`` folder convention."""
    parts = folder_name.split("_")
    if len(parts) < 2 or parts[0] != "data":
        raise ValueError(f"invalid SUGAR motion folder name: {folder_name}")
    try:
        return int(parts[1])
    except ValueError as exc:
        raise ValueError(f"invalid SUGAR motion folder name: {folder_name}") from exc


def load_clips(root: Path, names: list[str]) -> dict:
    ...
    for name in names:
        folder = root / name
        d = np.load(folder / "robot_50hz.npz", allow_pickle=True)
...
class CarryBoxEnv:
    ...
    def __init__(self, num_envs: int, clip_names: list[str] | None = None,
                 motion_root: str | Path = CLIPS,
                 teacher_motion_root: str | Path = CLIPS,
        ...
        self.teacher_motion_root = Path(teacher_motion_root).resolve()

        if clip_names is None:
            clip_names = sorted(p.name for p in self.motion_root.glob("data_*") if p.is_dir())
        if not clip_names:
        ...
        # This is the exact alignment contract in SUGAR MotionCommand: tracker/reference
        # folders may contain data_{motion_id}_{env_id}, while the frozen Refiner teacher
        # reads the matching raw data_{motion_id}. Missing IDs are fatal here instead of
        # silently feeding the critic motion to the teacher.
        teacher_by_id = {
            parse_motion_id(p.name): p.name
            for p in self.teacher_motion_root.glob("data_*") if p.is_dir()
        }
        missing = sorted({parse_motion_id(name) for name in clip_names} - teacher_by_id.keys())
        if missing:
            raise FileNotFoundError(
                f"teacher motions missing IDs {missing[:10]} under {self.teacher_motion_root}"
            )
        ...
        self.teacher_ref = {
            k: (torch.as_tensor(v, device=self.device) if isinstance(v, np.ndarray) else v)
            for k, v in teacher_raw.items()
        }
    ...
    def reset(self, env_ids: torch.Tensor | None = None, *,
              motion_ids: torch.Tensor | int | None = None,
              start_frames: torch.Tensor | int | None = None) -> None:
        if env_ids is None:
        ...
        if motion_ids is None:
            ...
            selected = torch.as_tensor(motion_ids, dtype=torch.long, device=self.device).flatten()
            if selected.numel() == 1:
                selected = selected.expand(n)
            if selected.numel() != n or ((selected < 0) | (selected >= self.num_motions)).any():
                raise ValueError("motion_ids must contain one valid ID per reset environment")
        self.motion_id[env_ids] = selected
        ...
        if start_frames is None:
            ...
            if starts.numel() == 1:
                starts = starts.expand(n)
            if starts.numel() != n or (starts < 0).any() or (starts >= lengths).any():
                raise ValueError("start_frames must contain one in-range frame per environment")
        self.start[env_ids] = starts
