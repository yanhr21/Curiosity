...
class RefinerRecedingKnotVecEnv(RefinerVecEnv):
    """Replan one bounded 29-D correction every five steps in the chunk."""
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(env, **kwargs)
        if self.policy_history_steps != 10:
            raise ValueError("receding-knot controller requires exact 10-frame history")
        checkpoint = Path(teacher_checkpoint).expanduser().resolve()
        if not checkpoint.is_file():
            raise FileNotFoundError(checkpoint)
        from sugar_newton.validation.refiner_open_loop import (
        ...
        self.teacher_checkpoint = checkpoint
        self.teacher_checkpoint_sha256 = sha256(checkpoint)
        self.latched_correction = torch.zeros(
            self.num_envs, N_DOF, device=self.device
        )
        ...
        self.cfg.update(
            ...
            receding_knot_count=self.KNOT_COUNT,
            receding_knot_steps=self.STEPS_PER_KNOT,
            receding_knot_horizon=self.CHUNK_STEPS,
            receding_knot_correction_bound=1.0,
            receding_knot_future_or_outcome_input=False,
            acting_teacher_checkpoint=str(checkpoint),
            acting_teacher_checkpoint_sha256=self.teacher_checkpoint_sha256,
            training_mask_obs_group="training_handoff_mask",
            training_mask_actor_input=False,
    ...
    def _chunk_active(self) -> torch.Tensor:
        step = self.episode_length_buf
        return (
            (step >= self.PREFIX_STEPS)
            & (step < self.PREFIX_STEPS + self.CHUNK_STEPS)
        )

    def _knot_decision(self) -> torch.Tensor:
        step = self.episode_length_buf
        return self._chunk_active() & (
            torch.remainder(step - self.PREFIX_STEPS, self.STEPS_PER_KNOT) == 0
        )
    ...
    def _obs(self) -> TensorDict:
        ...
        observations.set(
            "training_handoff_mask",
            self._knot_decision().to(torch.float32).unsqueeze(-1),
        )
        return observations

    def reset(self) -> tuple[TensorDict, dict]:
        observations, info = super().reset()
        self.latched_correction.zero_()
        self.correction_latched.zero_()
        return observations, info

    def step(self, actions: torch.Tensor):
        if tuple(actions.shape) != (self.num_envs, self.ACTION_DIM):
            raise ValueError(
                f"receding-knot shape is {tuple(actions.shape)}, expected "
                f"{(self.num_envs, self.ACTION_DIM)}"
            )
        if not torch.isfinite(actions).all():
            raise RuntimeError("receding-knot sample is non-finite")
        knot_decision = self._knot_decision()
        chunk_active = self._chunk_active()
        if bool(knot_decision.any()):
            correction = torch.tanh(actions)
            self.latched_correction[knot_decision] = correction[knot_decision]
            self.correction_latched[knot_decision] = True
            self.cumulative_plan_latches += int(knot_decision.sum().item())
            self.last_plan_action = actions.detach().clone()
        if bool((chunk_active & ~self.correction_latched).any()):
            raise RuntimeError("receding-knot execution has no latched correction")

        correction = torch.zeros(self.num_envs, N_DOF, device=self.device)
        correction[chunk_active] = self.latched_correction[chunk_active]
        with torch.inference_mode():
            teacher_action = self.acting_teacher(self._current_privileged)
        if not torch.isfinite(teacher_action).all():
            raise RuntimeError("receding-knot frozen Refiner action is non-finite")
        executed_action = teacher_action + correction
        self.maximum_teacher_execution_delta = max(
            self.maximum_teacher_execution_delta,
            float(
                (executed_action[~chunk_active] - teacher_action[~chunk_active])
                .abs()
                .max()
                .item()
            )
            if bool((~chunk_active).any())
            else 0.0,
        )
        self.maximum_policy_execution_delta = max(
            self.maximum_policy_execution_delta,
            float(
                (
                    executed_action[chunk_active]
                    - (teacher_action[chunk_active] + correction[chunk_active])
                )
                .abs()
                .max()
                .item()
        ...
        reset_ids = done.nonzero(as_tuple=False).flatten()
        if reset_ids.numel():
            self.latched_correction[reset_ids] = 0.0
            self.correction_latched[reset_ids] = False
        info["episode"].update(
            receding_knot_decision_fraction=knot_decision.float().mean(),
            receding_knot_execution_fraction=chunk_active.float().mean(),
            receding_knot_latches=torch.tensor(
                float(self.cumulative_plan_latches), device=self.device
            ),
            receding_knot_maximum_correction=torch.tensor(
                self.maximum_correction_abs, device=self.device
            ),
        )
        return observations, reward, done, info
...
def make_refiner(
    ...
    *,
    ...
    receding_knot_recovery: bool = False,
    **kwargs,
) -> RefinerVecEnv:
    ...
    if action_chunk_recovery or receding_knot_recovery:
        if failure_frontier_prefix_steps != 200:
            raise ValueError("chunk recovery requires fixed prefix200")
        if failure_frontier_teacher_checkpoint is None:
            raise ValueError("chunk frozen Refiner checkpoint is required")
        if receding_knot_recovery:
            return RefinerRecedingKnotVecEnv(
                env,
                teacher_checkpoint=failure_frontier_teacher_checkpoint,
                **wrapper_kwargs,
            )
