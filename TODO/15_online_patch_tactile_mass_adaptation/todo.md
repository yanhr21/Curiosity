...
- [x] replacement handoff-Z 的恢复链已完成：`151014/151015/151016` 均严格停在有限的
  ...
  retained `241217/server59` 用于 P 训练，backup job `241298` 正在排队。
- [ ] 新 `P`：完成三个匹配 3000-update seeds。seed `151014` 已严格完成并停在有限
  ...
  启动后遭调度器外部取消，最后完整点为 `model_250.pt`；当前在
  `241217/server59` 从 iteration251精确恢复，固定 endpoint 仍为 `model_2999.pt`；
  `151016` 尚未启动。
