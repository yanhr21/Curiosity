...
# Launch Phase 03 curiosity forward-model trainer inside an existing Curiosity
...
WINDOW_NAME="${WINDOW_NAME:-phase03_curiosity_forward_trainer}"
NEWTON_VENV="${NEWTON_VENV:-$ROOT/envs/newton/.venv}"
TRAINER_VENV="${TRAINER_VENV:-$ROOT/envs/residual_adapter/.venv}"
CONFIG="${CONFIG:-$ROOT/experiments/configs/curiosity_forward_model_trainer_v1.json}"
RUN_TAG="${RUN_TAG:-curiosity_forward_model_v1_20260627}"
...
bash -n "$ROOT/experiments/configs/run_curiosity_forward_model_trainer_in_alloc.sh"
sed -n '1,120p' AGENTS.md >/dev/null

log="$ROOT/logs/newton/${RUN_TAG}.log"
remote_cmd="cd $(printf '%q' "$ROOT") && RUN_TAG=$(printf '%q' "$RUN_TAG") RUN_MODE=$(printf '%q' "$RUN_MODE") NEWTON_VENV=$(printf '%q' "$NEWTON_VENV") TRAINER_VENV=$(printf '%q' "$TRAINER_VENV") CONFIG=$(printf '%q' "$CONFIG") DEVICE=$(printf '%q' "$DEVICE") bash $(printf '%q' "$ROOT/experiments/configs/run_curiosity_forward_model_trainer_in_alloc.sh")"
...
tmux send-keys -t "$window_id" \
  "$cmd 2>&1 | tee '$log'; printf '\nTMUX_CURIOSITY_FORWARD_TRAINER_EXIT=%s\n' \"\${PIPESTATUS[0]}\"" C-m
