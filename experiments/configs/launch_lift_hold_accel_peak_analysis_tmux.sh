...
WINDOW_NAME="${WINDOW_NAME:-lift_hold_accel_peak}"
...
bash -n "$ROOT/experiments/configs/run_lift_hold_accel_peak_analysis_in_alloc.sh"
sed -n '1,120p' AGENTS.md >/dev/null

log="$ROOT/logs/newton/${RUN_TAG}_accel_peak_analysis.log"
remote_cmd="cd $(printf '%q' "$ROOT") && RUN_TAG=$(printf '%q' "$RUN_TAG") NEWTON_VENV=$(printf '%q' "$NEWTON_VENV") TOP_K=$(printf '%q' "$TOP_K") bash $(printf '%q' "$ROOT/experiments/configs/run_lift_hold_accel_peak_analysis_in_alloc.sh")"
...
tmux send-keys -t "$window_id" \
  "$cmd 2>&1 | tee '$log'; printf '\nTMUX_LIFT_HOLD_ACCEL_PEAK_EXIT=%s\n' \"\${PIPESTATUS[0]}\"" C-m
