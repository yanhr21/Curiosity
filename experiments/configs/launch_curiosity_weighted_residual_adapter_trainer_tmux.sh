...
# Launch the Phase 03 curiosity-weighted residual adapter trainer inside an
# existing Curiosity tmux-held allocation. This script submits no allocation.

ROOT="${ROOT:-/public/home/yanhongru/Curiosity}"
JOB_ID="${JOB_ID:-}"
TMUX_SESSION="${TMUX_SESSION:-curiosity_forward_alloc_20260627_105456}"
WINDOW_NAME="${WINDOW_NAME:-phase03_curiosity_weighted_residual}"
NEWTON_VENV="${NEWTON_VENV:-$ROOT/envs/newton/.venv}"
TRAINER_VENV="${TRAINER_VENV:-$ROOT/envs/residual_adapter/.venv}"
CONFIG="${CONFIG:-$ROOT/experiments/configs/curiosity_weighted_residual_adapter_trainer_v1.json}"
RUN_TAG="${RUN_TAG:-curiosity_weighted_residual_adapter_v1_20260627}"
...
if [[ "$RUN_MODE" == "train" && "${ALLOW_REAL_TRAINING:-0}" != "1" ]]; then
  echo "ERROR: RUN_MODE=train requires ALLOW_REAL_TRAINING=1 after trainer review." >&2
  exit 5
fi

bash -n "$ROOT/experiments/configs/run_curiosity_weighted_residual_adapter_trainer_in_alloc.sh"
sed -n '1,120p' AGENTS.md >/dev/null

log="$ROOT/logs/newton/${RUN_TAG}.log"
remote_cmd="cd $(printf '%q' "$ROOT") && RUN_TAG=$(printf '%q' "$RUN_TAG") RUN_MODE=$(printf '%q' "$RUN_MODE") NEWTON_VENV=$(printf '%q' "$NEWTON_VENV") TRAINER_VENV=$(printf '%q' "$TRAINER_VENV") CONFIG=$(printf '%q' "$CONFIG") DEVICE=$(printf '%q' "$DEVICE") bash $(printf '%q' "$ROOT/experiments/configs/run_curiosity_weighted_residual_adapter_trainer_in_alloc.sh")"
...
tmux send-keys -t "$window_id" \
  "$cmd 2>&1 | tee '$log'; printf '\nTMUX_CURIOSITY_WEIGHTED_RESIDUAL_EXIT=%s\n' \"\${PIPESTATUS[0]}\"" C-m
