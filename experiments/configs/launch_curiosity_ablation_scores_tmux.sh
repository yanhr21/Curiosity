...
# Launch Phase07 curiosity ablation score generation inside an existing
# Curiosity tmux-held allocation. This script submits no new allocation and
# does not update a policy.

ROOT="${ROOT:-/public/home/yanhongru/Curiosity}"
JOB_ID="${JOB_ID:-}"
TMUX_SESSION="${TMUX_SESSION:-curiosity_forward_alloc_20260627_105456}"
WINDOW_NAME="${WINDOW_NAME:-phase07_curiosity_ablation_scores}"
NEWTON_VENV="${NEWTON_VENV:-$ROOT/envs/newton/.venv}"
TRAINER_VENV="${TRAINER_VENV:-$ROOT/envs/residual_adapter/.venv}"
CONFIG="${CONFIG:-$ROOT/experiments/configs/phase07_curiosity_ablation_scores_v1.json}"
RUN_TAG="${RUN_TAG:-phase07_curiosity_ablation_scores_v1_20260627}"
...
bash -n "$ROOT/experiments/configs/run_curiosity_ablation_scores_in_alloc.sh"
sed -n '1,120p' AGENTS.md >/dev/null

log="$ROOT/logs/newton/${RUN_TAG}.log"
remote_cmd="cd $(printf '%q' "$ROOT") && RUN_TAG=$(printf '%q' "$RUN_TAG") NEWTON_VENV=$(printf '%q' "$NEWTON_VENV") TRAINER_VENV=$(printf '%q' "$TRAINER_VENV") CONFIG=$(printf '%q' "$CONFIG") DEVICE=$(printf '%q' "$DEVICE") bash $(printf '%q' "$ROOT/experiments/configs/run_curiosity_ablation_scores_in_alloc.sh")"
...
tmux send-keys -t "$window_id" \
  "$cmd 2>&1 | tee '$log'; printf '\nTMUX_CURIOSITY_ABLATION_SCORES_EXIT=%s\n' \"\${PIPESTATUS[0]}\"" C-m
