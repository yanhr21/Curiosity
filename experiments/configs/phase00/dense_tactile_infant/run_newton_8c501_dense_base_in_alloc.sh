...
RUN_TAG="${RUN_TAG:-p00_dense_8c501_$(date +%Y%m%d_%H%M%S)}"
...
OUTPUT_DIR="${OUTPUT_DIR:-$ROOT/experiments/outputs/phase00/dense_tactile_infant/newton_8c501/base/$RUN_TAG}"
VIS_DIR="${VIS_DIR:-$ROOT/experiments/visuals/phase00/dense_tactile_infant/newton_8c501/base/$RUN_TAG}"
REPORT_DIR="${REPORT_DIR:-$ROOT/experiments/reports/phase00/dense_tactile_infant/newton_8c501/base/$RUN_TAG}"
LOG_DIR="${LOG_DIR:-$ROOT/logs/newton/phase00/dense_tactile_infant/newton_8c501/base/$RUN_TAG}"
...
if ! printf '%s\n' "$gpu_names" | grep -qi 'H200'; then
  echo "ERROR: Phase 00 dense tactile base export requires an H200 allocation." >&2
  printf '%s\n' "$gpu_names" >&2
  exit 5
fi

newton_commit="$(git -C "$NEWTON_ROOT" rev-parse HEAD)"
run_log="$LOG_DIR/newton_8c501_dense_base.log"

echo "PHASE00_DENSE_TACTILE_BASE_EXPORT_START"
...
echo "NOTE=candidate_newton_mjw_dense_tactile_base_not_curiosity_training"
...
echo "PHASE00_DENSE_TACTILE_BASE_EXPORT_END"
