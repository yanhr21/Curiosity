...
WINDOW_NAME="${WINDOW_NAME:-dense_base}"
RUN_TAG="${RUN_TAG:-p00_dense_8c501_$(date +%Y%m%d_%H%M%S)}"
LOG_PATH="${LOG_PATH:-$ROOT/logs/newton/phase00/dense_tactile_infant/newton_8c501/base/${RUN_TAG}.srun.log}"
...
bash -n "$ROOT/experiments/configs/phase00/dense_tactile_infant/run_newton_8c501_dense_base_in_alloc.sh"

env_file="$ROOT/logs/newton/phase00/dense_tactile_infant/newton_8c501/base/${RUN_TAG}_env.sh"
{
  printf 'ROOT=%q\n' "$ROOT"
  printf 'RUN_TAG=%q\n' "$RUN_TAG"
...
remote_cmd="cd $(printf '%q' "$ROOT") && set -a && source $(printf '%q' "$env_file") && set +a && bash experiments/configs/phase00/dense_tactile_infant/run_newton_8c501_dense_base_in_alloc.sh"
...
tmux send-keys -t "$window_id" \
  "$srun_cmd 2>&1 | tee '$LOG_PATH'; printf '\nPHASE00_DENSE_TACTILE_BASE_SRUN_EXIT=%s\n' \"\${PIPESTATUS[0]}\"" C-m
...
LOG_PATH=$LOG_PATH
ENV_FILE=$env_file
DOWNSTREAM_USE=active_phase00_dense_tactile_base_evidence
EOF
