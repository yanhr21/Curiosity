...
WINDOW_NAME="${WINDOW_NAME:-p00_chan_audit}"
RUN_TAG="${RUN_TAG:-p00_chan_audit_$(date +%Y%m%d_%H%M%S)}"
LOG_PATH="${LOG_PATH:-$ROOT/logs/newton/phase00/ref_tactile/channel_audit/${RUN_TAG}.srun.log}"
...
bash -n "$ROOT/experiments/configs/phase00/ref_tactile/run_channel_semantic_audit_in_alloc.sh"

env_file="$ROOT/logs/newton/phase00/ref_tactile/channel_audit/${RUN_TAG}_env.sh"
{
  printf 'ROOT=%q\n' "$ROOT"
  printf 'RUN_TAG=%q\n' "$RUN_TAG"
  ...
  if [[ -n "${REFERENCE_VIDEO:-}" ]]; then
    printf 'REFERENCE_VIDEO=%q\n' "$REFERENCE_VIDEO"
  fi
  if [[ -n "${CANDIDATE_VIDEO:-}" ]]; then
    printf 'CANDIDATE_VIDEO=%q\n' "$CANDIDATE_VIDEO"
  fi
  if [[ -n "${CANDIDATE_SUMMARY:-}" ]]; then
    printf 'CANDIDATE_SUMMARY=%q\n' "$CANDIDATE_SUMMARY"
  fi
  if [[ -n "${GATE_REVIEW_SUMMARY:-}" ]]; then
    printf 'GATE_REVIEW_SUMMARY=%q\n' "$GATE_REVIEW_SUMMARY"
  fi
} >"$env_file"

remote_cmd="cd $(printf '%q' "$ROOT") && set -a && source $(printf '%q' "$env_file") && set +a && bash experiments/configs/phase00/ref_tactile/run_channel_semantic_audit_in_alloc.sh"
...
tmux send-keys -t "$window_id" \
  "$srun_cmd 2>&1 | tee '$LOG_PATH'; printf '\nPHASE00_CHANNEL_SEMANTIC_AUDIT_SRUN_EXIT=%s\n' \"\${PIPESTATUS[0]}\"" C-m
...
LOG_PATH=$LOG_PATH
ENV_FILE=$env_file
DOWNSTREAM_USE=channel_semantic_audit_not_training_not_curiosity_success
EOF
