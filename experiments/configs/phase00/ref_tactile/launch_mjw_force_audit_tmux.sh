...
WINDOW_NAME="${WINDOW_NAME:-p00_mjw_audit}"
...
bash -n "$ROOT/experiments/configs/phase00/ref_tactile/run_mjw_force_audit_in_alloc.sh"
...
remote_cmd="cd $(printf '%q' "$ROOT") && set -a && source $(printf '%q' "$env_file") && set +a && bash experiments/configs/phase00/ref_tactile/run_mjw_force_audit_in_alloc.sh"
...
tmux send-keys -t "$window_id" \
  "$srun_cmd 2>&1 | tee '$LOG_PATH'; printf '\nPHASE00_MJW_FORCE_AUDIT_SRUN_EXIT=%s\n' \"\${PIPESTATUS[0]}\"" C-m
...
LOG_PATH=$LOG_PATH
ENV_FILE=$env_file
DOWNSTREAM_USE=official_newton_hydro_mjwarp_force_array_audit_not_training_not_curiosity_success
EOF
