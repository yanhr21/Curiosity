...
WINDOW_NAME="${WINDOW_NAME:-p00_sync}"
RUN_TAG="${RUN_TAG:-p00_sync_hydro_$(date +%Y%m%d_%H%M%S)}"
...
bash -n "$ROOT/experiments/configs/phase00/ref_tactile/run_sync_hydro_diagnostic_in_alloc.sh"
...
env_file="$ROOT/logs/newton/phase00/ref_tactile/newton_hydro/${RUN_TAG}_env.sh"
{
  ...
  if [[ -n "${SCENE_CAMERA:-}" ]]; then
    printf 'SCENE_CAMERA=%q\n' "$SCENE_CAMERA"
  fi
  if [[ -n "${SCENE_CAMERA_WIDTH:-}" ]]; then
    printf 'SCENE_CAMERA_WIDTH=%q\n' "$SCENE_CAMERA_WIDTH"
  fi
  if [[ -n "${SCENE_CAMERA_HEIGHT:-}" ]]; then
    printf 'SCENE_CAMERA_HEIGHT=%q\n' "$SCENE_CAMERA_HEIGHT"
  fi
} >"$env_file"

remote_cmd="cd $(printf '%q' "$ROOT") && set -a && source $(printf '%q' "$env_file") && set +a && bash experiments/configs/phase00/ref_tactile/run_sync_hydro_diagnostic_in_alloc.sh"
...
tmux send-keys -t "$window_id" \
  "$srun_cmd 2>&1 | tee '$LOG_PATH'; printf '\nPHASE00_SYNC_HYDRO_DIAGNOSTIC_SRUN_EXIT=%s\n' \"\${PIPESTATUS[0]}\"" C-m
...
LOG_PATH=$LOG_PATH
ENV_FILE=$env_file
DOWNSTREAM_USE=official_newton_hydro_sync_diagnostic_not_training_not_curiosity_success
EOF
