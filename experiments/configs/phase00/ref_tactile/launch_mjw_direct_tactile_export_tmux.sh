#!/usr/bin/env bash
set -euo pipefail

ROOT="${ROOT:-/public/home/yanhongru/Curiosity}"
JOB_ID="${JOB_ID:-}"
TMUX_SESSION="${TMUX_SESSION:-curiosity_phase00_ref_tactile}"
WINDOW_NAME="${WINDOW_NAME:-p00_mjw_direct}"
RUN_TAG="${RUN_TAG:-p00_mjw_direct_tac_$(date +%Y%m%d_%H%M%S)}"
LOG_PATH="${LOG_PATH:-$ROOT/logs/newton/phase00/ref_tactile/newton_hydro/${RUN_TAG}.srun.log}"

cd "$ROOT"
mkdir -p "$(dirname "$LOG_PATH")"

if [[ -z "$JOB_ID" ]]; then
  echo "ERROR: JOB_ID must be set to a running Curiosity tmux-held H200 allocation." >&2
  exit 2
fi
if ! tmux has-session -t "$TMUX_SESSION" 2>/dev/null; then
  echo "ERROR: tmux session not found: $TMUX_SESSION" >&2
  exit 3
fi
job_state="$(squeue -h -j "$JOB_ID" -o '%T' | head -n 1)"
if [[ "$job_state" != "RUNNING" ]]; then
  echo "ERROR: Slurm job $JOB_ID is not running yet." >&2
  squeue -j "$JOB_ID" -o '%.18i %.9P %.32j %.8u %.2t %.10M %.6D %R' >&2 || true
  exit 4
fi
if tmux list-windows -t "$TMUX_SESSION" -F '#W' | grep -qx "$WINDOW_NAME"; then
  echo "ERROR: tmux window already exists: $TMUX_SESSION:$WINDOW_NAME" >&2
  exit 5
fi

bash -n "$ROOT/experiments/configs/phase00/ref_tactile/run_mjw_direct_tactile_export_in_alloc.sh"

env_file="$ROOT/logs/newton/phase00/ref_tactile/newton_hydro/${RUN_TAG}_env.sh"
{
  printf 'ROOT=%q\n' "$ROOT"
  printf 'RUN_TAG=%q\n' "$RUN_TAG"
  if [[ -n "${NEWTON_ROOT:-}" ]]; then
    printf 'NEWTON_ROOT=%q\n' "$NEWTON_ROOT"
  fi
  if [[ -n "${NEWTON_VENV:-}" ]]; then
    printf 'NEWTON_VENV=%q\n' "$NEWTON_VENV"
  fi
  if [[ -n "${DEVICE:-}" ]]; then
    printf 'DEVICE=%q\n' "$DEVICE"
  fi
  if [[ -n "${NUM_FRAMES:-}" ]]; then
    printf 'NUM_FRAMES=%q\n' "$NUM_FRAMES"
  fi
  if [[ -n "${SCENE:-}" ]]; then
    printf 'SCENE=%q\n' "$SCENE"
  fi
  if [[ -n "${MAP_SIZE:-}" ]]; then
    printf 'MAP_SIZE=%q\n' "$MAP_SIZE"
  fi
  if [[ -n "${FPS:-}" ]]; then
    printf 'FPS=%q\n' "$FPS"
  fi
  if [[ -n "${MATERIAL_LABEL:-}" ]]; then
    printf 'MATERIAL_LABEL=%q\n' "$MATERIAL_LABEL"
  fi
  if [[ -n "${OVERRIDE_MU:-}" ]]; then
    printf 'OVERRIDE_MU=%q\n' "$OVERRIDE_MU"
  fi
  if [[ -n "${OVERRIDE_KH:-}" ]]; then
    printf 'OVERRIDE_KH=%q\n' "$OVERRIDE_KH"
  fi
  if [[ -n "${SCENE_CAMERA:-}" ]]; then
    printf 'SCENE_CAMERA=%q\n' "$SCENE_CAMERA"
  fi
  if [[ -n "${SCENE_CAMERA_WIDTH:-}" ]]; then
    printf 'SCENE_CAMERA_WIDTH=%q\n' "$SCENE_CAMERA_WIDTH"
  fi
  if [[ -n "${SCENE_CAMERA_HEIGHT:-}" ]]; then
    printf 'SCENE_CAMERA_HEIGHT=%q\n' "$SCENE_CAMERA_HEIGHT"
  fi
} >"$env_file"

remote_cmd="cd $(printf '%q' "$ROOT") && set -a && source $(printf '%q' "$env_file") && set +a && bash experiments/configs/phase00/ref_tactile/run_mjw_direct_tactile_export_in_alloc.sh"
srun_cmd="cd $(printf '%q' "$ROOT") && srun --jobid=$(printf '%q' "$JOB_ID") --overlap --export=ALL --nodes=1 --ntasks=1 --cpus-per-task=4 --gres=gpu:1 bash -lc $(printf '%q' "$remote_cmd")"

window_id="$(tmux new-window -P -F '#{window_id}' -t "$TMUX_SESSION" -n "$WINDOW_NAME")"
tmux send-keys -t "$window_id" \
  "$srun_cmd 2>&1 | tee '$LOG_PATH'; printf '\nPHASE00_MJW_DIRECT_TACTILE_EXPORT_SRUN_EXIT=%s\n' \"\${PIPESTATUS[0]}\"" C-m

cat <<EOF
RUN_TAG=$RUN_TAG
TMUX_SESSION=$TMUX_SESSION
WINDOW_NAME=$WINDOW_NAME
JOB_ID=$JOB_ID
LOG_PATH=$LOG_PATH
ENV_FILE=$env_file
DOWNSTREAM_USE=candidate_mjwarp_direct_force_tactile_not_training_not_curiosity_success
EOF
