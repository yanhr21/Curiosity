...
WINDOW_NAME="${WINDOW_NAME:-p00_refcmp}"
RUN_TAG="${RUN_TAG:-p00_refcmp_$(date +%Y%m%d_%H%M%S)}"
LOG_PATH="${LOG_PATH:-$ROOT/logs/newton/phase00/ref_tactile/ref_compare/${RUN_TAG}.srun.log}"
...
bash -n "$ROOT/experiments/configs/phase00/ref_tactile/run_reference_video_compare_in_alloc.sh"

env_file="$ROOT/logs/newton/phase00/ref_tactile/ref_compare/${RUN_TAG}_env.sh"
{
  printf 'ROOT=%q\n' "$ROOT"
  printf 'RUN_TAG=%q\n' "$RUN_TAG"
...
remote_cmd="cd $(printf '%q' "$ROOT") && set -a && source $(printf '%q' "$env_file") && set +a && bash experiments/configs/phase00/ref_tactile/run_reference_video_compare_in_alloc.sh"
...
tmux send-keys -t "$window_id" \
  "$srun_cmd 2>&1 | tee '$LOG_PATH'; printf '\nPHASE00_REFERENCE_VIDEO_COMPARE_SRUN_EXIT=%s\n' \"\${PIPESTATUS[0]}\"" C-m
...
LOG_PATH=$LOG_PATH
ENV_FILE=$env_file
DOWNSTREAM_USE=reference_video_comparison_not_training_not_curiosity_success
EOF
