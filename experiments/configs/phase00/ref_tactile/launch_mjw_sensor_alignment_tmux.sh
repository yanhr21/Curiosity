...
WINDOW_NAME="${WINDOW_NAME:-p00_mjw_align}"
RUN_TAG="${RUN_TAG:-p00_mjw_sensor_align_$(date +%Y%m%d_%H%M%S)}"
LOG_PATH="${LOG_PATH:-$ROOT/logs/newton/phase00/ref_tactile/mujoco_align/${RUN_TAG}.srun.log}"
...
bash -n "$ROOT/experiments/configs/phase00/ref_tactile/run_mjw_sensor_alignment_in_alloc.sh"

env_file="$ROOT/logs/newton/phase00/ref_tactile/mujoco_align/${RUN_TAG}_env.sh"
{
  printf 'ROOT=%q\n' "$ROOT"
  printf 'RUN_TAG=%q\n' "$RUN_TAG"
  if [[ -n "${NEWTON_ROOT:-}" ]]; then
  ...
  if [[ -n "${MATERIAL_LABEL:-}" ]]; then
    printf 'MATERIAL_LABEL=%q\n' "$MATERIAL_LABEL"
  fi
  if [[ -n "${OVERRIDE_MU:-}" ]]; then
    printf 'OVERRIDE_MU=%q\n' "$OVERRIDE_MU"
  fi
  if [[ -n "${OVERRIDE_KH:-}" ]]; then
    printf 'OVERRIDE_KH=%q\n' "$OVERRIDE_KH"
  fi
  if [[ -n "${MAX_RELATIVE_RMSE:-}" ]]; then
    printf 'MAX_RELATIVE_RMSE=%q\n' "$MAX_RELATIVE_RMSE"
  fi
  if [[ -n "${MIN_MEAN_COSINE:-}" ]]; then
    printf 'MIN_MEAN_COSINE=%q\n' "$MIN_MEAN_COSINE"
  fi
} >"$env_file"

remote_cmd="cd $(printf '%q' "$ROOT") && set -a && source $(printf '%q' "$env_file") && set +a && bash experiments/configs/phase00/ref_tactile/run_mjw_sensor_alignment_in_alloc.sh"
...
tmux send-keys -t "$window_id" \
  "$srun_cmd 2>&1 | tee '$LOG_PATH'; printf '\nPHASE00_MJW_SENSOR_ALIGNMENT_SRUN_EXIT=%s\n' \"\${PIPESTATUS[0]}\"" C-m
...
LOG_PATH=$LOG_PATH
ENV_FILE=$env_file
DOWNSTREAM_USE=diagnostic_only_candidate_mjwarp_vs_sensorcontact_alignment_not_training
EOF
