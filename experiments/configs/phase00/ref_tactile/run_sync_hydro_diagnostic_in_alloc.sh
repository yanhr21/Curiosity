...
if ! printf '%s\n' "$gpu_names" | grep -qi 'H200'; then
  echo "ERROR: Phase 00 sync hydro diagnostic requires an H200 allocation." >&2
  printf '%s\n' "$gpu_names" >&2
  exit 5
fi

newton_commit="$(git -C "$NEWTON_ROOT" rev-parse HEAD)"
run_log="$LOG_DIR/sync_hydro_diagnostic.log"

echo "PHASE00_SYNC_HYDRO_DIAGNOSTIC_START"
...
echo "SCENE_CAMERA_WIDTH=$SCENE_CAMERA_WIDTH"
echo "SCENE_CAMERA_HEIGHT=$SCENE_CAMERA_HEIGHT"
echo "NOTE=official_newton_hydro_sync_diagnostic_not_training_not_curiosity_success"
...
echo "PHASE00_SYNC_HYDRO_DIAGNOSTIC_END"
