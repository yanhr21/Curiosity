...
WINDOW_NAME="${WINDOW_NAME:-p00_mjc_sensor}"
RUN_TAG="${RUN_TAG:-p00_mjc_sensor_$(date +%Y%m%d_%H%M%S)}"
LOG_PATH="${LOG_PATH:-$ROOT/logs/newton/phase00/ref_tactile/mujoco_sensor/${RUN_TAG}.srun.log}"
...
bash -n "$ROOT/experiments/configs/phase00/ref_tactile/run_mujoco_sensor_probe_in_alloc.sh"

env_file="$ROOT/logs/newton/phase00/ref_tactile/mujoco_sensor/${RUN_TAG}_env.sh"
{
  printf 'ROOT=%q\n' "$ROOT"
  printf 'RUN_TAG=%q\n' "$RUN_TAG"
...
remote_cmd="cd $(printf '%q' "$ROOT") && set -a && source $(printf '%q' "$env_file") && set +a && bash experiments/configs/phase00/ref_tactile/run_mujoco_sensor_probe_in_alloc.sh"
...
tmux send-keys -t "$window_id" \
  "$srun_cmd 2>&1 | tee '$LOG_PATH'; printf '\nPHASE00_MUJOCO_SENSOR_PROBE_SRUN_EXIT=%s\n' \"\${PIPESTATUS[0]}\"" C-m
...
LOG_PATH=$LOG_PATH
ENV_FILE=$env_file
DOWNSTREAM_USE=diagnostic_only_mujoco_contact_sensor_not_active_hydro_base_not_training
EOF
