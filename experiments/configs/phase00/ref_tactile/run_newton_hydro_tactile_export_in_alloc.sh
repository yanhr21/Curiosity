...
RUN_TAG="${RUN_TAG:-p00_hydro_tac_$(date +%Y%m%d_%H%M%S)}"
...
for path in "$NEWTON_ROOT" "$NEWTON_VENV/bin/python" "$ROOT/src/newton_tactile_curiosity/phase00_newton_hydro_export.py"; do
  if [[ ! -e "$path" ]]; then
...
if ! printf '%s\n' "$gpu_names" | grep -qi 'H200'; then
  echo "ERROR: Phase 00 Newton hydro tactile export requires an H200 allocation." >&2
  printf '%s\n' "$gpu_names" >&2
  exit 5
fi

newton_commit="$(git -C "$NEWTON_ROOT" rev-parse HEAD)"
run_log="$LOG_DIR/newton_hydro_tactile_export.log"

echo "PHASE00_NEWTON_HYDRO_TACTILE_EXPORT_START"
...
echo "NOTE=official_newton_hydro_tactile_export_not_training_not_curiosity_success"

(
  cd "$ROOT"
  unset http_proxy https_proxy HTTP_PROXY HTTPS_PROXY ALL_PROXY all_proxy
  export PYTHONPATH="$NEWTON_ROOT:$ROOT/src:${PYTHONPATH:-}"
  timeout 2400 "$NEWTON_VENV/bin/python" "$ROOT/src/newton_tactile_curiosity/phase00_newton_hydro_export.py" \
    --root "$ROOT" \
    --run-tag "$RUN_TAG" \
    --device "$DEVICE" \
    --scene "$SCENE" \
...
echo "PHASE00_NEWTON_HYDRO_TACTILE_EXPORT_END"
