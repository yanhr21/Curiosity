...
WINDOW_NAME="${WINDOW_NAME:-p00_hydro_bench}"
...
bash -n "$ROOT/experiments/configs/phase00/ref_tactile/run_newton_hydro_benchmark_in_alloc.sh"
...
env_file="$ROOT/logs/newton/phase00/ref_tactile/newton_hydro/${RUN_TAG}_env.sh"
{
  ...
  if [[ -n "${BENCHMARK_SECONDS:-}" ]]; then
    printf 'BENCHMARK_SECONDS=%q\n' "$BENCHMARK_SECONDS"
  fi
} >"$env_file"

remote_cmd="cd $(printf '%q' "$ROOT") && set -a && source $(printf '%q' "$env_file") && set +a && bash experiments/configs/phase00/ref_tactile/run_newton_hydro_benchmark_in_alloc.sh"
...
tmux send-keys -t "$window_id" \
  "$srun_cmd 2>&1 | tee '$LOG_PATH'; printf '\nPHASE00_NEWTON_HYDRO_BENCHMARK_SRUN_EXIT=%s\n' \"\${PIPESTATUS[0]}\"" C-m
...
LOG_PATH=$LOG_PATH
ENV_FILE=$env_file
DOWNSTREAM_USE=official_newton_hydro_runtime_benchmark_not_training_not_curiosity_success
EOF
