...
WINDOW_NAME="${WINDOW_NAME:-p00_force_probe}"
...
bash -n "$ROOT/experiments/configs/phase00/ref_tactile/run_force_probe_in_alloc.sh"
...
remote_cmd="cd $(printf '%q' "$ROOT") && set -a && source $(printf '%q' "$env_file") && set +a && bash experiments/configs/phase00/ref_tactile/run_force_probe_in_alloc.sh"
...
LOG_PATH=$LOG_PATH
ENV_FILE=$env_file
DOWNSTREAM_USE=direct_force_probe_not_training_not_curiosity_success
EOF
