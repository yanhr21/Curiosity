...
WINDOW_NAME="${WINDOW_NAME:-alloc_ref_sem}"
JOB_NAME="${JOB_NAME:-curiosity_p00_ref_sem_1gpu_1day}"
PARTITION="${PARTITION:-gpu}"
GRES="${GRES:-gpu:NVIDIAH200:1}"
TIME_LIMIT="${TIME_LIMIT:-1-00:00:00}"
CPUS_PER_TASK="${CPUS_PER_TASK:-8}"
MEMORY="${MEMORY:-64G}"
LOG_DIR="${LOG_DIR:-$ROOT/logs/newton/phase00/ref_tactile/allocation}"
...
alloc_cmd=$(
  cat <<EOF
cd $(printf '%q' "$ROOT")
echo PHASE00_REFERENCE_ALLOCATION_REQUEST_START
...
echo CPUS_PER_TASK=$(printf '%q' "$CPUS_PER_TASK")
echo MEMORY=$(printf '%q' "$MEMORY")
...
srun --partition=$(printf '%q' "$PARTITION") \
  ...
  --pty bash -lc 'echo PHASE00_REFERENCE_ALLOCATION_GRANTED; echo SLURM_JOB_ID=\$SLURM_JOB_ID; echo HOSTNAME=\$(hostname); nvidia-smi --query-gpu=name,index,memory.total --format=csv; cd /public/home/yanhongru/Curiosity; bash -l'
echo PHASE00_REFERENCE_ALLOCATION_EXIT=\$?
EOF
)
...
LOG_PATH=$LOG_PATH
NEXT_CHECK=squeue -u "$USER" -n "$JOB_NAME"
DOWNSTREAM_USE=held_allocation_for_phase00_reference_sanity
EOF
