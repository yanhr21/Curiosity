...
# Launch Phase 04 residual-label source runner inside an existing
...
WINDOW_NAME="${WINDOW_NAME:-phase04_residual_source_runner}"
...
bash -n "$ROOT/experiments/configs/run_residual_label_source_runner_in_alloc.sh"
sed -n '1,120p' AGENTS.md >/dev/null

log="$ROOT/logs/newton/${RUN_TAG}.log"
remote_cmd="cd $(printf '%q' "$ROOT") && RUN_TAG=$(printf '%q' "$RUN_TAG") NEWTON_VENV=$(printf '%q' "$NEWTON_VENV") CONFIG=$(printf '%q' "$CONFIG") DEVICE=$(printf '%q' "$DEVICE") bash $(printf '%q' "$ROOT/experiments/configs/run_residual_label_source_runner_in_alloc.sh")"
...
tmux send-keys -t "$window_id" \
  "$cmd 2>&1 | tee '$log'; printf '\nTMUX_RESIDUAL_LABEL_SOURCE_RUNNER_EXIT=%s\n' \"\${PIPESTATUS[0]}\"" C-m
