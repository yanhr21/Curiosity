...
WINDOW_NAME="${WINDOW_NAME:-eval_smoke}"
RUN_TAG="${RUN_TAG:-p01_dense_clprobe_eval_smoke_$(date +%Y%m%d_%H%M%S)}"
LOG_PATH="${LOG_PATH:-$ROOT/logs/newton/phase01/dense/closed_loop_probe/eval_smoke/${RUN_TAG}.srun.log}"
...
bash -n "$ROOT/experiments/configs/phase01/dense/closed_loop_probe/run_eval_smoke_in_alloc.sh"

env_file="$ROOT/logs/newton/phase01/dense/closed_loop_probe/eval_smoke/${RUN_TAG}_env.sh"
{
  printf 'ROOT=%q\n' "$ROOT"
  printf 'RUN_TAG=%q\n' "$RUN_TAG"
  for name in PYTHON_BIN CHECKPOINT OUTPUT_DIR REPORT_DIR SCENE NUM_FRAMES MAP_SIZE REPETITIONS SEED OVERRIDE_MU OVERRIDE_KH SCORE_LIFT_WEIGHT SCORE_HOLD_WEIGHT SCORE_DROP_WEIGHT HOLD_LIFT_THRESHOLD; do
    if [[ -n "${!name:-}" ]]; then
      printf '%s=%q\n' "$name" "${!name}"
    fi
  done
...
remote_cmd="cd $(printf '%q' "$ROOT") && set -a && source $(printf '%q' "$env_file") && set +a && bash experiments/configs/phase01/dense/closed_loop_probe/run_eval_smoke_in_alloc.sh"
...
LOG_PATH=$LOG_PATH
ENV_FILE=$env_file
DOWNSTREAM_USE=phase01_dense_closed_loop_eval_smoke
EOF
