...
JOB_NAME="${JOB_NAME:-curiosity_p01_dense_probe_smoke_1gpu}"
PARTITION="${PARTITION:-gpu}"
GRES="${GRES:-gpu:NVIDIAH200:1}"
TIME_LIMIT="${TIME_LIMIT:-1-00:00:00}"
CPUS_PER_TASK="${CPUS_PER_TASK:-4}"
MEMORY="${MEMORY:-32G}"
LOG_DIR="${LOG_DIR:-$ROOT/logs/newton/phase01/dense/closed_loop_probe/allocation}"
...
alloc_cmd=$(
  cat <<EOF
cd $(printf '%q' "$ROOT")
echo PHASE01_DENSE_CLOSED_LOOP_PROBE_ALLOCATION_REQUEST_START
...
srun --partition=$(printf '%q' "$PARTITION") \
  ...
  --pty bash -lc 'echo PHASE01_DENSE_CLOSED_LOOP_PROBE_ALLOCATION_GRANTED; echo SLURM_JOB_ID=\$SLURM_JOB_ID; echo HOSTNAME=\$(hostname); nvidia-smi --query-gpu=name,index,memory.total --format=csv; cd /public/home/yanhongru/Curiosity; bash -l'
echo PHASE01_DENSE_CLOSED_LOOP_PROBE_ALLOCATION_EXIT=\$?
EOF
)
...
DOWNSTREAM_USE=held_allocation_for_phase01_dense_closed_loop_probe_smoke
EOF
