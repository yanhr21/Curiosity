...
JOB_ID="${JOB_ID:-}"
TMUX_SESSION="${TMUX_SESSION:-curiosity_phase01_dense}"
WINDOW_NAME="${WINDOW_NAME:-preflight}"
RUN_TAG="${RUN_TAG:-p01_dense_preflight_$(date +%Y%m%d_%H%M%S)}"
LOG_PATH="${LOG_PATH:-$ROOT/logs/newton/phase01/dense/preflight/${RUN_TAG}.srun.log}"
...
bash -n "$ROOT/experiments/configs/phase01/dense/preflight/run_in_alloc.sh"

env_file="$ROOT/logs/newton/phase01/dense/preflight/${RUN_TAG}_env.sh"
{
  printf 'ROOT=%q\n' "$ROOT"
  printf 'RUN_TAG=%q\n' "$RUN_TAG"
  for name in PYTHON_BIN CONFIG EVIDENCE_MANIFEST DESIGN_CONTRACT BASELINE_CONTRACT OUTPUT_DIR REPORT_DIR; do
    if [[ -n "${!name:-}" ]]; then
      printf '%s=%q\n' "$name" "${!name}"
    fi
  done
...
remote_cmd="cd $(printf '%q' "$ROOT") && set -a && source $(printf '%q' "$env_file") && set +a && bash experiments/configs/phase01/dense/preflight/run_in_alloc.sh"
...
LOG_PATH=$LOG_PATH
ENV_FILE=$env_file
DOWNSTREAM_USE=phase01_dense_closed_loop_training_preflight
EOF
