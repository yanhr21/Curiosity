...
WINDOW_NAME="${WINDOW_NAME:-p01_resid_cur}"
RUN_TAG="${RUN_TAG:-p01_resid_cur_$(date +%Y%m%d_%H%M%S)}"
CONFIG="${CONFIG:-$ROOT/experiments/configs/phase01/resid_curiosity_train.json}"
...
LOG_SUBDIR="${LOG_SUBDIR:-phase01/core/resid/curiosity}"
LOG_PATH="${LOG_PATH:-$ROOT/logs/newton/phase01/core/resid/curiosity/${RUN_TAG}.srun.log}"

cd "$ROOT"
mkdir -p logs/newton/phase01/core/resid/curiosity "$(dirname "$LOG_PATH")"
...
bash -n "$ROOT/experiments/configs/phase01/run_resid_curiosity_train_in_alloc.sh"
env_file="$ROOT/logs/newton/phase01/core/resid/curiosity/${RUN_TAG}_env.sh"
{
  printf 'ROOT=%q\n' "$ROOT"
  printf 'RUN_TAG=%q\n' "$RUN_TAG"
  printf 'RUN_MODE=%q\n' "$RUN_MODE"
...
remote_cmd="cd $(printf '%q' "$ROOT") && set -a && source $(printf '%q' "$env_file") && set +a && bash experiments/configs/phase01/run_resid_curiosity_train_in_alloc.sh"
...
tmux send-keys -t "$window_id" \
  "$srun_cmd 2>&1 | tee '$LOG_PATH'; printf '\nPHASE01_RESID_CURIOSITY_TRAIN_SRUN_EXIT=%s\n' \"\${PIPESTATUS[0]}\"" C-m
...
CONFIG=$CONFIG
RUN_MODE=$RUN_MODE
DOWNSTREAM_USE=curiosity_weighted_residual_candidate_not_success_until_heldout_eval
EOF
