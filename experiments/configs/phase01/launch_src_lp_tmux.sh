...
WINDOW_NAME="${WINDOW_NAME:-p01_src_lp}"
...
LOG_PATH="${LOG_PATH:-$ROOT/logs/newton/phase01/core/src_lp/${RUN_TAG}.srun.log}"

cd "$ROOT"
mkdir -p logs/newton/phase01/core/src_lp
...
bash -n "$ROOT/experiments/configs/phase01/run_src_lp_in_alloc.sh"

env_file="$ROOT/logs/newton/phase01/core/src_lp/${RUN_TAG}_env.sh"
{
  printf 'ROOT=%q\n' "$ROOT"
  printf 'RUN_TAG=%q\n' "$RUN_TAG"
  printf 'SRC_LP_CONFIG=%q\n' "$SRC_LP_CONFIG"
...
remote_cmd="cd $(printf '%q' "$ROOT") && set -a && source $(printf '%q' "$env_file") && set +a && bash experiments/configs/phase01/run_src_lp_in_alloc.sh"
...
LOG_PATH=$LOG_PATH
ENV_FILE=$env_file
SRC_LP_CONFIG=$SRC_LP_CONFIG
LP_SCORE_CONFIG=$LP_SCORE_CONFIG
DOWNSTREAM_USE=matched_curiosity_weights_for_phase01_residual_training
EOF
