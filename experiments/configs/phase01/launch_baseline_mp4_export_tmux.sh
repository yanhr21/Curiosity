...
WINDOW_NAME="${WINDOW_NAME:-p01_base_mp4}"
...
tmux new-window -t "$SESSION" -n "$WINDOW_NAME" \
  "cd '$ROOT' && RUN_TAG='$RUN_TAG' BASELINE_RUN_TAG='$BASELINE_RUN_TAG' srun --jobid='$SLURM_JOB_ID' --overlap --export=ALL --nodes=1 --ntasks=1 --cpus-per-task='$ALLOC_CPUS_PER_TASK' --gres=gpu:1 bash '$ROOT/experiments/configs/phase01/run_baseline_mp4_export_in_alloc.sh' 2>&1 | tee '$log_path'; echo PHASE01_BASELINE_MP4_EXPORT_SRUN_EXIT=\${PIPESTATUS[0]}"

echo "Launched Phase 01 baseline MP4 export in tmux session=$SESSION window=$WINDOW_NAME"
