...
WINDOW_NAME="${WINDOW_NAME:-p01_resid_eval}"
RUN_TAG="${RUN_TAG:-p01_resid_eval_$(date +%Y%m%d_%H%M%S)}"
...
LOG_PATH="${LOG_PATH:-$ROOT/logs/newton/phase01/core/resid/base_eval/${RUN_TAG}.srun.log}"

cd "$ROOT"
mkdir -p logs/newton/phase01/core/resid/base_eval
...
env_file="$ROOT/logs/newton/phase01/core/resid/base_eval/${RUN_TAG}_env.sh"
{
  printf 'ROOT=%q\n' "$ROOT"
  printf 'RUN_TAG=%q\n' "$RUN_TAG"
  printf 'CHECKPOINT=%q\n' "$CHECKPOINT"
...
tmux send-keys -t "$window_id" \
  "$srun_cmd 2>&1 | tee '$LOG_PATH'; printf '\nPHASE01_RESID_BASE_EVAL_SRUN_EXIT=%s\n' \"\${PIPESTATUS[0]}\"" C-m
...
DOWNSTREAM_USE=heldout_no_curiosity_residual_baseline_evaluation_not_curiosity_success
EOF
