...
LOG_SUBDIR="${LOG_SUBDIR:-phase01/core/local_adv}"
OUTPUT_SUBDIR="${OUTPUT_SUBDIR:-phase01/core/local_adv_lp}"
...
if ! printf '%s\n' "$gpu_names" | grep -qi 'H200'; then
  echo "ERROR: Phase 01 local-advantage preflight requires H200; observed GPU names:" >&2
  printf '%s\n' "$gpu_names" >&2
  exit 5
fi
...
echo "PHASE01_LOCAL_ADV_START"
echo "RUN_TAG=$RUN_TAG"
echo "SLURM_JOB_ID=$SLURM_JOB_ID"
echo "HOSTNAME=$(hostname)"
echo "GPU_NAMES=$gpu_names"
echo "SEGMENT_CONFIG=$SEGMENT_CONFIG"
echo "LP_SCORE_CONFIG=$LP_SCORE_CONFIG"
echo "NEWTON_VENV=$NEWTON_VENV"
echo "TRAINER_VENV=$TRAINER_VENV"
echo "DEVICE=$DEVICE"
echo "NOTE=local_advantage_source_preflight_and_learning_progress_scoring_not_training"
...
echo "=== LOCAL_ADV_SEGMENTS_START ==="
"$NEWTON_VENV/bin/python" "$ROOT/experiments/configs/phase01/build_local_adv_segments.py" \
  --root "$ROOT" \
  --config "$SEGMENT_CONFIG" \
  --fresh-sanity-json "$sanity_json"
echo "=== LOCAL_ADV_SEGMENTS_END ==="

echo "=== LOCAL_ADV_LP_SCORES_START ==="
"$TRAINER_VENV/bin/python" "$ROOT/experiments/configs/compute_curiosity_learning_progress_v1.py" \
  --config "$LP_SCORE_CONFIG" \
  --root "$ROOT" \
  --fresh-sanity-json "$sanity_json"
echo "=== LOCAL_ADV_LP_SCORES_END ==="
echo "PHASE01_LOCAL_ADV_EXIT=0"
