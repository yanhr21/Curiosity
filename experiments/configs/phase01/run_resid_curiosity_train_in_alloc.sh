...
if ! printf '%s\n' "$gpu_names" | grep -qi 'H200'; then
  echo "ERROR: Phase 01 curiosity residual training requires H200; observed GPU names:" >&2
  printf '%s\n' "$gpu_names" >&2
  exit 6
fi
...
echo "PHASE01_RESID_CURIOSITY_TRAIN_START"
...
echo "NOTE=curiosity_weighted_residual_training_component_not_success_until_heldout_eval"
...
set +e
"$TRAINER_VENV/bin/python" "$ROOT/experiments/configs/train_curiosity_weighted_residual_adapter_v1.py" \
  --config "$CONFIG" \
  --root "$ROOT" \
    ...
    with util_csv.open("r", encoding="utf-8", newline="") as handle:
        ...
        for row in reader:
            try:
                ...
                continue
payload = {
    "classification": "phase01_resid_curiosity_gpu_utilization_v1",
    "run_tag": run_tag,
    "csv": str(util_csv),
...
if summary_path.exists():
    ...
    summary["curiosity_training_component_not_success_until_heldout_eval"] = True
...
echo "PHASE01_RESID_CURIOSITY_TRAIN_END"
