...
RUN_TAG="${RUN_TAG:-phase08_guarded_overlay_probe_direct_v1_20260629}"
EVAL_TAG_PREFIX="${EVAL_TAG_PREFIX:-phase08_guarded_overlay_probe_direct}"
...
REPORT_PATH="${REPORT_PATH:-$ROOT/experiments/reports/2026-06-29_phase08_guarded_overlay_probe_direct_v1.md}"
...
echo "PHASE08_GUARDED_OVERLAY_PROBE_DIRECT_START"
...
run_cell() {
  ...
  echo "=== PHASE08_GUARDED_OVERLAY_DIRECT_CELL_START split=$split method=$method cell=$cell eval_tag=$eval_tag ==="
  RUN_TAG="$eval_tag" \
  ...
  PHYSICS_VARIANT_LABEL="phase08_guarded_overlay_direct_${cell}_${method}" \
  BODY_MASS_SCALE="1.0" \
  ...
  RUN_TAG="$eval_tag" \
  NEWTON_VENV="$NEWTON_VENV" \
  BASELINE_NAME="phase08_guarded_overlay_direct_${method}" \
  MASS_LABEL="$split" \
  FRICTION_LABEL="0.45" \
  ...
  echo "=== PHASE08_GUARDED_OVERLAY_DIRECT_CELL_END split=$split method=$method cell=$cell eval_tag=$eval_tag ==="
}
...
payload = {
    "classification": "phase08_guarded_overlay_probe_direct_v1",
    "run_tag": run_tag,
...
lines = [
    "# Phase08 Guarded Overlay Probe Direct V1",
    "",
...
echo "PHASE08_GUARDED_OVERLAY_PROBE_DIRECT_END"
