...
# Launch Phase 03 curiosity forward-model preflight inside an existing
...
WINDOW_NAME="${WINDOW_NAME:-phase03_curiosity_forward_preflight}"
NEWTON_VENV="${NEWTON_VENV:-$ROOT/envs/newton/.venv}"
CONFIG="${CONFIG:-$ROOT/experiments/configs/curiosity_forward_model_preflight_v1.json}"
RUN_TAG="${RUN_TAG:-curiosity_forward_model_preflight_v1_20260627}"
...
bash -n "$ROOT/experiments/configs/run_curiosity_forward_model_preflight_in_alloc.sh"
sed -n '1,120p' AGENTS.md >/dev/null

log="$ROOT/logs/newton/${RUN_TAG}.log"
remote_cmd="cd $(printf '%q' "$ROOT") && RUN_TAG=$(printf '%q' "$RUN_TAG") NEWTON_VENV=$(printf '%q' "$NEWTON_VENV") CONFIG=$(printf '%q' "$CONFIG") DEVICE=$(printf '%q' "$DEVICE") bash $(printf '%q' "$ROOT/experiments/configs/run_curiosity_forward_model_preflight_in_alloc.sh")"
...
tmux send-keys -t "$window_id" \
  "$cmd 2>&1 | tee '$log'; printf '\nTMUX_CURIOSITY_FORWARD_PREFLIGHT_EXIT=%s\n' \"\${PIPESTATUS[0]}\"" C-m
