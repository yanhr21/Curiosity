...
# Train-only source probe around the accepted validation offset from the
# expanded guarded-overlay probe. This is not training and not a success claim.

ROOT="${ROOT:-/public/home/yanhongru/Curiosity}"
NEWTON_VENV="${NEWTON_VENV:-$ROOT/envs/newton/.venv}"
TRAINER_VENV="${TRAINER_VENV:-$ROOT/envs/residual_adapter/.venv}"
DEVICE="${DEVICE:-cuda:0}"
RUN_TAG="${RUN_TAG:-phase08_guarded_overlay_train_focus_probe_direct_v1_20260629}"
EVAL_TAG_PREFIX="${EVAL_TAG_PREFIX:-phase08_guarded_overlay_train_focus_probe_direct}"
...
REPORT_PATH="${REPORT_PATH:-$ROOT/experiments/reports/2026-06-29_phase08_guarded_overlay_train_focus_probe_direct_v1.md}"
...
echo "PHASE08_GUARDED_OVERLAY_TRAIN_FOCUS_PROBE_DIRECT_START"
echo "RUN_TAG=$RUN_TAG"
echo "SLURM_JOB_ID=$SLURM_JOB_ID"
echo "HOSTNAME=$(hostname)"
echo "OVERLAY_CHECKPOINT=$OVERLAY_CHECKPOINT"
echo "NOTE=train_only_source_probe_not_training_not_success_claim"
...
run_cell() {
  ...
  echo "=== PHASE08_GUARDED_OVERLAY_TRAIN_FOCUS_CELL_START method=$method cell=$cell eval_tag=$eval_tag offset=$grasp_offset_delta ==="
  RUN_TAG="$eval_tag" \
  ...
  PHYSICS_VARIANT_LABEL="phase08_guarded_overlay_train_focus_${cell}_${method}" \
  BODY_MASS_SCALE="1.0" \
  ...
  RUN_TAG="$eval_tag" \
  NEWTON_VENV="$NEWTON_VENV" \
  BASELINE_NAME="phase08_guarded_overlay_train_focus_${method}" \
  MASS_LABEL="train" \
  FRICTION_LABEL="0.45" \
  ...
  echo "=== PHASE08_GUARDED_OVERLAY_TRAIN_FOCUS_CELL_END method=$method cell=$cell eval_tag=$eval_tag ==="
}
...
run_pair() {
  ...
  run_cell "$cell" "guarded_overlay" "lift_hold_feedback_residual_overlay" "$offset"
}

run_pair "pen_end_bias_overlay_train_focus_a" "-0.026,0.0125,0"
run_pair "pen_end_bias_overlay_train_focus_b" "-0.0258,0.0125,0"
run_pair "pen_end_bias_overlay_train_focus_c" "-0.0262,0.0125,0"
run_pair "pen_end_bias_overlay_train_focus_d" "-0.0258,0.0128,0"
run_pair "pen_end_bias_overlay_train_focus_e" "-0.0262,0.0122,0"
...
cells = [
    "pen_end_bias_overlay_train_focus_a",
    "pen_end_bias_overlay_train_focus_b",
    "pen_end_bias_overlay_train_focus_c",
    "pen_end_bias_overlay_train_focus_d",
    "pen_end_bias_overlay_train_focus_e",
]
...
payload = {
    "classification": "phase08_guarded_overlay_train_focus_probe_direct_v1",
    "run_tag": run_tag,
    "status": "pass_train_source_candidates_found" if accepted else "open_no_train_overlay_source_candidates",
    "not_training": True,
    "not_final_curiosity_success_claim": True,
    "overlay_checkpoint": checkpoint,
    "rows": rows,
    "paired": paired,
    "accepted_overlay_train_source_cells": accepted,
    "accepted_overlay_train_source_count": len(accepted),
    "held_out_cells_used": [],
...
lines = [
    "# Phase08 Guarded Overlay Train-Focus Probe Direct V1",
    "",
    "Train-only source probe. Not training and not a success claim.",
    "",
    f"- status: `{payload['status']}`",
    f"- accepted train source count: `{len(accepted)}`",
...
for cell, item in paired.items():
    lines.append(
        f"- `{cell}`: accepted `{item['candidate_for_overlay_training']}`, "
...
echo "PHASE08_GUARDED_OVERLAY_TRAIN_FOCUS_PROBE_DIRECT_END"
