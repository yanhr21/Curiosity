...
# Source/probe only: evaluate guarded feedback as the hold prior and a local
# learned curiosity residual overlay on train/validation cells. Held-out cells
# are not used. This is not training and not a success claim.

ROOT="${ROOT:-/public/home/yanhongru/Curiosity}"
NEWTON_VENV="${NEWTON_VENV:-$ROOT/envs/newton/.venv}"
TRAINER_VENV="${TRAINER_VENV:-$ROOT/envs/residual_adapter/.venv}"
DEVICE="${DEVICE:-cuda:0}"
RUN_TAG="${RUN_TAG:-phase08_guarded_overlay_probe_v1_20260629}"
EVAL_TAG_PREFIX="${EVAL_TAG_PREFIX:-phase08_guarded_overlay_probe}"
...
REPORT_PATH="${REPORT_PATH:-$ROOT/experiments/reports/2026-06-29_phase08_guarded_overlay_probe_v1.md}"
...
if [[ ! -f "$OVERLAY_CHECKPOINT" ]]; then
  echo "ERROR: missing overlay checkpoint: $OVERLAY_CHECKPOINT" >&2
  exit 4
fi

echo "PHASE08_GUARDED_OVERLAY_PROBE_START"
echo "RUN_TAG=$RUN_TAG"
echo "SLURM_JOB_ID=$SLURM_JOB_ID"
echo "HOSTNAME=$(hostname)"
echo "OVERLAY_CHECKPOINT=$OVERLAY_CHECKPOINT"
echo "ACTIVE_THRESHOLD=$ACTIVE_THRESHOLD"
echo "NOTE=train_validation_probe_not_training_not_success_claim"
...
run_cell() {
  ...
  echo "=== PHASE08_GUARDED_OVERLAY_CELL_START split=$split method=$method cell=$cell eval_tag=$eval_tag ==="
  RUN_TAG="$eval_tag" \
  ...
  PHYSICS_VARIANT_LABEL="phase08_guarded_overlay_${cell}_${method}" \
  BODY_MASS_SCALE="1.0" \
  ...
  RUN_TAG="$eval_tag" \
  NEWTON_VENV="$NEWTON_VENV" \
  BASELINE_NAME="phase08_guarded_overlay_${method}" \
  MASS_LABEL="$split" \
  FRICTION_LABEL="0.45" \
  ...
  echo "=== PHASE08_GUARDED_OVERLAY_CELL_END split=$split method=$method cell=$cell eval_tag=$eval_tag ==="
}
...
run_pair "train" "pen_end_bias_train_d" "-0.025,0.015,0"
run_pair "train" "pen_end_bias_train_c" "-0.026,0.012,0"
run_pair "validation" "pen_end_bias_val_c" "-0.025,0.014,0"
run_pair "validation" "pen_end_bias_val_e" "-0.0245,0.0145,0"
...
cells = [
    ...
    ("validation", "pen_end_bias_val_c"),
    ("validation", "pen_end_bias_val_e"),
]
...
payload = {
    "classification": "phase08_guarded_overlay_probe_v1",
    "run_tag": run_tag,
...
lines = [
    "# Phase08 Guarded Overlay Probe V1",
    "",
...
echo "PHASE08_GUARDED_OVERLAY_PROBE_END"
