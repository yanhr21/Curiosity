...
print(config.get("output_dir", "experiments/outputs/curiosity_forward_model_v1_20260627"))
PY
)"
...
summary_path = root / config.get("output_dir", "experiments/outputs/curiosity_forward_model_v1_20260627") / f"{run_tag}_summary.json"
    ...
    with util_csv.open("r", encoding="utf-8", newline="") as handle:
        ...
        for row in reader:
            try:
                ...
                continue
payload = {
    "classification": "curiosity_forward_model_gpu_utilization_v1",
    "run_tag": run_tag,
    "csv": str(util_csv),
