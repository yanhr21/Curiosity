...
def test_sensor_rgb_forcefield(setup_nut_rgb_ff):
    ...
    contact_normal = sensor.data.tactile_contact_normal_w
    reconstructed_tangent = full_velocity - contact_normal * torch.sum(
        contact_normal * full_velocity, dim=-1, keepdim=True
    )
    assert torch.isfinite(full_velocity).all()
    assert torch.any(torch.abs(full_velocity[active]) > 0.0)
    assert torch.all(full_velocity[~active] == 0.0)
    assert torch.all(tangential_velocity[~active] == 0.0)
    assert torch.allclose(
        tangential_velocity[active], reconstructed_tangent[active], atol=1.0e-6, rtol=1.0e-5
    )
