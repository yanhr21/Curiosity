...
class VisuoTactileSensorCfg(SensorBaseCfg):
    ...
    When ``None``, the released TacSL heuristic selects the smallest mesh
    extent. Sensorized assets with curved anatomical backing should declare
    the known axis explicitly so curvature cannot silently change the ray
    direction. Valid values are 0, 1, or 2.
    """

    tactile_tip_direction_sign: int | None = None
    """Optional outward sign along :attr:`tactile_sampling_axis`.

    Valid declared values are ``-1`` or ``1``. When ``None``, the released
    center-of-mass heuristic is retained.
    """

    tactile_sampling_mesh_prim_path: str | None = None
    """Optional exact visual-mesh prim used only for taxel surface sampling.

    When unset, TacSL retains its released first-visual-mesh search. This
    explicit adapter is useful when one deformable root contains multiple
    visual meshes with different bind-pose coordinate representations. The
    selected prim must be a non-collision ``UsdGeom.Mesh`` below the same
    elastomer root. It changes neither SDF queries nor force equations.
    """
